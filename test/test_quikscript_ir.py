...
from quikscript_fea import emit_quikscript_calt
from quikscript_ir import compile_glyph_families, compile_quikscript_ir, expand_join_transforms, flatten_join_glyphs
...
def test_expand_join_transforms_tracks_generated_sources_and_kinds():
    ...
    expanded, transforms = expand_join_transforms(glyphs)

    assert "qsLead.noentry" in expanded
    assert "qsLead.exit-extended" in expanded
    assert "qsFollow.entry-extended" in expanded
    assert expanded["qsLead.noentry"]["_generated_from"] == "qsLead"
    assert expanded["qsLead.exit-extended"]["_transform_kind"] == "exit-extended"
    assert expanded["qsFollow.entry-extended"]["_transform_kind"] == "entry-extended"
...
def test_join_planner_exposes_rules_and_emits_calt_from_plan():
    ...
    plan = plan_quikscript_joins(compiled)
    calt = emit_quikscript_calt(plan, compiled)
