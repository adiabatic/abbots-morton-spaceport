...
def _list_form_extension_data() -> GlyphData:
    return {
        "metadata": {},
        "glyphs": {},
        "glyph_families": {
            ...
            "qsFollow": {
                "prop": {
                    ...
                    "derive": {
                        "extend_entry_after": [
                            {"by": 1, "targets": [{"family": "qsLead"}]},
                            {"by": 2, "targets": [{"family": "qsLeadB"}]},
                        ],
                    },
                },
            },
...
def test_extend_entry_after_accepts_list_form_with_distinct_by_values():
    glyphs, _ = compile_quikscript_ir(_list_form_extension_data(), "junior")

    spec = glyphs["qsFollow"].extend_entry_after
    assert spec is not None
    assert len(spec.groups) == 2
    assert spec.by_for("qsLead") == 1
    assert spec.by_for("qsLeadB") == 2
    assert spec.targets == ("qsLead", "qsLeadB")

    expanded, _ = expand_join_transforms(glyphs, has_zwnj=False)
    assert "qsFollow.entry-extended" in expanded
    assert "qsFollow.entry-doubly-extended" in expanded
    assert expanded["qsFollow.entry-extended"].after == ("qsLead",)
    assert expanded["qsFollow.entry-doubly-extended"].after == ("qsLeadB",)
    # Entry-side widening fills connector ink leftward toward the entry
    # column without shifting the entry anchor itself.
    assert expanded["qsFollow.entry-extended"].entry == ((4, 0),)
    assert expanded["qsFollow.entry-extended"].bitmap == ("   ##",)
    assert expanded["qsFollow.entry-doubly-extended"].entry == ((4, 0),)
    assert expanded["qsFollow.entry-doubly-extended"].bitmap == ("  ###",)


def test_extend_entry_after_single_dict_form_keeps_one_group():
    data = _list_form_extension_data()
    data["glyph_families"]["qsFollow"]["prop"]["derive"] = {
        "extend_entry_after": {
            "by": 1,
            "targets": [{"family": "qsLead"}, {"family": "qsLeadB"}],
        },
    }
    glyphs, _ = compile_quikscript_ir(data, "junior")
...
def test_extend_entry_after_rejects_target_in_two_groups():
    import pytest
    ...
    data["glyph_families"]["qsFollow"]["prop"]["derive"] = {
        "extend_entry_after": [
            {"by": 1, "targets": [{"family": "qsLead"}]},
            {"by": 2, "targets": [{"family": "qsLead"}]},
        ],
    }
    with pytest.raises(ValueError, match="multiple groups"):
        compile_quikscript_ir(data, "junior")


def test_extension_spec_round_trip_preserves_list_vs_dict_shape():
    from quikscript_ir import flatten_join_glyphs

    glyphs, _ = compile_quikscript_ir(_list_form_extension_data(), "junior")
    flat = flatten_join_glyphs(glyphs)
    follow = flat["qsFollow"]
    assert follow["extend_entry_after"] == [
        {"by": 1, "targets": ["qsLead"]},
        {"by": 2, "targets": ["qsLeadB"]},
    ]
