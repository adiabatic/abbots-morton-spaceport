...
def digest(decision, treaty):
    h = hashlib.sha256()
    h.update(repr(sorted(decision.transitions.items()) if hasattr(decision, "transitions") else decision).encode())
...
sets = {
  ...
  "s10L":["qsAh","qsDay","qsIt","qsMay","qsPea","qsUtter","qsSee","qsTea","qsDay_qsUtter","qsSee_qsUtter"],
}
for label, names in sets.items():
    ...
    print(f"{label:5s} n={len(names):2d} wall={dt:7.3f}s windows={len(decision.rows) if hasattr(decision,'rows') else '?'}", flush=True)
