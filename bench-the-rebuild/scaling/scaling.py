...
for n in ligs:
    for part in spec.runes[n].sequence:
        if part not in base: base.append(part)
...
for k in [int(x) for x in sys.argv[1:]] or [6,8,10,12,14,16,18]:
    ...
    cpu=time.process_time()-t; wall=time.perf_counter()-w
    row = dict(runes=len(sub.runes), letters=letters, ligs=len(sub.runes)-letters,
               windows=len(d.transitions), rules=len(d.rules), cells=len(d.reachable_cells()),
               cpu=round(cpu,3), wall=round(wall,3))
    out.append(row)
...
for a,b in zip(out,out[1:]):
    ...
    print(f"{a['runes']:2d}->{b['runes']:2d}   windows {math.log(b['windows']/a['windows'])/r:5.2f}   cpu {math.log(b['cpu']/a['cpu'])/r:5.2f}")
