...
t = time.perf_counter()
blob = json.dumps(enc(spec), sort_keys=True)
...
for k, v in sorted(seen.items(), key=lambda r: -r[1]):
    print(f"   {v:6d}  {k}")
...
pol = {
    k: sum(len(getattr(r.policy, k)) for r in spec.runes.values())
    for k in ("refuse", "prefer", "extend", "contract", "resolve")
}
