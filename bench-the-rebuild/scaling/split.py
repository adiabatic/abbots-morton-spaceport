...
imp_cpu=time.process_time()-t0; imp_w=time.perf_counter()-w0

t0=time.process_time(); w0=time.perf_counter()
spec = load_default_spec()
spec_cpu=time.process_time()-t0; spec_w=time.perf_counter()-w0
...
keep=set()
for n in ligs:
    keep.add(n); keep.update(spec.runes[n].sequence)
for n in names:
    if len(keep)>=KEEP: break
    keep.add(n)
sub = replace(spec, runes={n:r for n,r in spec.runes.items() if n in keep})
print(f"subset runes={len(sub.runes)} (ligatures kept: {sorted(n for n in sub.runes if sub.runes[n].sequence)})")
...
def timed_rules(*a, **k):
    t=time.perf_counter(); r=orig_rules_for_input(*a,**k); acc["rules"]+=time.perf_counter()-t; return r
def timed_flag(*a, **k):
    t=time.perf_counter(); r=orig_flag(*a,**k); acc["flag"]+=time.perf_counter()-t; return r
...
total=time.perf_counter()-t; totc=time.process_time()-c
T._rules_for_input = orig_rules_for_input
T._flag_prospect_joints = orig_flag

t=time.perf_counter(); decision.assert_outcome_partition(); part=time.perf_counter()-t
t=time.perf_counter(); decision.assert_e_stranded(); estr=time.perf_counter()-t
...
print(f"windows={len(decision.transitions)} rules={len(decision.rules)} cells={len(decision.reachable_cells())} treaty={len(treaty.rows)}")
