...
ROOT = Path.cwd()
WATCH = {str((ROOT/f).resolve()): f for f in
         ["rebuild/pipeline/settle.py","rebuild/pipeline/table.py",
          "rebuild/pipeline/specificity.py","rebuild/pipeline/model.py",
          "rebuild/pipeline/spec_load.py","rebuild/pipeline/trace_memo.py"]}
...
KEEP=6
names = sorted(spec.runes); keep=set()
...
def fn_map(rel):
    src=(ROOT/rel).read_text(); tree=ast.parse(src)
    out=[]
...
def executable_lines(rel):
    ...
    while stack:
        ...
        for const in c.co_consts:
            if hasattr(const,"co_lines"): stack.append(const)
    return lines

print()
for rel in ["rebuild/pipeline/settle.py","rebuild/pipeline/table.py","rebuild/pipeline/specificity.py","rebuild/pipeline/model.py","rebuild/pipeline/trace_memo.py","rebuild/pipeline/spec_load.py"]:
...
for rel in ["rebuild/pipeline/settle.py","rebuild/pipeline/table.py"]:
    ex = executable_lines(rel); fns,_=fn_map(rel)
    ...
    print(f"   TOTAL never-entered: {sum(n for _,n in dead)} lines of {sum(1 for _ in (ROOT/rel).read_text().splitlines())}")
    print()
