...
subsets = {
 ...
 "s12": ['qsAh','qsDay','qsFee','qsIt','qsMay','qsUtter','qsPea','qsSee','qsRoe','qsTea','qsNo','qsLow'],
 "s13l": ['qsAh','qsDay','qsFee','qsIt','qsMay','qsUtter','qsPea','qsSee','qsRoe','qsTea','qsNo','qsLow','qsSee_qsUtter'],
}
name = sys.argv[1]
keep = subsets[name]
...
print(f"{name}: {t1-t0:.3f}s rules={len(dt_.rules)} windows={len(dt_.transitions)} cells={len(dt_.reachable_cells())} treaty={len(tt_.rows)} sha={h.hexdigest()[:16]}", file=sys.stderr)
