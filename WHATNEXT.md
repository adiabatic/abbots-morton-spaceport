...
**Current lean (2026-06-12): accept the drift and postpone the engine work**, on the expectation that a more capable model will make the engine change cheaper and safer to attempt later. The drift is fully documented and recoverable: the 27 + 2 windows are recorded as engine-limited residue in the two reviewed ledger entries (`halves-entry-extension-restored`, `same-seam-extension-non-summing` in `rebuild/m1-divergences.yaml`), the proof and structural analysis live in those two ledger entries, and the verdict pile is `verdicts-08.19.24PM.json`. The lever-hunt evidence is the workflow at `rebuild/evidence/lever-hunt-wf.js` and its result `rebuild/evidence/wf2-result.json`; the build harness that produced it is `rebuild/tools/scratch_build.py` (parameterized `load_spec(runes_dir, ...)` build into a scratch out-dir).
