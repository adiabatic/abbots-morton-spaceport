...
**Current state: the qsLow batch is in flight — the sitting’s verdicts are adjudicated into the runes, and the review queue is down to 484 blanks.** The ·Day·Utter·Low fork stays resolved in The Manual’s favor (§5.7 late formation, `settle.formation_blocked`), and qsUtter’s §5.9 follower one-liner (`policy.prefer[2]`) is now scoped `when: {right: {family: qsLow}}` per the sitting’s recorded verdict-note principle: alt-·Utter engages only before ·Low (The Manual’s pin) or where it strictly gains a connection, so the ~370 rejected one-join swaps render mono again and the 16 rejected ·X·Utter·Tea join losses are back — the record’s `why:` is stubbed `TODO`, text pending, the user’s to write. ·Day now yields before ·Tea-then-·Low so ·Tea joins forward into ·Low, and full-·Tea takes the ss03 x-height entry after ·Low and ·Day·Utter ungated, so the full bar shows even word-finally. The font-vs-settle conform sweep is exact at 9 letters (271,452 sequences, 0 divergences, full rule and transition coverage); the oracle stands at 8,700 unmatched verdict-gated taste rows; the boundary and Manual-pin (29/29) gates pass. The artifact cycle has re-run over these edits with every gate green; the carried verdict master is now `rebuild/evidence/verdicts-carried-0f5155b.json` (3,193 of 3,661 verdicts carried — most of the drop is ~390 rejects whose windows now match the old font, left the surface, and retired satisfied). What remains is finishing the sitting through the app’s live docket view (`#view=docket`, recomputed from the verdict store as verdicts land — no re-bakes): import `verdicts-carried-forward.json` (the master re-keyed to the currently served surface), then work the 484 blanks, mostly the alt→mono ·Utter and full-·Tea render changes this adjudication produced. `make verdict-ready` — and the same status as a banner in the app — now answers whether the surface, gates, and verdict store are ready; the next artifact cycle is what turns its freshness and gates checks green.
...
- **Word-final full-·Tea after mono-·Utter (ss03)** — 24 sitting notes ask for the full bar (“·Utter.!alt ~x~ ·Tea.!half”), but 33 carried approvals accept the half bar in the same shape, and ·Tea’s unlock grammar cannot see whether ·Utter was itself entered. Choosing full means ungating qsUtter in full-·Tea’s ss03 x-height unlock (as was just done for ·Low and ·Day·Utter) and flipping those 33.
