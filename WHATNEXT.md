...
The rebuild is migrating the cursive-join engine rune by rune, validating each batch against the old shipped font via the oracle. The migration is now between batches — the quiet window the boundary-keyed work below was parked for — and everything parked for this window has landed. (Landed since the last artifact cycle, so the checked-in audit census is stale until the next `run_m1` + `review.build` re-pin it: the qsDay `-en-ext-1` adjudication as `day-baseline-entry-extension-dropped`; the x-height-halves deletion — all ten transcribed records gone, with the ·No and ·May forks adjudicated from live renders as the intended `no-xheight-entry-extension-dropped` and `may-ligature-seam-loosened` classes; and the ss04 redefinition — the enumerated unlocks collapsed to one unconditional baseline-pass-through grant, a real shaping change whose newly-unlocked ss04 windows will surface as fresh review units. The next surface will mint blank units for the two deletion classes’ windows unless their ledger entries get `no_verdict: true` first.)
