...
- The exit side works the same way: `_iter_related_extension_targets` copies the trailing component’s `extend_exit_before` / `contract_exit_before` onto a `qsX_qsY` ligature, and `calt_liga` maps `(qsX, qsY.<exit-modifier>)` to `qsX_qsY.<exit-modifier>`. Don’t repeat the trailing component’s exit rules on a ligature. The exception is a ligature that declares its own `noentry_after`, which turns off the copying, so the ligature’s exit rules must be written in its YAML. `qsDay_qsEat` and `qsThey_qsUtter` are the ligatures that do this. The copying stays off for the ligature’s stances too, and the `lig → lig.noentry` rule skips a stance that declares no `noentry_after` of its own, so `qsDay_qsEat.half` keeps its baseline entry after ·It (`·It ~b~ ·Day+Eat.half`). `calt_post_reflip_bk_*` re-fires a follower’s backward substitution after the isolated form a `restore_isolated_form_overrides` row names, for a predecessor that reaches it only after the follower’s backward lookups ran; `calt_post_liga_reflip_bk_*` does the same for a ligature led by that follower, before the `noentry_after` lookup (`·He.half ~x~ ·No ~x~ ·It ~b~ ·Day+Eat.half`).
- Don’t list ligature names (`qsJay_qsUtter`) by hand in `select.after` / `select.before`. The `expand_selectors_for_ligatures` IR pass adds them from the trailing or lead component, and its docstring covers the edge cases. The pass leaves `not_after` / `not_before` alone, so a ligature a negative selector must cover is named by hand, as ·Gay’s `entry_baseline` stance names the ·Day+Eat and ·See+Eat ligatures beside `flat_on_right_at_baseline`.
...
An authored entry-preserving `ex-noentry` stance with its own `after:` becomes a backward pair override scoped to those predecessors, as `qsPea.en-y0.ex-noentry` does after ·Et and ·Awe; the stances derived from it (`.en-ext-N`) stay out of backward selection. Since the override takes the letter’s exit away, its `not_before:` lists by family every follower the letter joins onward after those predecessors, so the onward join wins. The same `not_before:` keeps the emitter from guarding the followers’ own entry substitutions against an override that cannot fire before them. `not_before:` names families, and the pair lookup can’t see the follower’s final form, so where the follower takes a form with no baseline entry because of the letter after it, the override leaves ·Pea on its bare stance. Rows in `predecessor_demote_overrides`, which run after `liga` and match the follower’s final stance, give ·Pea its baseline-entry stance before ·At’s `before-may` stance and before the ·See+Eat and ·See+Utter ligatures, so ·Et or ·Awe joins it (`·Et ~b~ ·Pea | ·At ~b~ ·May`). Before ·Utter’s alternate and ·See’s exit-only stances, ·Pea joins neither side (`·Et | ·Pea | ·Utter.alt ~b~ ·Ah`). `test_et_and_awe_join_pea_at_baseline_exactly_when_pea_breaks_after` in `test/test_calt_regressions.py` is the authority for the family list and for the followers before which ·Pea joins neither side.
