...
[^glyph-identity]: A glyph's identity in OpenType is its **glyph index** (or glyph ID): a plain integer from 0 to one less than the font's glyph count (declared in the `maxp` table; index 0 is reserved for `.notdef`). The index is just the glyph's slot in the font's glyph order — the order of records in `glyf`/`loca` for TrueType outlines, or of charstrings in the `CFF`/`CFF2` table — and is _not_ computed from the outline or bitmap data, so two glyphs with byte-identical pixels stored in different slots are still distinct identities. Code points enter in exactly one place, the `cmap` table, which maps each Unicode code point to a glyph index; from there on the run is pure glyph indices, which is all that GSUB substitution and GPOS positioning ever see. A pair-kern lookup (PairPos, GPOS lookup type 2) keys on those indices — either directly, through a Coverage table that lists the glyph IDs (format 1), or by glyph class, through ClassDef tables that bucket glyph IDs into classes (format 2) — never on the pixels. The readable names this document uses (`qsNo.alt`, `qsTea.half`) live in the `post` table (or the CFF charset) for tooling and debugging; they label the indices but play no part in matching.
