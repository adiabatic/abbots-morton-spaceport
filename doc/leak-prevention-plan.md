...
The single-form "selected across a non-join" class is gone from `test/isolation-leak-snapshot.txt` (Phases 1–2); `make test` and `make test-leaks` are green against the re-blessed snapshot; and the FEA for any non-behavioral step diffs to zero bytes. The dangle class is meant to go too, leaving every residual entry as one of: an author-declared cosmetic tuck (carries a `before-X`/`after-X` modifier), a `doc/leak-emergent-verdicts.txt`-accepted emergent leak, or a cross-lookup-compose leak no contract reaches (family F4) — the enumerated, reviewed set the investigation promised, not a discovery problem.
