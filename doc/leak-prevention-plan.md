...
Phases 1–2 landed and the per-rule contract enforces (commit "Do phase 2 (and skip phase 3)"). **Phase 3 retirement was deliberately skipped:** `_PENDING_BK_ENTRY_GUARDS` / `_PENDING_LIGA_ENTRY_GUARDS` and the demote-override tables are all still live, so every retirement opportunity above remains open follow-up.
