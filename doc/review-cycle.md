...
- The three human-reviewed ledgers hash through a projection (`divergence_ledger_digest`, `contact_allow_digest`, `standing_approvals_digest`), so only a structural edit to an entry moves a gate. A divergence entry's `why` reaches the surface manifest's class list and rides `explain_prose` the same way; a contact-allow `why` reaches nothing downstream; a standing rule's `note` is quoted into every fill it lands and rides the verdict plumbing's key by raw bytes (`plumbing_skip_fingerprint`), and no key the standing fill's memo reads.
- Code files reaching `fingerprint.path_lines` hash through a projection too (`code_file_digest`): a Python docstring and a whole-line Rust comment are invisible to it, so rewording either leaves the tables' stamp, the run_m1 skip key, the trace-memo and replay stamps, the oracle row cache and settle memo, both surface stores and the standing-fill memo where they stand. `gate:rebuild-contracts` and `gate:make-test` still run on that edit, because their closures hash raw — test fixtures read source text, and a `# pyright: ignore` comment changes the answer being gated.
...
- A live pass that follows a rehearsal whose surface reproduces the current inputs byte for byte moves that directory into `rebuild/out/review` (the `surface-promote` step) instead of rebuilding it, unit store and signature store with it. `promotable_surface` in `rebuild/tools/artifact_cycle.py` holds the preconditions; the rehearsal is found through the last `cycle_summary.json`'s `plan.review_out` or at the conventional `var/rehearsal-review`, which is why a rehearsal goes there. The move replaces every shard and the stamp under the app, so such a pass takes the port and runs the full carry rather than the plumbing skip or the store-only merge. The outgoing tree steps aside as `rebuild/out/review.superseded` for the length of the move; a tree of that name left by an interrupted pass is put back as the live surface, or deleted when a live surface stands beside it, at the start of the next pass, before the first-run question is asked. A `--dry-run` pass still puts a lone tree back, so the plan it prints is the one a real pass follows, but leaves a tree beside a live surface for the next real pass to delete, and says so.
...
- A unit's id is its content key's (`unit_cache.unit_id_for`; `rebuild/REVIEW-PLAN.md` §2.1 states the shape), so the carry lands a verdict on the unit of the same id and opens no other surface: a verdict whose unit is on the new surface lands there, and one whose unit is not is stranded. The carry's four figures — human units, key hits, unhit, stranded — ride the run line in `rebuild/out/cycle-timings.ndjson`.
- A pass whose surface did not move skips the carry and merges the master straight into the store (the store-only route), since every unit id would resolve to itself. The merge refuses any input stamped for another surface, so the route needs the master stamped for the served one: the auto-resolved carry source's line says whether it is, and `master_stamped_for_surface` in `rebuild/tools/artifact_cycle.py` answers for a `--verdicts` master. A pass stopped after the surface build wrote a new surface and before the carry leaves the store stamped for the surface before it; the next pass skips the build as unchanged, names the master as stamped for an older surface, and carries it by unit id.
...
- The standing fill memoizes its per-unit decisions in `rebuild/out/standing-fill-memo.ndjson.gz` — beside the surface directory rather than inside it, so a surface rebuild never clears it — and a pass whose surface moved evaluates only the units whose key is new. A unit's key is its build-time `content_key` stamp, its `ink_deltas`, and the after font's compiled-glyph digest for every family its after cells name, so a rune edit re-evaluates exactly the windows that can feel it; the file's own stamp is the deciding code, the before font outside its `head` and `name` tables, the after font's family-blind remainder, and `uv.lock`'s dependency pins, and any of those moving drops it whole — so a version bump's rebuilt fonts and refreshed lock leave it serving, while a fontTools or uharfbuzz bump drops it. The rules file reaches the memo per entry instead: the header carries the rule roster (each rule's match digest and verdict, the non-composable rules every composed reading reads, and whether a composed reading is on), each entry carries the composable rules with a candidate position in its window, and a stored decision holds rule ids and no prose, so a rule commit re-evaluates only the windows the moved rules can reach and a reword re-evaluates nothing while every fill quotes the new wording. The fills and the report come out byte-identical served or computed, `--fresh` rewrites the memo from scratch, and the `memo:` line in the plumbing step's log says what a pass served and computed. `Memo`, `memo_environment`, `rules_roster`, `unit_key` and `Decider._serve` in `rebuild/tools/standing_verdicts.py` are the authority, and `rebuild/test_standing_verdicts.py` proves the identity over the frozen mini bundle.
...
- `doc/running-long-steps.md` is the recipe for running a pass detached, where the per-step logs land, judging whether a run is hung, and stopping a pass whole.
