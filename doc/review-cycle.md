...
- A unit's id is its content key's (`unit_cache.unit_id_for`; `rebuild/REVIEW-PLAN.md` §2.1 states the shape), so the carry lands a verdict on the unit of the same id without opening the snapshot it was recorded on. A verdict recorded on a snapshot that named its units positionally resolves through that snapshot, and the chain's carry step rewrites the journal's lines under that snapshot's stamp onto content ids in the same pass (`journal.migrate_unit_ids`, through `carry_verdicts.id_migration`), so the history a restore replays names the units the surface names; a content-addressed snapshot maps nothing, so the rewrite runs once.
