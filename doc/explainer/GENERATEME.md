...
- Read `tools/build_font.py`, `tools/quikscript_fea.py`, and `tools/quikscript_ir.py` for the actual implementation. The live entry points are `emit_quikscript_senior_features()` and the transform-expansion helpers in `tools/quikscript_ir.py` such as `generate_noentry_variants()`.
