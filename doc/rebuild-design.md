...
**The sound factorization is subtables, never per-family lookups.** The transducer encoding — one settlement lookup, optionally partitioned into per-family **subtables** for size (subtables share the lookup’s single left-to-right pass, so backtrack still sees settled neighbors) — is architecturally sound: backtrack-sees-settled _within a lookup_ is shaper-spec behavior the current font already exercises. Per-family **lookups** are unsound and are not an option: each lookup finishes the whole buffer before the next starts, so any rune whose family’s lookup runs first would see a raw left neighbor, and the cyclic left-adjacency between families means no lookup order fixes this — the only sound multi-lookup factorization is the two-strata fallback. The kill criterion is emitted size, measured by `_report_gsub_budget` and gated per build (`rebuild/out/m1/budget.json`); the cross-shaper smoke tests cover within-lookup sequential-substitution semantics, ZWNJ-interleaved cases included, over HarfBuzz and CoreText — the designed DirectWrite leg is unrun. The subtable partitioning is load-bearing, not a size nicety: feaLib compiles each settlement rule to its own format-3 chained-context subtable, and at current rule counts those per-rule offsets overrun the lookup’s 64K offset space before the font can even be written, so `pack_gsub` regroups the compiled lookup in memory — order-preserving, self-verifying by re-deriving every rule from the packed form — into shared-ClassDef format-2 subtables, keeping format-3 in sequence position for the rules whose class sets a single ClassDef cannot express; the budget gate measures the packed reality. The designed fallback is the two-strata staged-wave emission (stratum 1 over raw facts, stratum 2 keyed on stratum-1 output classes — pass count provably 2, never an empirical fixpoint); note honestly that stratum-internal chains still rely on within-lookup sequencing, so the bottom fallback is per-dependency-rank waves, bounded by the longest settled chain. Either way the guard/demote/repair categories stay deleted; only emitted size is at stake. The design-time forecast for the primary path, kept as the forecast record because it is what the kill criterion aimed at: ≈950 curs statements unchanged, settlement in the low thousands of rules, total FEA roughly 3–6k lines against today’s 30.8k. What measures the outcome now is `rebuild/out/m1/budget.json` — GSUB bytes, lookup and subtable counts, and the offset headroom the gate floors.
...
3. **The per-transition conformance gate** — enumerate every decision-table transition, derive a shortest example sequence per transition (including sequences longer than 5 runes that today’s depth-4 sweep provably misses, and ZWNJ-interleaved sequences for every transition class), shape through HarfBuzz, diff against the settlement function. Coverage is per-transition, not per-depth — this converts “no fixed sweep depth is provably complete” from a permanent fear into a closed obligation. Where the deep slots enumerate at class grain, a transition is an equivalence class of label windows and coverage is per equivalence class — the classes being build-proven fibres whose per-member indiscernibility is asserted (partition, echo, and fibre verification), not assumed, with the summary noting how many class rows stand for how many label windows. Run per stylistic-set configuration for each set alone, every configuration the Manual’s pins use, and at least one declared multi-set combination.
...
11. **Deleted knowledge can be silently lost in corners no gate reaches** (the archive documents real depth-5-only regressions). The per-transition conformance gate closes most of this class — per equivalence class of transitions where the deep slots are class-grain, which detects a dead class but no longer a dead label combination inside a live class; the build-proven fibre indiscernibility is what keeps that narrowing honest. The depth-4 belt stays for the rest, and some taste will still be rediscovered by eye, later. That is the accepted price of deleting rather than porting.
