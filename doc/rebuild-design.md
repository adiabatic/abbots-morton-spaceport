...
3. **Transcription and reachability, proven per build** — read-back re-parses the compiled font’s own bytes and structurally compares every GSUB stage, feature registration, LookupList order, and curs anchor against the emitters’ plan (`rebuild/pipeline/readback.py`), while the font-free witness gate (`rebuild/test_rule_witnesses.py`) derives a shortest realizing string for every settlement rule from the decision table’s own windows and verifies it fires against the settlement function. Together these own the coverage obligation the sweep used to carry: a rule missing from the font fails read-back, a rule no string can realize is dead code and fails the witness gate, and neither proof depends on a sweep horizon — which converts “no fixed sweep depth is provably complete” from a permanent fear into a closed obligation. Where the deep slots enumerate at class grain, a witnessed row stands for a build-proven fiber of label windows whose per-member indiscernibility is asserted (partition, echo, and fiber verification), not assumed.
