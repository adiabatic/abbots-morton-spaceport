...
3. **Transcription and reachability, proven per build** — read-back re-parses the compiled font’s own bytes and structurally compares every GSUB stage, feature registration, LookupList order, and curs anchor against the emitters’ plan (`rebuild/pipeline/readback.py`), while the font-free witness gate (`rebuild/test_rule_witnesses.py`) derives a shortest realizing string for every settlement rule from the decision table’s own windows and verifies it fires against the settlement function. That gate counts its coverage over the emitted lookup rather than over the tables behind it: the fold read-back proved the font holds, recorded beside the read-back summary with each row’s per-configuration sources, is held to the stamped enumerations, so every row that ships is the fold of a witnessed table rule and what is counted is what ships. A missing or stale enumeration fails the gate outright, naming the build to run, rather than being rebuilt in process. Together these own the coverage obligation the sweep used to carry: a rule missing from the font fails read-back, a rule no string can realize is dead code and fails the witness gate, and neither proof depends on a sweep horizon — which converts “no fixed sweep depth is provably complete” from a permanent fear into a closed obligation. Where the deep slots enumerate at class grain, a witnessed row stands for a build-proven fiber of label windows whose per-member indiscernibility is asserted (partition, echo, and fiber verification), not assumed.
4. **The empirical HarfBuzz sweep, in two forms** — the per-edit belt: exhaustive font-vs-settle at horizon 4 over every acceptance configuration, no witness top-ups and no coverage claim, carrying the ZWNJ-structure and split-buffer checks on every boundary-bearing text (the standalone horizon-5 boundary gate retired into it; the deep sweep carries them past 4), whose charter is what only shaping the real binary can test — HarfBuzz’s application semantics (lookup interaction across features, backtrack-sees-settled across subtable breaks, default-ignorable skipping, class matching, Extension indirection) and the sufficiency of the 6-slot window abstraction itself, which witness-constructed strings structurally cannot probe because witnesses are built from that abstraction. And the periodic deep sweep (`make conform-deep`): the same sweep at horizon 5 or deeper, overnight or on demand, green-keyed on the fail-closed behavior-class enumeration over the emitted lookup (`emit_gsub.behavior_classes`) plus the font-compilation code and the uharfbuzz version — a rune edit that introduces no novel rule shape never stales it, and a rule that classifies into no known class fails the build, so a novel shape can never silently fail to arm it. The recorded trade of this re-charter: stronger on transcription (proven for the first time) and on depth, slightly delayed on shaper-interaction sampling — an interaction only visible at length 5 waits for the deep sweep instead of the next cycle.
