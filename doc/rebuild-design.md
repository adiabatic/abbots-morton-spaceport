...
1. **The settlement-transducer emitter carries the load.** It is built and shipping — one `useExtension` settlement lookup whose per-rule subtables ride type-7 Extension offsets, with the budget gate’s yellow flag as its steady state — so the residual risk is emitted size rather than feasibility, and the two-strata fallback of §7 (whose own residual within-lookup reliance is noted there) stays designed and unbuilt.
