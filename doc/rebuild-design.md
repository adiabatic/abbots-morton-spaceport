...
when:
  ...
  right:                       # the RAW right neighbor — static facts only, never resolved state
    ...
    then: {family: [qsExam], class: can_enter_at_xheight}    # one further static hop; the window's outer edge
...
The direction asymmetry is the depth bound, by construction: `left` may reference resolved state because the left neighbor is already settled (and transitively summarizes all deeper left context — the system’s only memory); `right` may reference only static capability plus one further static hop. There is no `left2`, no `right.stance`, no `right.offers`: forward viability is the settlement algorithm’s built-in lookahead, never an authorable predicate. The grammar literally has no words to reach past the depth-2 window, which is what makes depth-2 verification complete for the rules that can exist. If a genuine case ever demands resolved-right or deeper reach, that is a schema change with a design review — deliberately expensive.
...
Formation semantics, stated precisely after verification: in the model, **formation is a settlement decision** like any other — but every one of today’s 13 ligatures forms unconditionally (today’s `calt_liga` is plain non-contextual GSUB type-4; what is context-dependent about `qsThey_qsUtter` is its entry/extension _variant_, which the new model expresses natively as ordinary settlement at the ligature’s position over its settled left neighbor). So the emitter’s type-4-first encoding over bare runes is an optimization valid exactly when formation provably doesn’t depend on resolved state — true for all current data. The named contingency for a future genuinely resolved-context ligature is **late formation** (a second type-4 application staged after the affected predecessors settle, keyed on settled classes — the mid-pipeline variant-keyed shape today’s font already proves), never an un-form-and-repair pass, which the architecture forbids.
...
8. **Resolved-context ligature formation has no live example today** (verification overturned the one suspected case — today’s formation is unconditional), and the named contingency (late formation on settled classes) is designed but unbuilt. If a future ligature genuinely needs it, that is where the design bends first.
