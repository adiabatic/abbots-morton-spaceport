...
- When a `noentry_after` ligature leaves a predecessor's bare bitmap with unsupported exit-side ink, add an explicit `.exit-noentry` form with no entry or exit anchors and the trimmed bitmap. The post-liga cleanup will choose that form over the bare fallback for the `noentry_after` context; `qsMay` before `qsThey_qsUtter.noentry` is the running example. A second pass, `calt_post_liga_left_cleanup_pred`, then walks one position further back: any pre-predecessor whose `select.before` clause was triggered by the now-demoted family is reverted to its bare base when followed by the `.exit-noentry` replacement, so a glyph like `qsRoe.exit-baseline` doesn't keep extending its baseline stroke toward an entry that no longer exists. Don't pull back a later component inside a ligature to make a left-side context easier; if `qsAt_qsMay` precedes `qsThey_qsUtter.noentry`, the ·May component should stay part of the whole `qsAt_qsMay` ligature shape.
