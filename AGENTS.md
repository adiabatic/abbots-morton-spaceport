...
- After any glyph or code change, run `make test`. `doc/testing.md` maps every gate to the change it answers for.
  - `make test-rebuild` gates anything under `rebuild/` and `make kernel-gate` a kernel-semantics change. No rebuild test reads a live artifact; the build proves its own tables (the rule certificates among them), so run the M1 build after a rune or pipeline edit.
...
- Detach any pass that includes the heavy gates (the rebuild suite and the conform sweep, so any `make artifact-cycle` or `make review-cycle` pass) from the shell's lifetime and watch `var/build-logs/latest/`; `doc/running-long-steps.md` has the recipe and the hung-run traps.
- `tmp/` is scratch and may be wiped between change sets; `var/` (gitignored too) holds what outlives a run: the cycle's build logs and hand-kept archives under `var/keep/`. Anything that must survive a `tmp/` wipe goes under `var/`.
