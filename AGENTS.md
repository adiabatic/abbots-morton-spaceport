...
- When an entry-preserving `exit-noentry` form is used as a `before:` forward-pair override to match split shaping across a non-joining break, scope its left context to predecessors whose isolated left half already chooses the same entry form. Use an anchor selector with `except` for the positive predecessor set, and add a `trailing_demote_overrides` entry when the follower may already have taken a backward upgrade that must return to the isolated right-half form (e.g. `qsIt.entry-baseline.exit-noentry.before-day-exam` demotes `qsDay.half` back to `qsDay`).
