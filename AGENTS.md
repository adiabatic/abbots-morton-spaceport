...
- The M1 table build runs in Rust: `rebuild/kernel-rs/` is the engine of record (issues #40 and #48), so `cargo` is a build prerequisite alongside `uv` — a box without it cannot build the M1 artifacts at all (`run_m1 --engine python` is the in-process fallback arm). Python’s settle kernel still ships beside it — `gate:conform` re-settles every swept string through it and `emit_gsub` reads `formation_blocked` — so a change to kernel semantics in `rebuild/pipeline/table.py`, `settle.py`, `model.py` or `specificity.py` must land in the crate too, and vice versa. The artifact cycle’s `gate:kernel-differential` polices exactly that: it arms when either engine’s kernel sources move — never on a rune edit — and builds both sides fresh (the kernel fan-out and one Python fixpoint per configuration), byte-comparing the three artifacts and the contract digest; `make kernel-gate` is the standalone form, and `gate:conform` remains the every-cycle independent check mediated by HarfBuzz. `make kernel-check` is the crate’s own fmt/clippy/test gate, and the deep standalone harnesses (`make kernel-fixpoint` and its pinned/label-grain siblings, `make kernel-liveness`, `make kernel-differential`) are the instruments to reach for around a substantial kernel-semantics change.
