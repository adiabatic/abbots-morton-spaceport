...
- **A count in prose names the artifact that reports it, never the number.** The rebuild measures itself on every cycle, so a tally written into a note is stale by the next one — and when a fact has both a prose copy and a machine copy, it is always the prose that rots while the machine copy stays true. Drifting state has homes already: `rebuild/out/cycle_summary.json` and the per-gate summaries beside it under `rebuild/out/m1/`, `rebuild/out/review/manifest.json` for surface totals, `rebuild/review-census-pins.json` for the machine-checked census, and `make verdict-ready` for whether a sitting can start. Point at those rather than transcribing them, and prefer the qualitative shape — "the sweep is exact", "the budget gate's yellow is the steady state", "the unmatched rows are verdict-gated, not failing" — to a figure. Definitional numbers are not counts and stay: Tall/Deep/Short as 9/9/6 rows, the depth-4 chain cap, the dev-server ports, a hash that _is_ a byte-identity contract, and a frozen fact about the old shipped font. The same goes for a commit-stamped filename — write `verdicts-carried-<sha>.json`, not this cycle's hash.
- **When a fact has both a prose home and an executable one, the executable one is binding and the prose says so.** A field list a validator already enforces, a gate's expected-failure set, a keyboard map the frontend binds, a class list a ledger already carries — restating any of these in a note creates a second copy that can only drift out of agreement, and the note is the copy nothing checks. Name the authority (`build.check_manifest`, `BASELINE_REBUILD_FAILURES`, `keyboard.js`, `rebuild/m1-divergences.yaml`) and describe the shape; don't maintain a parallel specification alongside it.
...
- After any glyph or code changes, run `make test` to make sure nothing is broken. It self-skips in about a second when nothing it checks has changed since its last green run — changes confined to `rebuild/`, `glyph_data/runes/`, `doc/`, `tmp/`, `.claude/`, and Markdown are outside its input closure, and those are the rebuild suite's and artifact cycle's job to gate, so don't force a run for them. `make test FORCE=1` overrides the skip.
- The M1 table-build kernel has a Rust twin under `rebuild/kernel-rs/` (issue #40), so `cargo` is a build prerequisite alongside `uv`. After touching `rebuild/pipeline/table.py`, `settle.py`, `model.py` or `specificity.py`, or the crate itself, the artifact cycle’s `gate:kernel-differential` is what proves the two engines still fold to the same tables. `make kernel-gate` re-runs that gate against the tables a cycle already built, so after a Python-source edit it can only report the artifacts stale until the cycle rebuilds them — the standalone target is for re-checking a settled tree or iterating on the crate against unchanged Python sources. `make kernel-check` is the crate’s own fmt/clippy/test gate. The deep landing evidence re-arms itself: `gate:kernel-harness` re-runs the port’s three heavy harnesses (the three `kernel-fixpoint` worlds at the live alphabet, exhaustive `kernel-liveness`, and `kernel-differential`) on the cycle after the alphabet’s structure — a new rune family, ligature, or class membership — or either engine’s sources move; ordinary rune ink edits leave it proved. It defers to a converging pass like the other heavy gates and `make verdict-ready` stays NOT READY until it lands green, so a new letter automatically owes one deep-harness pass before its sitting can start; `make kernel-harness-gate` is the standalone form, and `make cycle-timings` is where its cost is on record.
...
- `make review-serve` serves the rebuild review surface on `http://localhost:7294/`. When the user asks whether everything is ready to verdict, don't reason it out from the git log — run `make verdict-ready` (surface freshness by input fingerprint, gate greenness, verdict-store alignment, server, blanks remaining); the app shows the same status as a banner. The fingerprints are prose-blind toward the rune files: YAML comments, reformatting, and documentation prose (`ductus`, `notes`, and `why` everywhere but refuse records, whose `why` the surface's explain panel quotes) sit outside every input fingerprint — `rebuild/pipeline/fingerprint.rune_file_digest` is the authority — so editing them never demands a cycle.
- `make review-cycle` is the hands-off loop: run the artifact cycle — whose merge step (`uv run python -m rebuild.tools.merge_verdicts`, the app's newer-`at`-wins import union, headless and never-shrinking) lands the carried verdicts in `verdicts-autosave.json` with no browser import — and serve the surface at the end. Whether the review server has to come down for a pass is the cycle's decision, not the recipe's, because only the resolved plan knows: two of the things a cycle writes belong to the running app — the surface it serves, where a restamped manifest orphans the open tab's store, and the verdict store, which `merge_verdicts` refuses to touch under a live server anyway — so a pass that rebuilds the surface or moves the store stops the server first (`--stop-server` is the recipe's permission to; a bare `make artifact-cycle` still refuses and says how), while a pass that writes neither leaves it up and the letters stay on screen for the whole run. That second shape is the gate pass, so the half hour of deferred verification is now background work rather than a lockout. Serving in the foreground is what an interactive caller wants and what makes the target hands-off, but it also means the recipe never exits: run it from anywhere that waits on a command and the cycle summary never lands as a completed run. `make review-cycle SERVE=0` is the form for those callers — same cycle, but it prints the `make review-serve` command instead of serving, so the target terminates; a gate-only pass costs them nothing, since the server it never stopped is still up, and the recipe says so instead. One knock-on: while a server is up the retention pass leaves the journal and the stash sweep that indexes off it alone, because the app appends to that journal as you verdict and a compaction rewrites it around a read. After that merge the cycle also lands the echo prefill and the standing approvals: `rebuild/standing-approvals.yaml` holds checked-in once-and-for-all pattern rules (applied by `rebuild/tools/standing_verdicts.py`; a rule's `except_left` families still queue, and any human verdict outranks a standing fill on merge), so a delta shape the user has blessed forever never reaches the docket again. Every store write (app autosaves included) is journaled in `verdicts-journal.ndjson`; `merge_verdicts --restore-as-of <time> --apply` rolls the store back to any recorded moment, and `--list` shows the history. A green cycle ends with a retention pass over the regenerable piles: only this cycle's `tmp/review-pre-*` snapshot and the stamp-aligned `verdicts-carried-*.json` survive, `verdicts-autosave-*` stashes older than the journal's last base event go (the journal replays them), and the journal is compacted to a 7-day restore floor — `ARGS='--keep-history'` skips the pass, and the tracked copy under `rebuild/evidence/` is never touched. Repeated runs converge rather than re-verifying everything every time: a pass that rebuilds M1 or the surface records the heavy gates pending instead of running them — and the census with them, since nothing that pass runs reads it — so a rune edit puts the letters back on screen quickly; the next pass has nothing left to rebuild and runs them; the one after that costs seconds, because the verdict plumbing skips too (snapshot, carry, merge, both fills and the complaint docket ride one green record keyed on the surface, the verdicts master, the store, and `standing-approvals.yaml`, so a pass that changed none of them re-derives nothing). One gate needs one more flag: a rune edit leaves the census pins stale, and gate:rebuild stays deferred under known-stale pins — a suite run there could never record green — until a `make review-cycle ARGS='--update-pins'` pass refreshes the pins and runs the suite against them. Deferral is not a waiver — a deferred gate reads as unverified, so `make verdict-ready` stays NOT READY until a pass clears it. `make artifact-cycle` is the one-pass form for commit time, and `make review-cycle ARGS='--no-defer-gates'` gets the same in the loop. Every cycle also appends per-step wall times — host-tagged, with each child's inner `[t]` phase lines — to `rebuild/out/cycle-timings.ndjson`; `make cycle-timings` summarizes recent runs, and `ARGS='--by-step'` compares step medians across machines.
