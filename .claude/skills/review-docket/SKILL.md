...
3. **First docket pass.** `uv run python rebuild/tools/review_docket.py <verdicts-file>` — writes `rebuild/out/review/docket.html` and `tmp/docket-data.json`. The page is an app-first work order: numbered class rulings, a tranche of the top `TRANCHE_SIZE` cluster cards (the only render pairs on the page), later clusters as compact rows, singletons as link tables, and the disagreeing echo groups — every decision is one `index.html#units=…` button that stacks its representatives as an app worklist, because all judging happens in the app's keyboard flow. Re-bake after a working session to promote the next tranche.
