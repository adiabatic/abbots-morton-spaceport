...
- ss10: decide `SS10_UNCOVERED_BY_OLD_FONT` membership in `rebuild/pipeline/oracle.py` — the qsAwe shape (no stances in the old record, both anchors on the base cmap glyph) or qsAt's direct pair evidence — and extend the set's comment there the way the later members did, with the pair evidence in data-expect notation (`·May ~x~ ·Ox` and `·Ox ~b~ ·Vie` stay joined); the `ss10-isolation-completed` `why:` in `rebuild/m1-divergences.yaml` defers to that comment instead of listing members. `classify_divergence` grows an arm only for a genuinely new phenomenon; most letters add no ledger class, and a new ligature needs no ss10 arm at all, since `oracle.ss10_formable_pairs` reads the formable pairs off `rebuild/script.yaml`.
