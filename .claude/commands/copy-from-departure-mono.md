...
1. Extract the glyph bitmap:
   ...
   ```zsh
   uv run python tools/extract_glyph.py reference/DepartureMono-Regular.otf $ARGUMENTS
   ```
...
5. Verify the glyph matches exactly:
   ...
   ```zsh
   uv run python tools/extract_glyph.py --compare $ARGUMENTS reference/DepartureMono-Regular.otf site/AbbotsMortonSpaceportMono-Regular.otf
   ```
