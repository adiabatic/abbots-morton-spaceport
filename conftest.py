...
# What one worker holds in a run this hook cannot narrow by inspection — a bare `uv run pytest rebuild/`, a single rebuild test file, a mixed `pytest rebuild/ test/`, a `pytest .` that reaches the rebuild suite without rebuild/conftest.py ever being loaded in time to answer — since a worksteal pool cannot keep any collected item off any worker, so every slot in such a pool has to be priced at the heaviest worker this repo has. That worker is a rebuild validators worker, and this is the same measured peak rebuild/conftest.py prices as VALIDATORS_WORKER_BYTES, which stays the authority on it — including the argument for the headroom above the reading and for why a downward re-seed reads as an overrun in `make job-costs` for a while. It is restated here rather than imported for a reason worth knowing: pytest loads every conftest under the plain name `conftest`, so `from rebuild.conftest import ...` finds nothing cached and executes that whole file a second time under a second name, beside the copy pytest has already loaded and armed the lane audit hook in. So this is one fact written twice on purpose — the shape `rebuild/test_kernel_io.py`'s spec-echo parity test already holds the crate's spec contract in — and it is checked rather than trusted: test_the_root_fallback_prices_a_worker_the_way_the_validators_lane_does in rebuild/test_memory_budget.py reads this literal back out of the source and holds the two equal, so re-seeding either one off a fresher peak-RSS line fails loudly until the other moves in the same commit.
HEAVIEST_WORKER_BYTES = 1_500_000_000


# What `-n auto` resolves to repo-wide, in the two shapes this hook has to answer for, both of them derived now. When every collected target is under test/ or site/ the answer is the box's usable cores — the ones this process may actually run on, affinity mask and cgroup CPU quota included, which os.cpu_count() reads straight past: those workers cost FONT_SUITE_WORKER_BYTES apiece, so the RAM argument that once put a 2 here never applied to them, and the whole of that suite's 1,090 s at width 2 was the width. Everything else is a run whose composition this hook cannot see, so it is priced at HEAVIEST_WORKER_BYTES and capped at those same cores, rather than at a legacy floor sized for validators workers that peaked at 14–17 GB and no longer exist in that shape. memory_budget is imported inside the hook rather than at module scope because this file is loaded by every pytest run in the repo while this hook is called only by the ones that actually spell `-n auto`, so a run that states its own width never pays for the import at all — the same function-scope shape peak_rss and fingerprint are reached in here, though only fingerprint's caller is genuinely kept off the font suite's path, since both branches below want usable_cores and a font run at `-n auto` therefore imports the rebuild package like any other. Answering this firstresult hook shadows xdist's own, which is where PYTEST_XDIST_AUTO_NUM_WORKERS is normally read, so the variable is read here too and keeps overriding whichever default applies — including rebuild/conftest.py's, which returns None when it is set precisely so this line gets it — a run at a time.
def pytest_xdist_auto_num_workers(config: pytest.Config) -> int:
    override = os.environ.get("PYTEST_XDIST_AUTO_NUM_WORKERS")
    if override:
        return max(1, int(override))
    from rebuild.tools.memory_budget import how_many_fit, usable_cores

    cores = usable_cores()
    return cores if _is_font_suite_only(config) else how_many_fit(HEAVIEST_WORKER_BYTES, cap=cores)
