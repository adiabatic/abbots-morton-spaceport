...
# AMS_RUN_PYRIGHT is what type-checks rebuild/: a rebuild-only edit provably cannot move gate:make-test's fingerprint (make_test_exempt exempts rebuild/ wholesale), so `make test` can never be its gate. The suite runs under -n auto, so the same pytest_configure hook fires here, but it recognizes a rebuild-only run and skips the font build (unless the site fonts are absent, as after `make clean`) — this suite shapes against the site fonts exactly as its input-closure fingerprint already hashed them — leaving pyright to run alone and still fast-fail before the workers spawn — when it runs at all: it skips on its own green record (rebuild/tools/pyright_gate.py), so a rune edit that narrows the suite to the tests the rune reaches spawns no type check ahead of them, while FORCE=1 forces it with the suite; a pyright failure exits pytest nonzero with no FAILED/ERROR lines, which classify_rebuild_output already buckets as a hard failure. The flag rides into whichever lane spawns first and is stripped from every lane after it, since pyright's answer over one working tree cannot change between them.
test-rebuild:
