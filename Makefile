...
# Self-skipping: the wrapper exits 0 in ≈a second when nothing the suite reads has changed since its last green run (make_test_exempt in rebuild/tools/artifact_cycle.py is the authority on what is outside its closure — the exempt trees and files, Markdown, and this Makefile beyond what `make -n all` and `make -n test` print, so a comment here or a target the suite never runs cannot re-arm it; the green record at rebuild/out/make-test-green.json is shared with the artifact cycle's gate:make-test). FORCE=1 runs the suite regardless. The pyright gate runs inside pytest_configure (via AMS_RUN_PYRIGHT) so it overlaps the font build instead of preceding it serially; it still fast-fails before the workers spawn, and it skips itself on its own green record (rebuild/out/pyright-green.json, keyed by rebuild/tools/pyright_gate.py on every file pyright can read) when no Python, stub, pyproject.toml or uv.lock edit has landed since its last green — FORCE=1 runs it too. Which paths get checked is `[tool.pyright] include` in pyproject.toml, not the argv here — every invocation is a bare `uv run pyright` so that list is the single authority. The `typecheck` target stays for standalone use; pre-commit runs black only.
test:
...
# AMS_RUN_PYRIGHT is what type-checks rebuild/: a rebuild-only edit provably cannot move gate:make-test's fingerprint (make_test_exempt exempts rebuild/ wholesale), so `make test` can never be its gate. The suite runs under -n auto, so the same pytest_configure hook fires here, but it recognizes a rebuild-only run and skips the font build (unless the site fonts are absent, as after `make clean`) — this suite shapes against the site fonts exactly as its input-closure fingerprint already hashed them — and on that fonts-present run, with nothing left to overlap, pyright starts in that hook and is joined at session end, so it runs beside the suite instead of ahead of it (the fonts-absent run builds them and waits on pyright beside the build, as `make test` does); the trade is fast-fail, since a type-broken tree burns one narrowed suite run before the wrapper goes red — when it runs at all: it skips on its own green record (rebuild/tools/pyright_gate.py), so a rune edit that narrows the suite to the tests the rune reaches spawns no type check ahead of them, while FORCE=1 forces it with the suite; a pyright failure exits pytest nonzero with no FAILED/ERROR lines, which classify_rebuild_output already buckets as a hard failure. The flag rides into whichever lane spawns first and is stripped from every lane after it, since pyright's answer over one working tree cannot change between them.
test-rebuild:
