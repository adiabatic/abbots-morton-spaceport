...
- **`noentry_after`.** A successor opts out of a left-side join after specific predecessors. If the right glyph is a ligature that becomes `.noentry`, `calt_post_liga_left_cleanup` can also route predecessors to explicit `.exit-noentry` forms when the predecessor's own bare bitmap would leave an unsupported exit-side stub. Authoring an entry-preserving `.exit-noentry` companion (e.g. `qsMay.entry-baseline.exit-noentry`) keeps the predecessor's baseline join intact — the `_propagate_noentry_after_to_not_before` skip recognizes the companion and lets calt_cycle pick `qsMay.entry-baseline` pre-liga, so `·Roe·May` joins at the baseline whether or not `·They+Utter` follows. When no entry-preserving companion exists, a second pass (`calt_post_liga_left_cleanup_pred`) reverts any pre-predecessor whose `before:` clause was triggered by the now-demoted family. Do not pull back a later component inside a ligature to satisfy a left-side context; the historical example was `qsAt_qsMay` staying whole before `qsThey_qsUtter.noentry` — that ligature has been retired in favor of a contextual ·At form, but the principle still holds for similar setups.
- **`entry: null` on a ligature.** Ligature opts out of all left-side joins (the running example was `qsAt_qsMay`, since retired). Triggers `entry_explicitly_none`, blocks lead-inheritance, and tells `calt_post_liga_left_cleanup` to revert any predecessor.
