...
- **`noentry_after`.** A successor opts out of a left-side join after specific predecessors. If the right glyph is a ligature that becomes `.noentry`, `calt_post_liga_left_cleanup` can also route predecessors to explicit `.exit-noentry` forms when the predecessor's own bare bitmap would leave an unsupported exit-side stub. A second pass (`calt_post_liga_left_cleanup_pred`) then reverts any pre-predecessor whose `before:` clause was triggered by the now-demoted family — without it, a glyph like `qsRoe.exit-baseline` would keep extending toward the entryless follower it can no longer reach. Do not pull back a later component inside a ligature to satisfy a left-side context; `qsAt_qsMay` stays whole before `qsThey_qsUtter.noentry`.
