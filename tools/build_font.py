...
def _write_if_changed(path: Path, data: bytes) -> bool:
    # The build is byte-deterministic, so rewriting an identical output would only bump its mtime — and mtime-derived cache keys downstream (the rebuild review surface's fixture cache) read that as a fresh build and throw away work.
    try:
