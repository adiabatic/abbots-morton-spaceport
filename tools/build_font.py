...
def _write_if_changed(path: Path, data: bytes) -> bool:
    # The build is byte-deterministic, so rewriting an identical output would only bump its mtime — and mtime-derived cache keys downstream (the rebuild review surface's fixture cache) read that as a fresh build and throw away work. The prior copy is hashed in chunks rather than read whole, so the comparison never holds a second copy of the output.
    try:
...
class _GlyphInventory:
    """What a variant's font carries, before any outline is drawn: its glyph definitions (Departure Mono's merged in for mono), its glyph order, its cmap, and the code point behind each glyph name. Shared between `build_font` and `compile_senior_otl` so the Senior OTL is compiled against exactly the glyph order the Senior fonts will carry."""
...
def _assemble_fea(
    ...
    """The whole feature file a variant compiles, or None for mono, which inherits Departure Mono's tables instead. `senior_fea` is the precomputed senior join code (see `build_font`); left None, a Senior assembly emits its own."""
    is_proportional = variant != "mono"
...
class CompiledSeniorOTL:
    """The Senior members' layout tables as compiled bytes, keyed by tag, plus the `usMaxContext` feaLib would have stamped on the font's OS/2 for them — a side effect of the compile that has to travel with the tables, since the font that receives them never runs feaLib."""
...
def compile_senior_otl(
    ...
    """Compile the Senior members' OpenType layout tables once, over a bare font that carries nothing but the glyph order. Regular and Bold Senior share their glyph order and their whole feature file — bold only widens the painted pixels — so their GDEF, GSUB and GPOS are byte-identical, and feaLib's compile is the costliest step of a Senior build in both time and memory. `main()` runs this in one pool worker while the other families build, then hands the bytes to both Senior jobs through `build_font(senior_otl=…)`, which writes them verbatim instead of compiling the feature code a second time."""
...
def build_font(
    ...
    Args:
        ...
        compiled: The variant's compiled glyph set, when the caller already has it; `bold` does not enter the compile, so `main()` compiles Senior once and hands it to every Senior job. Left None, the build compiles its own.
        senior_otl: The Senior members' compiled GDEF/GSUB/GPOS, as `compile_senior_otl` returns them, used only when `variant == "senior"`. Given, the build writes those bytes verbatim in place of compiling the feature code, which is the costliest step of a Senior build in both time and memory; left None, it compiles the feature code itself.
...
def _quiet_dm_coverage_sort_warning() -> Any:
    # The mono build inherits Departure Mono's GSUB/GPOS wholesale, and some of DM's coverage tables aren't stored in glyph-id order. fontTools re-sorts them on write (so the output is correct) but logs "Coverage is not sorted by glyph ids." once per such table. That's noise about DM's own data, not ours, so drop just that message while serializing the mono font.
...
def _build_one_font(
    ...
    # Worker entry point for ProcessPoolExecutor. Each worker reloads glyph data from disk (≈125 ms) so the parent doesn't have to pickle the GlyphData payload over. The Senior members receive their shared compiled glyph set, feature code and layout tables so neither worker compiles or emits any of them again.
...
def main() -> None:
    ...
    # The Senior Regular and Bold members share one compiled glyph set, one feature file and one set of layout tables, so each is made once: the glyph set and the feature code here, since the emit is what the layout compile consumes, and the layout compile in a pool worker of its own, alongside the mono and Junior builds, so the two Senior jobs start with everything but their outlines already in hand.
    ...
    # Half the box's cores: the pool is never the only thing running, since the test suite runs this build under `conftest.py` while pyright runs beside it, and `build_check_html.py` runs that suite in-process.
