...
Bold is generated by rendering each "on" pixel as a 1.5× wide rectangle
(half-pixel rightward overstrike) while leaving the logical 50-unit pixel
grid, advance widths, kerning, and anchors unchanged.
"""
...
def build_font(
    ...
    Args:
        ...
        bold: When True, widen each drawn pixel by `pixel_width // 2` units
              (half-pixel rightward overstrike). The logical pixel grid,
              advance widths, and anchors are unchanged; only the painted
              rectangles grow. The resulting font is named and style-linked
              as the Bold member of its family.
