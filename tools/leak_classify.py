...
A leak is **bad** ⇔ it is **visible** (the rendered run differs from the concatenation of its boundary-faithful halves) *and* a changed flanking form is an **additive dangle** — it gained, in context, a break-facing connector the isolated form lacked, so the stroke reaches toward a neighbor that isn't there. Everything else is **benign**: subtractive trims that make a letter more self-contained, standalone variant swaps, and all invisible swaps. A little benign leakage is welcome — it is the faux-organic variation the script wants.

### The additive signal is the *gained break-facing anchor*, not a static token

`doc/definitions/shaping-leakage.md` decision 6's table filed the additive class under `ex-ext-N`/`en-ext-N`. Measured against the 99 human-verified "broken" leaks that fires on **zero** of them: the real dangle in this font is the in-context form **gaining a break-facing connecting anchor** vs its isolated form — `ex-y0`/`ex-y5`/`ex-y6` on the left glyph's exit (reaching right into the break), `en-y0`/`en-y5`/`en-y6`/`en-y8` on the right glyph's entry (reaching left into the break). 97 of 100 broken rows involve such an anchor. This matches the decision's *prose* ("reaches connector ink toward the across-break neighbor"); only the token bucketing was wrong. So the test is a **delta** — what the chosen form gained relative to the isolated form on the break-facing edge — and the `ex-ext`/`en-ext` length tokens (and `extended`) stay in the additive set because they co-occur and are genuinely additive.

A break-facing *subtractive* edge wins over any additive token on that same edge: a left form carrying `noexit`/`ex-noentry` has no exit to dangle; a right form carrying `noentry` has no entry to dangle. So `qsThey_qsUtter.noentry.ex-con-1` (subtractive on the entry) reads benign even though it changed.
