...
- `make test-rebuild` — the rebuild suite's self-skipping wrapper classifies the failure set itself: expect a green naming the four documented baseline spec-pins plus the census-pinned review tests (audit/enrich/ink/families/build), which re-baseline at the artifact cycle, and no hard failures; `rebuild/out/cycle_summary.json`'s `gates.rebuild` line names how many of each the last cycle counted. The three qsAh-era content pins (test_table orphan window, test_emit guard lines ×2, test_specificity depth-3 axes) are already updated and green.
