...
The whole-corpus sweeps that used to live here are gone: what every drafted pin and policy record must satisfy — the pin parses and replays "pass", the record is schema-valid, the any-of candidates are distinct and parseable — the drafter now refuses to produce anything else, raising `DraftError` where the draft is made rather than recording a failure for `check_unit` to reject downstream, so no sweep of shipped fragments can witness a violation that exists. (A machine-approved or verdict-exempt unit is never drafted at all and its fragment omits `drafts`; that a record names only trace provenance in a real file is still `check_unit`'s and `check_shards`'.) The worked examples that remain take their windows from `example_units`, a filtered load of the frozen mini bundle's audit, and shape them in the bundle's own font — how the drafter words a record is a property of the drafter, so nothing here reaches the live corpus.
"""
