...
The whole-corpus sweeps that used to live here are gone: what every drafted pin and policy record must satisfy — the pin parses and replays "pass", the record is schema-valid and names only trace provenance in a real file, the any-of candidates are distinct and parseable — `check_unit` now asserts on every human-workload unit the build ships, cached ones included (the ink-identical and Junior-equivalent units carry `drafts: null` and are never drafted at all), which is a superset of what a re-enrichment of the workload could witness. The worked examples that remain take their windows from `example_units`, a filtered load of the audit rather than the whole 451k-unit graph.
"""
