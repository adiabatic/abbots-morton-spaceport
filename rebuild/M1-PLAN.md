...
1. `baseline_subset.py` filters every `rebuild/out/baseline-*.tsv.gz` (one streaming pass each; canonical row order preserved) and the triage TSV to the alphabet → cached sub-tables under `rebuild/out/m1/`.
