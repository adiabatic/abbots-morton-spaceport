...
8. **Oracle conformance** (§6 above) — no subset baseline row matches more than one ledger entry (`multi_matched == 0`), across every configuration in `conform.ACCEPTANCE_CONFIGS`, with the ss06/ss07 identity assertion; unmatched rows are informational and verdict-gated on the review surface.
