...
def write_settle_fold(path: Path, plan: GsubPlan, inputs: str | None, readback_pass: bool, font: Path | None = None) -> None: ...
    # the emitted settlement rows in FEA order with their (config, rule index) sources, stamped like the
    # windows and written whether or not read-back passed; rebuild/test_rule_witnesses.py holds it to the
    # stamped tables, so its coverage is counted over the rows that ship.
...
def run_conformance(font_path, spec, configs) -> ConformReport: ...
    ...
    # font holds every planned rule per build, and rebuild/test_rule_witnesses.py is the font-free
    # dead-rule alarm. `make conform-deep` (rebuild/tools/deep_sweep.py) runs the same sweep at horizon
    # 5+ on demand, green-keyed on emit_gsub.behavior_classes + the compilation code + uharfbuzz.
