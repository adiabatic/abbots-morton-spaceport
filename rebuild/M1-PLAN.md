...
**The namer dot is in scope as a token**: M1's emitter supplies its own dot-lowering mini-calt stage (Recon B proved `_namer_dot_calt_fea` is a no-op on the `senior_fea` path), and the glyph set includes `periodcentered` and `periodcentered.lowered`. No subset record conditions on `is: namer-dot`, so the condition axis is registered but unexercised (recorded in the report).
...
The `families` registry is what lets the dead-policy gate distinguish a **deferred-partner record** (condition references only families with no rune file yet — e.g. qsMay's after-·Fee bound contract, qsPea's baseline entry `from: [qsEt, qsAwe]`) from genuine dead policy. Deferred-partner records are listed in the report, not failed.
...
### §6.1/§6.2 semantics coverage — what the real records exercise (for the report)
...
Unexercised by real records, recorded honestly: **prefers in both modes and both grains, `resolve` and the arbitration errors, positive `word:` records, `require`, `split:`, `trim:`**, `bind:` **at settlement level** (qsMay's after-·Fee bound contract is authored but deferred-partner — qsFee has no rune file; geometry unit-tests `bind:` synthetically), `is: namer-dot` conditions, `stroke:` conditions in policy, case-group promotion and the subsumption linter, late formation. **§6.2 extensional specificity is implemented in full with its dedicated regression-test class, but its two named design cases (the decline-discriminator window, the qsJay contract-vs-extend overlap) need qsThey/qsJay and run on synthetic fixture specs, not M1 rune files.** All of this goes in `rebuild/M1-REPORT.md` verbatim-in-spirit, as the prototype's out-of-scope list did.
...
6. **Dead policy asserted empty or explained** — deferred-partner records reported as a list; genuinely-dead-in-alphabet records must each carry a written explanation in the report (expected: qsPea's baseline-entry row).
...
10. **Ductus parity + draft flags** — every stance names a way; every drafted way carries `# DRAFT — pending author sign-off`; the report enumerates all drafts and unrealized ways for sign-off.
...
Phase 3 (author the six rune files + `rebuild/script.yaml` + the two schemas, ductus drafts flagged) can start immediately and runs alongside Phase 4's three implementation groups, which begin by freezing `model.py`. Phase 5 integrates: load the real rune files through Group 1, run Groups 2–3 end to end, populate the alias map and the ledger counts, drain triage to ground, run all twelve gates. Phase 6 writes `rebuild/M1-REPORT.md` (including the semantics-coverage section, the deferred-partner list, the dead-policy explanations, the draft-ductus sign-off list, and the registry/`script.yaml` location deviation) and re-verifies byte identity and the full suite.
...
- **`rebuild/pipeline/run_m1.py` is the Phase 5 driver** (`uv run python -m rebuild.pipeline.run_m1`): tables + TSVs, glyph minting (settled cells named by `settle.cell_label`, so rules and glyphs agree by construction; raw cmap glyphs carry no curs anchors), defect gates merged with `surface.check_anchor_conventions`, emit, mini-font build, then the oracle gate. Font-side conformance and the CoreText smoke run via `run_font_conformance` and `python -m rebuild.pipeline.coretext_smoke`.
