...
def test_kernel_threads_budget_takes_the_pytest_pool_off_the_box_first():
    """The pytest pool comes off the box beside default's retained memo before division. At the 5.7 GB per-delta bound beside the 2.5 GB memo snapshot, the 33.5 GB box fits four deltas alone; subtracting the normal 0.6 GB pytest pool leaves room for three. This boundary makes a missing reservation change the answer."""
    solo = ac.kernel_threads_budget(skip_make_test=True, ncores=8, total_bytes=BOX_33_5_GB)
    beside = ac.kernel_threads_budget(ncores=8, total_bytes=BOX_33_5_GB)
...
def test_replay_threads_budget_takes_the_pytest_pool_off_the_box_first():
    ...
    solo = ac.replay_threads_budget(skip_make_test=True, ncores=8, total_bytes=BOX_33_5_GB)
    beside = ac.replay_threads_budget(ncores=8, total_bytes=BOX_33_5_GB)
...
def test_a_plan_reserves_for_the_pytest_pool_only_when_that_gate_runs():
    ...
    assert _plan(ncores=8, total_bytes=BOX_33_5_GB).kernel_threads == 3
    assert (
        _plan(
            ncores=8, total_bytes=BOX_33_5_GB, skip_make_test=True, make_test_note="closure unchanged"
        ).kernel_threads
        == 4
    )
    assert _plan(ncores=8, total_bytes=BOX_33_5_GB, skip_gates=True).kernel_threads == 4
