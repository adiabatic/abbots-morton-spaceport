...
- The drafted `ductus` awaits the author's vet; the `# DRAFT` marker is the worklist (the old record has no ductus at all, so the whole motion prose is drafted from the bitmap).
