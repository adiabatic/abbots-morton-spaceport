...
GAIN_UNDER_CREATED_JOIN_RULE = {
    "id": "fixture-ink-gain-under-a-created-join",
    "verdict": "approve",
    "note": "·Tea keeps the full bar under ss03, and the fuller letter carries ·F3 a column right",
    "match": {
        "before": {"pivots": ["qsTea.half.en-y5.after-xheight-exit"]},
        "after": {
            "pivots": ["qsTea.full"],
...
GAINED_CREATED_JOIN_RULE = {
    "id": "fixture-join-created-behind-a-gain",
    "verdict": "approve",
...
FRAMED_REDRAWN_RULE = {
    "id": "fixture-redrawn-framed-contraction",
    "verdict": "approve",
    "note": "\u00b7Eight pulls its entry in and reaches forward in one stroke, its own frame taking the contraction so the letter stands where it was",
    "match": {
        "before": {"pivots": ["qsEight.frame-fixture"]},
        "after": {
            "pivots": ["qsEight.framed-loop.en-con-1"],
            "dropped": [[0, 0]],
            "added": [[3, 0]],
            "shift": 0,
        },
...
FRAMED_EIGHTISH = (_rect(50, 0, 100, 150), _rect(100, 100, 150, 150), _rect(150, 0, 200, 50))
OVERFRAMED_EIGHTISH = (_rect(100, 0, 150, 150), _rect(150, 100, 200, 150), _rect(200, 0, 250, 50))
...
SLIDE_FONTS = {
    ...
    "after-redrawn-overframed-pivot": (
        {**AFTER_GLYPHS, "qsEight.framed-loop.en-con-1": (OVERFRAMED_EIGHTISH, 150)},
        AFTER_CMAP,
    ),
}
...
def gained_created_join_window(uid="gcj-1"):
    return unit(
        uid,
        list(GAINED_CREATED_JOIN_GLYPHS),
        ["y0", "break"],
        [
            "qsL/full/None/None/",
            "qsTea/full/x-height/baseline/",
...
def test_neither_rule_alone_reads_a_created_join_behind_a_gain(slide_context):
...
def test_a_frame_moving_further_than_its_contraction_names_is_refused(slide_context):
    assert not sv._matches(
        FRAMED_REDRAWN_RULE["match"],
        framed_redrawn_window(),
        context=slide_context("after-redrawn-overframed-pivot"),
    )
