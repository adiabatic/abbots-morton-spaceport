...
Two mechanisms keep a wrong record from laundering itself forever, because nothing else would: a served record is re-emitted under the current stamp, so provenance alone can never age it out, and `gate:conform` — which does re-settle the identical universe every cycle — proves the font reproduces a fresh settlement and never compares a cached verdict against a fresh one, so it is not a backstop for this. First, every record keeps the `derived_at_pass` it was *derived* at rather than the pass that last re-emitted it, and `RowStore.stale` forces a re-derivation once that age reaches `MAX_RECORD_AGE`, spread by row ordinal so one row in that many re-derives every pass rather than the whole table on the pass the cap comes due. Second, `VerificationSample` draws up to `VERIFICATION_SAMPLE_PER_FAMILY` served rows for every family that served any, seeded on the stamp, the family and the ordinal of the pass the store is being read into, so the covered slice rotates instead of re-proving the same fraction of a percent forever — and since that ordinal advances only when a store is written, a pass forbidden to write one rotates on the clock instead of standing still, which is what keeps `--gates-only` from re-proving a single frozen sample every time it runs; the caller re-walks those rows and compares the full record. Stratifying by family is what catches a family-wide poisoning with probability one rather than with probability sample-over-served, and a family-wide poisoning is the shape the mid-run rune edit produces.
