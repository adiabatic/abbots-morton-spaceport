...
What is cached is exactly `conform._compare_row`'s answer — the `DivergentRow | None` a row's settled stream and the alias map produce — and nothing else. That narrowing is the whole design, and it rests on two signatures: `_compare_row(spec, aliases, config, features, row, settled)` takes no font and no shaper, and `_position_drift(shaper, kern, features, row)` takes no settled stream. So the position channel is recomputed on every row on every pass, and `M1.otf`, the GSUB fold, and `glyph_data/senior_quikscript_kerning.yaml` are outside this key entirely — which is what makes the cache worth having at all. The review surface's unit cache measured the alternative: folding the after font's GSUB wiring into a stamp produced "a store that never once served a unit", because a rune edit moves the lookup list on essentially every cycle. The position channel's only mutation of a row here is appending `"position"` to `kinds` and `("position-drift",)` — plus optionally `"position-kern-attributable"` — to `phenomena`, so a served row and a fresh one enter that channel in the same state. If any of those three facts stops holding, this cache is silently dishonest and its format version must move.

Classification is outside the key by construction rather than by argument: `classify_divergence` and `_match_ledger` run over every row on every pass, served or fresh, so `rebuild/m1-divergences.yaml` and every ledger predicate are re-applied to the served verdict. A pure ledger edit therefore serves every row and still rewrites every `matched_entry` — the workflow `run_m1 --gates-only` advertises, and the one this cache actually pays for. What that argument cannot buy is the classifier's *code*: `classify_divergence`, `_cell_deltas` and `_compare_row` share `conform.py`, so no module-grain split can put the producer inside the stamp and the classifier outside it, and `rebuild/pipeline/conform.py` stays in `ORACLE_ROW_CODE_PATHS` permanently.
...
This is deliberately not `review.unit_cache.family_content_keys`, and the divergence is the point rather than an oversight. That key folds a `glyphs` line over the after font's compiled outlines, which is right for a cache whose product is a rendered review card and wrong for one whose product never reads a font — folding it in here would drop the store on exactly the workflow the store exists for. The import direction settles where the sibling lives: `rebuild/review/` imports `rebuild/pipeline/`, never the reverse, so the shared halves (`fingerprint.rune_digests`, `spec_load.rune_closure`, `spec_load.capability_features`, `spec_load.spec_structure_digest`) live in the pipeline and both callers reach for them there.

Deliberately outside the whole-store stamp, each for a reason worth stating. `M1.otf` and the kern sidecar: the position channel always recomputes. `rebuild/m1-divergences.yaml`: classification always recomputes. `rebuild/m1-contact-allow.yaml`: no oracle stage reads it — it is the defect gate's allow-list — and it moved in eighteen of the last sixty rune commits, so stamping it would collapse the store for nothing. The rune files: they invalidate at per-family grain through the keys. What is inside is everything that can move a verdict without moving a named family — the comparison's own code closure, the remaining data inputs, the resolved spec structure and the capability-feature universe (a predicate class gaining a member, a rune-local group, a ligature sequence, a feature unlock: cross-rune routes the per-family grain cannot decompose, since `specificity::family_set` expands a class reference to its whole member set and ranks by set size, so a rune joining a class can flip a window holding no such rune), the engine's semantics flags, the configuration's feature set, and the subset table's own bytes.
...
Two mechanisms keep a wrong record from laundering itself forever, because nothing else would: a served record is re-emitted under the current stamp, so provenance alone can never age it out, and `gate:conform` — which does re-settle the identical universe every cycle — proves the font reproduces a fresh settlement and never compares a cached verdict against a fresh one, so it is not a backstop for this. First, every record keeps the `derived_at_pass` it was *derived* at rather than the pass that last re-emitted it, and `RowStore.stale` forces a re-derivation once that age reaches `MAX_RECORD_AGE`, spread by row ordinal so a twentieth of the table re-derives every pass rather than all of it every twentieth. Second, `VerificationSample` draws up to `VERIFICATION_SAMPLE_PER_FAMILY` served rows for every family that served any, seeded on the stamp, the family and the ordinal of the pass the store is being read into, so the covered slice rotates instead of re-proving the same fraction of a percent forever — and since that ordinal advances only when a store is written, a pass forbidden to write one rotates on the clock instead of standing still, which is what keeps `--gates-only` from re-proving a single frozen sample every time it runs; the caller re-walks those rows and compares the full record. Stratifying by family is what catches a family-wide poisoning with probability one rather than with probability sample-over-served, and a family-wide poisoning is the shape the mid-run rune edit produces.
...
Everything degrades toward a full pass. `load_store` answers `None` for an absent, unreadable, format-mismatched, stamp-mismatched, digest-mismatched, short or trailer-less store, and a `None` costs one cold oracle and nothing else.
"""
...
STORE_FORMAT = "ams-m1-oracle-rows/1"
...
def scratch_store_path(scratch_dir: Path, config: str) -> Path:
    """Where a store is staged while the oracle runs: a subdirectory of this run's pid-named audit scratch, so `discard_oracle_audit_scratch` sweeps a killed run's stores the way it already sweeps its shards, and so `join_oracle_audit`'s missing-shard diagnostic lists one directory rather than six stores."""
...
def stamped_data_paths(repo_root: Path) -> list[Path]:
    """The data inputs the whole-store stamp folds: `fingerprint.data_paths` less the rune files, which invalidate at family grain through the keys instead, and less the four the comparison never consults — the alias map, whose family heads are stamped per family and whose two boundary heads ride their own line; the divergence ledger and the contact-allow list, since classification and the defect gate re-read them over every row on every pass; and the kern sidecar, since the position channel never serves. It is exported because `artifact_cycle.oracle_cache_note` has to say what a moved input will cost the store before the oracle has run, and a second hand-kept copy of this list is precisely the thing that would drift out of agreement with the stamp it is describing."""
    root = Path(repo_root)
...
def encode_record(codepoints: Sequence[int], cached: CachedRow | None, derived_at_pass: int) -> str:
    """One store line: the anchor, then `-` and the pass for a clean row, or `P` and the verdict's five fields and the pass for a divergent one. `|` separates cells and `,` separates the token tuples, matching what `divergence-audit.tsv` already does with the same values."""
    anchor = row_anchor(codepoints)
    if cached is None:
        return f"{anchor}\t-\t{derived_at_pass}"
    return "\t".join(
        (
            anchor,
            "P",
...
def decode_record(line: str) -> CachedRow | None:
    fields = line.split("\t")
    if fields[1] == "-":
        return None
    return CachedRow(
        kinds=_split(fields[2], ","),
        position=int(fields[3]),
        new_cells=_split(fields[4], "|"),
        new_seams=_split(fields[5], ","),
        phenomena=_split(fields[6], ","),
    )
...
class RowStore:
    ...
    def due(self, index: int) -> bool:
        """Whether this row must re-derive regardless of its families. The ordinal clause retires one row in `MAX_RECORD_AGE` every pass, so no verdict can stand that many passes without being recomputed and the renewal is spread across passes rather than arriving all at once on the twentieth; the age clause is the belt to those braces, and catches a store whose pass ordinals skipped. Only the slice rotates for a read-only pass — the age arithmetic stays on the true ordinal, because a rotated `current` would read every record as older than the cap and retire the whole table."""
        if self.pass_ordinal + 1 - self._ages[index] >= MAX_RECORD_AGE:
            return True
    ...
    def serve(self, index: int, codepoints: Sequence[int]) -> CachedRow | None:
        """This row's recorded verdict, after proving the record is this row's. A mismatched anchor is not a miss and must not be treated as one — it means the table under this store was replaced or reordered, and every other record is wrong the same way."""
...
def load_store(
    ...
    try:
        ...
        while cursor < limit:
            ...
            ages.append(int(blob[blob.rindex(b"\t", cursor, end) + 1 : end]))
            cursor = end + 1
        ...
        return None
    mask = StaleMask(spec, moved_families(recorded_keys, current_keys))
    return RowStore(
...
class RowWriter:
    ...
    def append(self, codepoints: Sequence[int], cached: CachedRow | None, derived_at_pass: int) -> None:
        """Record one row, divergent or clean — every subset row gets a record, in table order, because the ordinal is the key and a clean row with no age would be the one thing the renewal cap could not retire. `derived_at_pass` is the pass the verdict was computed at: `RowStore.age(index)` for a record this pass only served, `self.pass_ordinal` for one it derived. Passing this pass for a served verdict is the laundering the age exists to prevent."""
        self._stream.write((encode_record(codepoints, cached, derived_at_pass) + "\n").encode())
        self.rows += 1
