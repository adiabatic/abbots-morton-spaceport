...
# What default's retained trace memo holds for the length of the delta wave, the co-resident term taken off the box before the division. `run_config_tables` folds and drops default's product before it returns, so what survives into the wave is the `MemoSnapshot` alone — one hash map and its pools (`rebuild/kernel-rs/src/memo.rs`), kept at the release point in `rebuild/kernel-rs/src/fixpoint.rs` where every delta's memos go — a memo rather than a configuration held through enumeration, which is why it is smaller in kind than DELTA_PEAK_BYTES and priced separately. No census line reports it directly: `--cache-census`'s `resident_after_release` line for default reads 4.15 GB while the process still holds default's transitions and seats for the fold, so the snapshot is bounded from above by that reading and from the whole-process peak of a build holding every delta at once (14.78 GB) less what the deltas account for. On a seeded build the term stands for two snapshots, not one: `build_table_files` (`rebuild/kernel-rs/src/fanout.rs`) reads the previous build's default memo whole through `load_seed` and holds it beside the fresh snapshot for the length of the wave, so every artifact-cycle pass after the first carries a second default memo the from-scratch readings never see — the cycle's seeded pass four deltas wide records 15.89 GB against the 14.78 GB from-scratch arm. The pair is chosen with DELTA_PEAK_BYTES against the criterion its comment states, both fleet boxes seating the whole delta wave, and the two are re-measured together in both arms, never one alone. `TABLE_BUILD_PEAK_BYTES` is the whole-process reading the two bound.
DEFAULT_MEMO_BYTES = 2_500_000_000
# What the one `build-tables` process holds at its peak: default's retained memo beside every delta in flight, each held through its memo write, bounded by DEFAULT_MEMO_BYTES plus DELTA_PEAK_BYTES times the delta width. The direct whole-alphabet measurement with ·Zoo, four deltas wide and writing memo files without a previous-build seed, peaks at 14.78 GB; the artifact cycle records a 17.09 GB build peak. This estimate keeps headroom above both readings because the configurations' peaks and memo writes need not coincide identically across runs. `make job-costs` watches the cycle's build peak against this figure; it does not divide a pool by it.
TABLE_BUILD_PEAK_BYTES = 20_000_000_000


def kernel_threads_default(*, coresident_bytes: float = 0, total_bytes: int | None = None) -> int:
    """How many delta configurations this box has room for at once beside `default`'s memo: `AMS_KERNEL_THREADS` wherever it is set at all, else `DELTA_PEAK_BYTES` divided into the box by `memory_budget.how_many_fit` with `DEFAULT_MEMO_BYTES` taken off it first — the memo `default` keeps alive for the wave, a snapshot of one hash map and its pools rather than a configuration held through enumeration, which is why the two terms are different numbers — after the reserve that policy states, floored at one. A stated width is floored at one and clamped no further, because the configuration count and the cores this process may actually run on are not memory facts and belong to `run_m1.build_tables`'s own `min()`. A value that is not a bare count — a typo, a `GB` suffix, a variable declared without one — raises rather than falling quietly through to the arithmetic, which is where this knob parts company with `AMS_TOTAL_MEMORY_BYTES`: that one reproduces a box and ignoring a typo in it costs a reproduction, while this one is what someone reaches for to keep a build out of swap, so handing them a width they did not ask for is the failure they set it to prevent. `total_bytes` is a keyword so an assertion about another box is a pure function over an invented one; the alternative is `importlib.reload`, which re-runs module scope, resets `_BUILT`, and drops the live `_SPEC_DUMPS` entries' temporary directories under a caller still holding a `spec_path`.
