...
# What the one `build-tables` process holds at its peak: default's retained memo beside every seat in flight, bounded by DEFAULT_MEMO_BYTES plus DELTA_PEAK_BYTES times the width. Window rows retain product-local label IDs through artifact emission, with outcomes indexed by settled seat; `fixpoint.rs` pins their compact layout and drains directly into the final vector before sorting it in place. Table hashing in `artifacts::table_digest` retains a SHA-256 block and a reusable row buffer rather than a whole windows payload. Enumeration and folding still hold corpus-sized state. Direct measurements with ·Ye on the 32 GiB box, under `/usr/bin/time -l`, with `--cache-census` at four seats and memo writes, read 10.55 GB maximum resident memory and 14.13 GB peak memory footprint from scratch, and 11.85 GB resident and 16.27 GB footprint when seeded with ·Ye invalidated. Those gaps exceed the two-gigabyte admission margin argued beside DELTA_PEAK_BYTES, so the compressed resident readings do not seed a smaller bound. The 22 GB bound covers those footprints and the fleet's admitted 20.37 GB seeded four-seat reading, with headroom for the scheduling-dependent overlap of the configurations' high-waters. The 48 GiB box's five-seat wave requires its own direct measurement before a smaller fleet bound is justified. `make job-costs` watches the cycle's sampled run_m1 process peak against this figure; it does not divide a pool by it, and the delta width remains priced by DELTA_PEAK_BYTES and DEFAULT_MEMO_BYTES.
