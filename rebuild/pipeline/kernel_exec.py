...
# What one configuration in flight costs, and so the divisor every box divides itself by to reach a fan-out width. The figure is one configuration's own rather than the whole tree's, which is what makes it a per-unit cost at all: `peak_rss` maxes `ru_maxrss` over children instead of summing it, `run_m1.build_tables` hands every child `threads=1`, and the `run_m1` parent beside them holds only heads — measured at 0.04 GB against a child's several — so the widest process in that tree is one single-threaded child holding one configuration however wide the fan-out around it is. Seeded off the tree the three memo-side levers of issue #105 landed on (issues #165–#167: the trace memo seated under a packed key, the prospect memo holding only its fallbacks, the candidate memo's lists seated once), measured directly rather than read off the cycle-timings journal: a solo `build-tables` child on the whole-alphabet dump peaks between 4.69 and 4.73 GB across the six acceptance configurations, and the first cycle on that tree read its widest child at 4.78 GB where the cycles before the levers read 10 to 12. The headroom past that is deliberate rather than the tightest divisor the measurement allows, since erring low is what puts a box into swap and erring high only costs a configuration of parallelism — a width that overshot by one was observed paging, at roughly sixty percent longer per configuration for a total wall worse than the next width down — and the peak is still the trace memo's bucket table, sitting just past a power-of-two doubling that the next letter batch will not undo. Re-measure it whenever the crate's per-configuration levers land, by timing one child on its own the way `bench-the-rebuild/levers/kernel_all_configs.py` does and reading `--cache-census` for where the memo sits; it is a reading to keep current, never a contract.
CONFIG_PEAK_BYTES = 5_500_000_000


def kernel_threads_default(*, coresident_bytes: float = 0, total_bytes: int | None = None) -> int:
    """How many configurations this box has room for at once: `AMS_KERNEL_THREADS` wherever it is set at all, else `CONFIG_PEAK_BYTES` divided into the box by `memory_budget.how_many_fit`, which takes off the reserve that policy states and floors at one. A stated width is floored at one and clamped no further, because the configuration count and the cores this process may actually run on are not memory facts and belong to `run_m1.build_tables`'s own `min()`. A value that is not a bare count — a typo, a `GB` suffix, a variable declared without one — raises rather than falling quietly through to the arithmetic, which is where this knob parts company with `AMS_TOTAL_MEMORY_BYTES`: that one reproduces a box and ignoring a typo in it costs a reproduction, while this one is what someone reaches for to keep a build out of swap, so handing them a width they did not ask for is the failure they set it to prevent. `total_bytes` is a keyword so an assertion about another box is a pure function over an invented one; the alternative is `importlib.reload`, which re-runs module scope, resets `_BUILT`, and drops the live `_SPEC_DUMPS` entries' temporary directories under a caller still holding a `spec_path`.
