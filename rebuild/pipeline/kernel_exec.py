...
# What default's retained trace memo holds for the length of the delta wave, the co-resident term taken off the box before the division. default's fold runs at one of the `--threads` seats (`run_configs_tables` in `rebuild/kernel-rs/src/fanout.rs`), so the product it holds is priced as a seat's — bounded by DELTA_PEAK_BYTES, since default enumerated from scratch is exactly what that divisor prices — and the wave is never wider than the width the division answered; on a box whose width exceeds the delta count (the 48 GiB box, where `run_m1._table_build_threads` caps the width at the configuration count) that seat is one the deltas would have left idle, so every delta and default's post phase are resident together there, one working set more than the deltas alone, still inside the seats the division priced. What is co-resident with every seat for the length of the wave is the `MemoSnapshot` alone — one hash map and its pools (`rebuild/kernel-rs/src/memo.rs`), kept at the release point in `rebuild/kernel-rs/src/fixpoint.rs` where every delta's memos go — a memo rather than a configuration held through enumeration, which is why it is smaller in kind than DELTA_PEAK_BYTES and priced separately. Its floor is the map's own pinned widths: a twenty-byte key beside a sixteen-byte entry over the 2^25 buckets default's trace_cache reaches (`--cache-census` prints the capacity, seven eighths of that), about 1.2 GB of buckets plus the pools. No census line reports it directly: `--cache-census`'s `resident_after_release` line for default — sampled once `take_memo` has moved the trace memo into the snapshot and freed the engine's other memos — reads 2.89 GB from scratch against a `resident_before_release` of 4.37 GB, the other memos' pages gone back while the process still holds default's transitions and seats for the fold, so the snapshot is bounded from above by that reading; the `resident_after_memo_write` line after it reads 3.59 GB from scratch, the writer's rows and file tables not yet returned, and the whole-process peak of a build holding every delta at once (19.24 GB four wide) less what the deltas account for bounds it a second way. On a seeded build the term stands for two snapshots, not one: `build_table_files` (`rebuild/kernel-rs/src/fanout.rs`) reads the previous build's default memo whole through `load_seed` and holds it beside the fresh snapshot for the length of the wave, so every artifact-cycle pass after the first carries a second default memo the from-scratch readings never see — default's `resident_after_memo_write` reads 3.50 GB seeded, and the direct measurement four deltas wide reads 20.37 GB seeded against 19.24 GB from scratch. Both seeded readings are taken over a seed whose edit set is empty, the cheapest seeded shape: default carries the whole previous memo whatever moved (`load_seed` keeps every key for it) and re-settles nothing — its own trace_cache reads empty in the census, every window a `memo_base_hits` hit — so they price a whole carried seed beside an empty fresh snapshot, and a pass whose edit re-settles windows holds a fuller fresh snapshot beside the same carried seed, two full maps at the limit; the figure sits between one snapshot and two, and `make job-costs`' kernel-build row, the cycle's own seeded builds after real edits, is the record that watches that arm against `TABLE_BUILD_PEAK_BYTES`. The pair is chosen with DELTA_PEAK_BYTES against the criterion its comment states, both fleet boxes seating the whole delta wave, and the two are re-measured together in both arms, never one alone. `TABLE_BUILD_PEAK_BYTES` is the whole-process reading the two bound.
DEFAULT_MEMO_BYTES = 2_500_000_000
# What the one `build-tables` process holds at its peak: default's retained memo beside every seat in flight — a delta held through its enumeration and the memo write at its release point, or default's own fold at the seat it takes — bounded by DEFAULT_MEMO_BYTES plus DELTA_PEAK_BYTES times the width. The direct whole-alphabet measurement with ·Jay on the 32 GiB box under the packed keys and the sixteen-byte trace entry, four deltas wide and writing memo files, peaks at 19.24 GB without a previous-build seed and 20.37 GB seeded, and at five seats — the 48 GiB box's width, every delta beside default's fold — at 19.56 GB and 18.92 GB, with the `peak memory footprint` line at 19.06, 19.58, 22.35 and 21.45 GB beside those four (DELTA_PEAK_BYTES's comment says why a resident reading counts only where that line sits within two gigabytes of it, which on this box admits the two four-seat readings and leaves the five-seat pair to the compressor); `make job-costs`' kernel-build row is the artifact cycle's own record of the build peak, read by run_m1's two-second sampler, which reads about 12% under `/usr/bin/time -l` for the same shape. The 32 GiB box's width of four seats three deltas beside default's post phase; the 48 GiB box seats all four deltas beside it, and its own five-seat readings under this binary sit under the 32 GiB box's four-seat ones: 15.93 GB seeded with the footprint line level at 15.90 GB, an admitted reading, and 10.96 GB from scratch against a footprint of 14.71 GB, the compressor's, so that arm is bounded below by its footprint and seeds nothing. #265's baselines on that box, 20.02 GB from scratch and 21.06 GB seeded at five seats, are from a binary that carried every delta's memo through its drain and its sort under forty-byte trace keys, so the seeded arm's fall from 21.06 to 15.93 GB is what the release point, the packed keys and the sixteen-byte entry give back together, unattributed among the three; the same shape read by run_m1's sampler in an artifact-cycle pass on that box is 16.26 GB, the kernel-build row's record there. The figure is seeded from the highest admitted reading on either box, the 32 GiB box's 20.37 GB seeded four wide, and keeps headroom above every admitted reading because the instant the deltas' high-waters coincide moves from run to run — the two arms order differently at the two widths, and the four footprints above span 3.3 GB — so one pass reads a gigabyte either way of another. `make job-costs` watches the cycle's build peak against this figure; it does not divide a pool by it.
TABLE_BUILD_PEAK_BYTES = 22_000_000_000
# What one configuration's horizon-4 string replay holds at its peak, the divisor `run_m1.run_replay_strings`'s width comes from: the engine's trace memo over the windows the string universe's texts reach, with no liveness probe cascade beyond the prospect's own, plus the window memo's inverse label map and block buffer when the walk files its dump — a subset of what the enumeration's engine holds, which is why it is a fraction of DELTA_PEAK_BYTES and priced on its own rather than inherited from it. Nothing comes off the box before the division: the replay starts after the `build-tables` process has exited, so no configuration's retained memo is alive, and each worker loads one settlement TSV and builds its own engine. With ·Jay in the alphabet, the trace key packed onto ordinals and the trace entry at sixteen bytes, one `replay-strings` over `default` alone at `--threads=1` with `--memo-dir` on — the shipped path, since a whole-universe walk always files its dump — peaks at 3.39 GB under `/usr/bin/time -l`, its `peak memory footprint` line the same, and walks 1,544,760 texts over 2,587,847 windows in 13.1 s; the same walk over every settlement configuration at the configuration count peaks at 16.49 GB whole-process, 3.30 GB per worker, with no swaps, the footprint level with the resident figure and under three seconds of system time against 85 of user, and each configuration's `[t] replay[<config>]` line at 15.2 to 15.4 s against the solo 13.1 — five walks sharing the cores' memory rather than paging, which shows in the system time and in a footprint above the resident figure, and neither moves here. The headroom past the reading errs high on purpose, as DELTA_PEAK_BYTES does and for the same reason — a per-unit cost that errs low is what puts the box into swap, while one that errs high only narrows the wave — and it also stands for the neighbors no term subtracts: the replay shares run_m1's process tree with the glyph chain, the window packers and the shipped-order walkers, each a small, flat working set beside it (`run_m1._core_bound_threads` prices the last two). The criterion it is chosen against is that both fleet boxes (`doc/fleet.md`) seat every settlement configuration in one wave, solo and under a gated cycle with gate:make-test's pytest pool on the box; `rebuild/test_memory_budget.py` and `rebuild/test_artifact_cycle.py` pin it. Re-measure it the same way whenever the alphabet grows, `REPLAY_HORIZON` moves or the trace memo's key or entry changes shape: the two runs above, both with the memo dump on, and a per-configuration line that lengthens in the wide run is the width having been bought with paging.
REPLAY_PEAK_BYTES = 4_000_000_000


def kernel_threads_default(*, coresident_bytes: float = 0, total_bytes: int | None = None) -> int:
    """How many seats of the table build's wave — a delta configuration each, one of them taken by `default`'s fold — this box has room for at once beside `default`'s memo: `AMS_KERNEL_THREADS` wherever it is set at all, else `DELTA_PEAK_BYTES` divided into the box by `memory_budget.how_many_fit` with `DEFAULT_MEMO_BYTES` taken off it first — the memo `default` keeps alive for the wave, a snapshot of one hash map and its pools rather than a configuration held through enumeration, which is why the two terms are different numbers — after the reserve that policy states, floored at one. A stated width is floored at one and clamped no further, because the configuration count and the cores this process may actually run on are not memory facts and belong to `run_m1.build_tables`'s own `min()`. A value that is not a bare count — a typo, a `GB` suffix, a variable declared without one — raises rather than falling quietly through to the arithmetic, which is where this knob parts company with `AMS_TOTAL_MEMORY_BYTES`: that one reproduces a box and ignoring a typo in it costs a reproduction, while this one is what someone reaches for to keep a build out of swap, so handing them a width they did not ask for is the failure they set it to prevent. `total_bytes` is a keyword so an assertion about another box is a pure function over an invented one; the alternative is `importlib.reload`, which re-runs module scope, resets `_BUILT`, and drops the live `_SPEC_DUMPS` entries' temporary directories under a caller still holding a `spec_path`.
