...
# What one configuration in flight costs, and so the divisor every box divides itself by to reach a fan-out width. The figure is one configuration's own rather than the whole tree's, which is what makes it a per-unit cost at all: `peak_rss` maxes `ru_maxrss` over children instead of summing it, `run_m1.build_tables` hands every child `threads=1`, and the `run_m1` parent beside them holds only heads — measured at 0.04 GB against a child's ten — so the widest process in that tree is one single-threaded child holding one configuration however wide the fan-out around it is. Measured directly rather than read off the cycle-timings journal, and the two disagree: a solo `build-tables` child peaks around 10.3 GB and the widest configuration of the six around 11.0, where the `run_m1` rows still in the journal record 8.6 to 8.8 from a tree several rune batches back. That drift is the argument for the headroom here rather than for the tightest divisor the last measurement allows, since erring low is what puts a box into swap and erring high only costs a configuration of parallelism — a width that overshot by one was observed paging, at roughly sixty percent longer per configuration for a total wall worse than the next width down. Re-measure it whenever the crate's per-configuration levers land, by timing one child on its own the way `bench-the-rebuild/levers/kernel_all_configs.py` does and reading `--cache-census` for where the memo sits; it is a reading to keep current, never a contract.
