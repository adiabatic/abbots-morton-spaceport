...
# What one delta holds through enumeration and its memo write, the divisor bounding each delta in flight: a delta that shares nothing with default's memo traces the whole configuration, so it is bounded by a configuration enumerated from scratch, and the seed is the worst delta rather than default. With ·Ye in the alphabet, the trace, prospect and candidates keys packed onto ordinals and the trace entry at sixteen bytes (`rebuild/kernel-rs/src/engine.rs` pins each width), the worst delta enumerated from scratch and alone is `ss03+ss05` — at `--threads=1`, tracing every window itself, its memo write at the release point and its fold included — which peaks at 6.23 GB and 6.14 GB whole-process over two runs under `/usr/bin/time -l` on the 48 GiB box; `ss05` reads 6.12 and 6.17 GB, `ss03` 5.52 GB and `ss04` 5.35 GB, and every run's `peak memory footprint` line sits level with its resident one. The 32 GiB box has no solo reading with ·Ye in the alphabet. That whole-process reading is a seat's cost rather than a seat's plus the process's: what the seats share is the parsed spec and its index, which `spec_parse` reports at 0.0 s and which the process holds in about 0.1 GB before a configuration's engine is built, so the seed rounds the highest reading up to the tenth. The high-water sits at the release point, with every memo table built, and in the memo write that follows it: the `resident_before_release` census line reads 6.06 and 6.14 GB for `ss03+ss05` and 6.03 and 6.12 GB for `ss05`, within two tenths of a gigabyte of each run's peak, and level with the peak for `ss03` and `ss04`, while `resident_after_sort` reads 2.2 to 4.7 GB, so the drain of the transitions, their sort and the fold stay under it and a narrower memo key or entry moves this divisor as far as it shrinks the tables. No live delta traces from scratch — every one runs over default's memo, and its `--cache-census` lines say how much: `memo_base_hits` counts the windows it takes from that memo and its trace_cache length what it traces beyond it; only `config_seed=False` enumerates a live configuration the solo way. In one `build-tables` over every settlement configuration on the 48 GiB box, with each delta's memo file written at its release point and no previous-build seed, `ss03` and `ss03+ss05` each trace 18.37M windows beyond default's memo against the 25.27M they trace alone, `ss05` 4.96M and `ss04` 3.62M, and the process peaks at 18.02 GB whole-process four seats wide, DEFAULT_MEMO_BYTES plus about 3.9 GB a seat, and at 15.87 GB three seats wide; on the 32 GiB box, with ·Jay the newest letter, the same build four seats wide peaks at 19.24 GB, and at 20.37 GB seeded over a previous build's memo files. Those wave readings are ones whose `peak memory footprint` line sits within two gigabytes of the resident figure, on either side of it (18.77 GB beside the 18.02, 15.10 GB beside the 15.87, 19.06 GB beside the 19.24 and 19.58 GB beside the 20.37), which is the condition for seeding from a resident figure at all: the resident line under-reads a wave by whatever the compressor has taken from the process, so the same shape of build under forty-byte trace keys on a pass under memory pressure reads 14.91 GB and 15.92 GB four wide against footprints of 21.08 GB and 21.92 GB, and a reading whose footprint sits gigabytes above its resident line is the compressor's rather than the build's. The divisor prices a seat by its bound rather than by what the wave's seats read, and it errs high on purpose: a per-unit cost that errs low is what puts a box into swap, while one that errs high only narrows the wave. The pair is chosen together with DEFAULT_MEMO_BYTES so that the 48 GiB box (`doc/fleet.md`) seats the whole delta wave at once — every configuration past default in one round — solo and under a gated cycle with gate:make-test's pytest pool on the box, while the 32 GiB box seats three, one short of the four deltas, solo and gated alike, so its wave takes a trailing round; held to three seats, a from-scratch wave on the 48 GiB box takes 245.7 s against 198.9 s at four. `rebuild/test_memory_budget.py` and `rebuild/test_artifact_cycle.py` pin both boxes' widths, and the gated arm is what caps this divisor from above: past 7.75 GB the pool's bytes cost the 32 GiB box its third seat, and past 10.11 GB they cost the 48 GiB box its fourth. Re-measure the two together, never one alone, whenever the alphabet grows or a per-configuration working set changes. The recipe is the solo reading above beside the wave measurement, run in both arms — from scratch, and seeded over a previous build's memo files, the arm every artifact-cycle pass after the first runs — since the seeded arm holds a second default snapshot for the wave (DEFAULT_MEMO_BYTES says which): one `build-tables` over every settlement configuration with memo output, `--cache-census`, and `/usr/bin/time -l`, read only where the footprint line sits within two gigabytes of the resident one. With ·Ye in the alphabet, the seeded arm has no such reading on either box.
DELTA_PEAK_BYTES = 6_300_000_000
# What the shared default snapshots hold for the delta wave, the co-resident term taken off the box before division by DELTA_PEAK_BYTES. `fanout::run_configs_tables` retains the fresh default snapshot beside the previous default seed when present; loading drops keys naming edited runes, and lookup and writing apply the full read-journal exclusion. `memo::SnapshotEntries` holds thirty-six-byte key/entry records plus four-byte hash-prefix offsets, with pools beside them and no hash-table slack. With ·Ye in the alphabet, default's `--cache-census` entry count prices its immutable records and offsets at 0.90 GB, against 1.24 GB for a hash table at that census's capacity; a seeded build can retain two default-sized snapshots, so one snapshot's saving is not the shared term. On the 32 GiB box, all settlement configurations at four seats with memo writes measure 12.46 GB maximum resident memory / 15.49 GB peak memory footprint cold, and 10.92 / 14.95 GB seeded with ·Ye invalidated, under `/usr/bin/time -l`. The corresponding hash-map runs measure 11.73 / 14.62 GB and 11.98 / 16.39 GB; table, window and uncompressed memo bytes agree in both arms. These footprint/resident gaps exceed the two-gigabyte admission margin beside DELTA_PEAK_BYTES; the 48 GiB box's one compact-snapshot reading, the cold four-seat wave DELTA_PEAK_BYTES cites, sits within the margin but cannot separate the shared snapshots from the seats, and neither box has an admitted compact-snapshot reading in the seeded arm, so the 2.5 GB allowance remains conservative and buys no wider pool. Re-measure the retained allowance and TABLE_BUILD_PEAK_BYTES together with the delta divisor under its whole-wave criterion; a compact holder alone does not establish a smaller process peak.
DEFAULT_MEMO_BYTES = 2_500_000_000
# What the one `build-tables` process holds at its peak: shared default snapshots beside every seat in flight, bounded by DEFAULT_MEMO_BYTES plus DELTA_PEAK_BYTES times the width. Snapshot construction is part of this bound: `Engine::take_memo` releases the other live memos before collecting the trace map into an array, holds the map and array together during collection, then drops the map before partitioning. Partitioning holds one four-byte source position per record and the offset cursors, with a temporary sort buffer for an unusually large collision bucket. `read_memo` reserves from a count of TSV window lines, stores accepted records directly, and boxes the compacted array after indexing; it holds no growing hash table. Enumeration, the memo writer and folding still hold corpus-sized state. With ·Ye on the 32 GiB box, direct four-seat measurements under `/usr/bin/time -l` with memo writes and `--cache-census` give 12.46 GB resident / 15.49 GB footprint cold and 10.92 / 14.95 GB with ·Ye invalidated in a previous-build seed. Kernel wall times are 352.20 s and 356.42 s, against 338.08 s and 330.79 s for hash-map snapshots on the same spec; the cold peak is higher despite the smaller retained holder, while the seeded footprint is lower. The resident readings fail DELTA_PEAK_BYTES's admission margin, so they do not justify lowering the 22 GB bound, which also covers the fleet's admitted 20.37 GB seeded reading and scheduling-dependent overlap. The 48 GiB box's five-seat wave requires a direct compact-snapshot measurement before a smaller fleet bound is justified. `make job-costs` watches the cycle's sampled run_m1 process peak against this figure; the delta width remains priced by DELTA_PEAK_BYTES and DEFAULT_MEMO_BYTES.
TABLE_BUILD_PEAK_BYTES = 22_000_000_000
# What one configuration's horizon-4 string replay holds at its peak, the divisor `run_m1.run_replay_strings`'s width comes from: the engine's trace memo over the windows the string universe's texts reach, with no liveness probe cascade beyond the prospect's own and no explain ladder (`Replay::new` pins it off), plus the window memo's inverse label map and block buffer when the walk files its dump — a subset of what the enumeration's engine holds, which is why it is a fraction of DELTA_PEAK_BYTES and priced on its own rather than inherited from it. Nothing comes off the box before the division: the replay starts after the `build-tables` process has exited, so no configuration's retained memo is alive, and each worker loads one settlement TSV and builds its own engine. With ·Ye in the alphabet, the trace key packed onto ordinals, the trace entry at sixteen bytes and no explain ladder, one `replay-strings` over `default` alone at `--threads=1` with `--memo-dir` on — the shipped path, since a whole-universe walk always files its dump — peaks on the 18-core M5 Pro 48 GiB MacBook Pro (`doc/fleet.md`) at 1.24 GB under `/usr/bin/time -l`, its `peak memory footprint` line the same, and walks 1,727,604 texts over 2,909,666 windows in 7.4 s; the same walk over every settlement configuration at the configuration count peaks at 6.10 GB whole-process, 1.22 GB per worker, with no swaps, the footprint level with the resident figure and 0.31 s of system time against 45.4 s of user, and each configuration's `[t] replay[<config>]` line at 8.9 to 9.3 s against the solo 7.4 — five walks sharing the cores' memory rather than paging, which shows in the system time and in a footprint above the resident figure, and neither moves here. The same divisor sizes the settle-memo absorbs that follow a whole-universe walk, since `run_m1.run_replay_strings` opens their pool at the replay's own width, and one absorb of `default`'s dump (`conform.absorb_replay_memo`, 2,909,666 rows) peaks at 0.25 GB in a process of its own, a fraction of the divisor. It rounds the higher of the solo reading and the wide run's per-worker figure up by a quarter and then to the tenth, erring high on purpose, as DELTA_PEAK_BYTES does and for the same reason — a per-unit cost that errs low is what puts the box into swap, while one that errs high only narrows the wave — and the headroom also stands for the neighbors no term subtracts: the replay shares run_m1's process tree with the glyph chain, the window packers and the shipped-order walkers, each a small, flat working set beside it (`run_m1._core_bound_threads` prices the last two). The criterion it is chosen against is that both fleet boxes (`doc/fleet.md`) seat every settlement configuration in one wave, solo and under a gated cycle with gate:make-test's pytest pool on the box; `rebuild/test_memory_budget.py` and `rebuild/test_artifact_cycle.py` pin it. Re-measure it the same way whenever the alphabet grows, `REPLAY_HORIZON` moves or the trace memo's key or entry changes shape: the two runs above, both with the memo dump on, and the absorb over the solo run's dump; a per-configuration line that lengthens in the wide run while the system time climbs is the width having been bought with paging.
REPLAY_PEAK_BYTES = 1_600_000_000


def kernel_threads_default(*, coresident_bytes: float = 0, total_bytes: int | None = None) -> int:
    """How many seats of the table build's wave — a delta configuration each, one of them taken by `default`'s fold — this box has room for at once beside `default`'s memo: `AMS_KERNEL_THREADS` wherever it is set at all, else `DELTA_PEAK_BYTES` divided into the box by `memory_budget.how_many_fit` with `DEFAULT_MEMO_BYTES` taken off it first — the fresh default snapshot plus its filtered previous seed when present, each holding a hash map and its pools rather than a configuration through enumeration, which is why the two terms are different numbers — after the reserve that policy states, floored at one. A stated width is floored at one and clamped no further here, because the configuration count and the cores this process may actually run on are not memory facts, so this module leaves them to `run_m1._table_build_threads`'s own `min()` and to the cycle's `artifact_cycle.kernel_threads_budget`: this module cannot name `conform.SETTLEMENT_CONFIGS`, since `conform` imports it. A value that is not a bare count — a typo, a `GB` suffix, a variable declared without one — raises rather than falling quietly through to the arithmetic, which is where this knob parts company with `AMS_TOTAL_MEMORY_BYTES`: that one reproduces a box and ignoring a typo in it costs a reproduction, while this one is what someone reaches for to keep a build out of swap, so handing them a width they did not ask for is the failure they set it to prevent. `total_bytes` is a keyword so an assertion about another box is a pure function over an invented one; the alternative is `importlib.reload`, which re-runs module scope, resets `_BUILT`, and drops the live `_SPEC_DUMPS` entries' temporary directories under a caller still holding a `spec_path`.
...
def replay_threads_default(*, coresident_bytes: float = 0, total_bytes: int | None = None) -> int:
    """How many settlement configurations the string replay's one crate process walks at once, as far as memory decides it: `AMS_REPLAY_THREADS` wherever it is set at all, else `REPLAY_PEAK_BYTES` divided into the box by `memory_budget.how_many_fit` after the reserve that policy states, floored at one. The divisor differs in kind from `DELTA_PEAK_BYTES` and the division from `kernel_threads_default`'s: a replay loads one settlement TSV and builds its own engine after the `build-tables` process has exited, so no `default` memo is alive and nothing is taken off the box ahead of the division — one configuration's walk is the whole per-seat cost. What comes back is the memory answer alone; the configuration count and the cores this process may actually run on are not memory facts, so this module leaves them to `run_m1._replay_threads`'s own `min()` and to the cycle's `artifact_cycle.replay_threads_budget`, as `kernel_threads_default` leaves the same two to `run_m1._table_build_threads` and `artifact_cycle.kernel_threads_budget`, and for the same reason: this module cannot name `conform.SETTLEMENT_CONFIGS`, since `conform` imports it. A stated width is floored at one and clamped no further here, and a value that is not a bare count raises rather than falling through to the arithmetic, exactly as `AMS_KERNEL_THREADS` is treated: the knob is what someone reaches for to keep the replay out of swap, and a width they did not ask for is the failure they set it to prevent.

    `coresident_bytes` is for the caller that runs the replay beside something else — the artifact cycle, whose pytest pool is hot across the whole run_m1 step — and `total_bytes` is a keyword so an assertion about another box is a pure function over an invented one. On every fleet box (`doc/fleet.md`) the configuration count binds ahead of the memory answer, with or without the pool subtracted, so the term is insurance for a smaller box rather than a number that moves the width here.
    """
