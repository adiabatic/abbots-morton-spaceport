...
# What the shared default snapshots hold for the delta wave, the co-resident term taken off the box before division by DELTA_PEAK_BYTES. `fanout::run_configs_tables` retains the fresh default snapshot beside the previous default seed when present. Loading rejects keys naming edited runes, reducing retained entries and, at capacity thresholds, their hash buckets; lookup and writing still apply the full read-journal exclusion for edited runes and moved predicate classes. A seeded default stores its misses in the fresh snapshot, so the previous seed still answers useful delta windows absent from it. Default writes its memo inside `enumerate_for_tables` before the wave and occupies one wave seat for its fold, priced by DELTA_PEAK_BYTES; both shared stores remain corpus-sized. With ·Ye in the alphabet, `/usr/bin/time -l` on the 32 GiB box over all five settlement configurations at `--threads=5` with memo writes measures maximum resident memory / peak memory footprint of 12.76 / 15.43 GB without a seed, 13.19 / 17.88 GB with an unfiltered seed and twelve runes invalidated, and 12.22 / 15.49 GB with that seed filtered, the spec unchanged across the comparison. Filtering reduces the previous-default map from 2^25 to 2^23 buckets; at 37 bytes per bucket (twenty-byte key, sixteen-byte entry and control byte), it holds 0.31 GB beside the fresh default map's 1.24 GB, about 1.55 GB together before pools. The previous seed answers 5.2–21.4 million hits per delta; filtering preserves artifacts, digests and hit counts. The 2.5 GB allowance stays conservative across the fleet: savings depend on the edit set and bucket thresholds, these footprint/resident gaps exceed the two-gigabyte admission margin beside DELTA_PEAK_BYTES and cannot justify a smaller bound or wider pool, and the 48 GiB box has no corresponding measurement. Re-measure both terms together under that constant's whole-wave criterion; TABLE_BUILD_PEAK_BYTES bounds the whole process.
