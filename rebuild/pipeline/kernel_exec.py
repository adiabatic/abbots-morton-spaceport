...
# What one configuration holds through enumeration and its memo write, which is both the co-resident term for default's retained trace memo and the divisor bounding each delta in flight: a delta that shares nothing traces the whole configuration. With ·Zoo in the alphabet, default reads 4.25 GB when its worklist drains and 5.23 GB after sorting the memo writer's window list (`--cache-census`'s `resident_before_release` and `resident_after_sort` lines); the latter is the peak this bound must cover. The census's trace_cache length and capacity locate the memo relative to its bucket-table doubling, and each delta's trace_cache length reports how much it traces beyond default's memo. The headroom covers the writer as well as enumeration, even where that narrows the delta wave. `TABLE_BUILD_PEAK_BYTES` is the whole-process reading these bound; re-measure both with the full alphabet whenever it grows or a per-configuration working set changes, using one `build-tables` over every settlement configuration with memo output, `--cache-census`, and `/usr/bin/time -l`.
CONFIG_PEAK_BYTES = 6_000_000_000
# What the one `build-tables` process holds at its peak: default's retained memo beside every delta in flight, each held through its memo write, bounded by CONFIG_PEAK_BYTES times one more than the delta width. The direct whole-alphabet measurement with ·Zoo, four deltas wide and writing memo files without a previous-build seed, peaks at 14.78 GB; the artifact cycle records a 17.09 GB build peak. This estimate keeps headroom above both readings because the configurations' peaks and memo writes need not coincide identically across runs. `make job-costs` watches the cycle's build peak against this figure; it does not divide a pool by it.
