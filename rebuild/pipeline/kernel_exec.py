...
# What one delta holds through enumeration and its memo write, the divisor bounding each delta in flight: a delta that shares nothing with default's memo traces the whole configuration, so it is bounded by a configuration enumerated from scratch, and the seed is the worst delta rather than default. With ·Jay in the alphabet, the trace, prospect and candidates keys packed onto ordinals and the trace entry at sixteen bytes (`rebuild/kernel-rs/src/engine.rs` pins each width), the worst delta enumerated from scratch and alone — `ss03` at `--threads=1`, tracing every window itself, its memo write at the release point and its fold included — peaks at 5.37 GB whole-process under `/usr/bin/time -l` on the 32 GiB box, its `peak memory footprint` line at 5.31 GB beside it, and at 5.50 GB on the 48 GiB box with the footprint line level at 5.48 GB, and that whole-process reading is a seat's cost rather than a seat's plus the process's: what the seats share is the parsed spec and its index, which `spec_parse` reports at 0.0 s and which the process holds in about 0.1 GB before a configuration's engine is built, so the seed rounds the higher of the two boxes' readings up to the tenth. The high-water sits past the three memo tables rather than at them: the `resident_before_release` census line, read with every table built, is 4.36 GB on the 32 GiB box and 4.63 GB on the 48 GiB one, and the process peaks after the memo write instead, in the drain of the transitions, their sort and the fold (`resident_after_sort` 5.07 GB and 4.96 GB, the fold's own working set above it), so a narrower memo key or entry moves this divisor only as far as it moves the drain, the sort and the fold. No live delta traces from scratch — every one runs over default's memo, and its `--cache-census` lines say how much: `memo_base_hits` counts the windows it takes from that memo and its trace_cache length what it traces beyond it; only `config_seed=False` enumerates a live configuration the solo way — which is why one `build-tables` over every settlement configuration on the 32 GiB box, four deltas wide with each delta's memo file written at its release point and no previous-build seed, peaks at 19.24 GB whole-process, DEFAULT_MEMO_BYTES plus about 4.2 GB per delta in flight (4.5 GB seeded over a previous build's memo files, from 20.37 GB). Those wave readings are ones whose `peak memory footprint` line sits within two gigabytes of the resident figure, on either side of it (19.06 GB beside the 19.24 and 19.58 GB beside the 20.37), which is the condition for seeding from a resident figure at all on this box: the resident line under-reads a wave by whatever the compressor has taken from the process, so the same shape of build under forty-byte trace keys on a pass under memory pressure reads 14.91 GB and 15.92 GB four wide against footprints of 21.08 GB and 21.92 GB, and a reading whose footprint sits gigabytes above its resident line is the compressor's rather than the build's. The divisor prices a seat by its bound rather than by what the wave's seats read, and it errs high on purpose: a per-unit cost that errs low is what puts a box into swap, while one that errs high only narrows the wave. The pair is chosen together with DEFAULT_MEMO_BYTES so that both fleet boxes (`doc/fleet.md`) seat the whole delta wave at once — every configuration past default in one round — solo and under a gated cycle with gate:make-test's pytest pool on the box; `rebuild/test_memory_budget.py` and `rebuild/test_artifact_cycle.py` pin that criterion, and the gated arm on the 32 GiB box is what caps this divisor from above: past 5.81 GB the pool's bytes cost that box its fourth seat. Re-measure the two together, never one alone, whenever the alphabet grows or a per-configuration working set changes: a memo term seeded at default's whole resident reading beside a divisor a little under this one lands the 32 GiB box back on a split wave. The recipe is the solo reading above beside the wave measurement, run in both arms — from scratch, and seeded over a previous build's memo files, the arm every artifact-cycle pass after the first runs — since the seeded arm holds a second default snapshot for the wave (DEFAULT_MEMO_BYTES says which): one `build-tables` over every settlement configuration with memo output, `--cache-census`, and `/usr/bin/time -l`, read only where the footprint line sits within two gigabytes of the resident one.
DELTA_PEAK_BYTES = 5_500_000_000
# What the shared default snapshots hold for the delta wave, the co-resident term taken off the box before division by DELTA_PEAK_BYTES. `fanout::run_configs_tables` retains the fresh default snapshot beside the previous default seed when present; loading drops keys naming edited runes, and lookup and writing apply the full read-journal exclusion. `memo::SnapshotEntries` holds thirty-six-byte key/entry records plus four-byte hash-prefix offsets, with pools beside them and no hash-table slack. With ·Ye in the alphabet, default's `--cache-census` entry count prices its immutable records and offsets at 0.90 GB, against 1.24 GB for a hash table at that census's capacity; a seeded build can retain two default-sized snapshots, so one snapshot's saving is not the shared term. On the 32 GiB box, all settlement configurations at four seats with memo writes measure 12.46 GB maximum resident memory / 15.49 GB peak memory footprint cold, and 10.92 / 14.95 GB seeded with ·Ye invalidated, under `/usr/bin/time -l`. The corresponding hash-map runs measure 11.73 / 14.62 GB and 11.98 / 16.39 GB; table, window and uncompressed memo bytes agree in both arms. These footprint/resident gaps exceed the two-gigabyte admission margin beside DELTA_PEAK_BYTES, and the 48 GiB box has no corresponding compact-snapshot reading, so the 2.5 GB allowance remains conservative and buys no wider pool. Re-measure the retained allowance and TABLE_BUILD_PEAK_BYTES together with the delta divisor under its whole-wave criterion; a compact holder alone does not establish a smaller process peak.
