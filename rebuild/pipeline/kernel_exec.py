...
# What one delta holds through enumeration and its memo write, the divisor bounding each delta in flight: a delta that shares nothing with default's memo traces the whole configuration, so it is bounded by a configuration enumerated from scratch, and the seed is the worst delta rather than default. With ·Jay in the alphabet, the worst delta enumerated from scratch and alone — `ss03` at `--threads=1`, tracing every window itself, its memo write at the release point and its fold included — peaks at 5.61 GB whole-process under `/usr/bin/time -l` on the 32 GiB box, and that whole-process reading is a seat's cost rather than a seat's plus the process's: what the seats share is the parsed spec and its index, which `spec_parse` reports at 0.0 s and which the process holds in about 0.1 GB before a configuration's engine is built, so the seed rounds the reading up to the tenth. No live delta traces from scratch — every one runs over default's memo, and its `--cache-census` lines say how much: `memo_base_hits` counts the windows it takes from that memo and its trace_cache length what it traces beyond it; only `config_seed=False` enumerates a live configuration the solo way — which is why one `build-tables` over every settlement configuration on the 32 GiB box, four deltas wide with each delta's memo file written at its release point and no previous-build seed, peaks at 14.91 GB whole-process, DEFAULT_MEMO_BYTES plus about 3.1 GB per delta in flight (3.4 GB seeded over a previous build's memo files, from 15.92 GB). The divisor prices a seat by its bound rather than by what the wave's seats read, and it errs high on purpose: a per-unit cost that errs low is what puts a box into swap, while one that errs high only narrows the wave. The pair is chosen together with DEFAULT_MEMO_BYTES so that both fleet boxes (`doc/fleet.md`) seat the whole delta wave at once — every configuration past default in one round — solo and under a gated cycle with gate:make-test's pytest pool on the box; `rebuild/test_memory_budget.py` and `rebuild/test_artifact_cycle.py` pin that criterion, and the gated arm on the 32 GiB box is what caps this divisor from above: past 5.81 GB the pool's bytes cost that box its fourth seat. Re-measure the two together, never one alone, whenever the alphabet grows or a per-configuration working set changes: a memo term seeded at default's whole resident reading beside a divisor a little under this one lands the 32 GiB box back on a split wave. The recipe is the solo reading above beside the wave measurement, run in both arms — from scratch, and seeded over a previous build's memo files, the arm every artifact-cycle pass after the first runs — since the seeded arm holds a second default snapshot for the wave (DEFAULT_MEMO_BYTES says which): one `build-tables` over every settlement configuration with memo output, `--cache-census`, and `/usr/bin/time -l`.
DELTA_PEAK_BYTES = 5_700_000_000
# What default's retained trace memo holds for the length of the delta wave, the co-resident term taken off the box before the division. default's fold runs at one of the `--threads` seats (`run_configs_tables` in `rebuild/kernel-rs/src/fanout.rs`), so the product it holds is priced as a seat's — bounded by DELTA_PEAK_BYTES, since default enumerated from scratch is exactly what that divisor prices — and the wave is never wider than the width the division answered; on a box whose width exceeds the delta count (the 48 GiB box, where `run_m1._table_build_threads` caps the width at the configuration count) that seat is one the deltas would have left idle, so every delta and default's post phase are resident together there, one working set more than the deltas alone, still inside the seats the division priced. What is co-resident with every seat for the length of the wave is the `MemoSnapshot` alone — one hash map and its pools (`rebuild/kernel-rs/src/memo.rs`), kept at the release point in `rebuild/kernel-rs/src/fixpoint.rs` where every delta's memos go — a memo rather than a configuration held through enumeration, which is why it is smaller in kind than DELTA_PEAK_BYTES and priced separately. No census line reports it directly: `--cache-census`'s `resident_after_release` line for default — sampled once the engine's other memos are freed and before the memo file is written from the snapshot — reads 3.60 GB from scratch while the process still holds default's transitions and seats for the fold, so the snapshot is bounded from above by that reading and from the whole-process peak of a build holding every delta at once (14.91 GB four wide) less what the deltas account for; the `resident_after_memo_write` line after it reads higher (4.31 GB), the writer's freed pages staying resident, and bounds nothing. On a seeded build the term stands for two snapshots, not one: `build_table_files` (`rebuild/kernel-rs/src/fanout.rs`) reads the previous build's default memo whole through `load_seed` and holds it beside the fresh snapshot for the length of the wave, so every artifact-cycle pass after the first carries a second default memo the from-scratch readings never see — default's `resident_after_release` reads 5.38 GB seeded, and the direct measurement four deltas wide reads 15.92 GB seeded against 14.91 GB from scratch. Both seeded readings are taken over a seed whose edit set is empty, the cheapest seeded shape: default carries the whole previous memo whatever moved (`load_seed` keeps every key for it) and re-settles nothing — its own trace_cache reads empty in the census, every window a `memo_base_hits` hit — so they price a whole carried seed beside an empty fresh snapshot, and a pass whose edit re-settles windows holds a fuller fresh snapshot beside the same carried seed; `make job-costs`' kernel-build row, the cycle's own seeded builds after real edits, is the record that watches that arm against `TABLE_BUILD_PEAK_BYTES`. The pair is chosen with DELTA_PEAK_BYTES against the criterion its comment states, both fleet boxes seating the whole delta wave, and the two are re-measured together in both arms, never one alone. `TABLE_BUILD_PEAK_BYTES` is the whole-process reading the two bound.
DEFAULT_MEMO_BYTES = 2_500_000_000
# What the one `build-tables` process holds at its peak: default's retained memo beside every seat in flight — a delta held through its enumeration and the memo write at its release point, or default's own fold at the seat it takes — bounded by DEFAULT_MEMO_BYTES plus DELTA_PEAK_BYTES times the width. The direct whole-alphabet measurement with ·Jay on the 32 GiB box, four deltas wide and writing memo files, peaks at 14.91 GB without a previous-build seed and 15.92 GB seeded, and at five seats — the 48 GiB box's width, every delta beside default's fold — at 16.97 GB and 14.21 GB; `make job-costs`' kernel-build row is the artifact cycle's own record of the build peak. The 32 GiB box's width of four seats three deltas beside default's post phase; the 48 GiB box seats all four deltas beside it, so its own reading is the one to re-seed this figure from, and the five-seat readings above stand in for it until it is taken: the readings on record there, 20.02 GB from scratch and 21.06 GB seeded at five seats (#265's baselines), are from a binary that carried every delta's memo through its drain and its sort, so this figure is known exceeded on that box under that binary and unmeasured under this one, and the five-seat pair on the 32 GiB box — the fresh arm 0.27 GB up and the seeded arm 2.11 GB down against that binary — does not predict which way it reads. This estimate keeps headroom above every reading on the 32 GiB box because the instant the deltas' high-waters coincide moves from run to run — the two arms order differently at the two widths — so one pass reads a few hundred megabytes either way of another. `make job-costs` watches the cycle's build peak against this figure; it does not divide a pool by it.
