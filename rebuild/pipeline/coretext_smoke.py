...
DEFAULT_FONT = REPO_ROOT / "rebuild" / "out" / "m1" / "M1.otf"
DEFAULT_SEQUENCES = PIPELINE_DIR / "smoke_sequences_m1.txt"
SWIFT_SOURCE = PIPELINE_DIR / "coretext_smoke.swift"
BINARY = REPO_ROOT / "rebuild" / "out" / "m1" / "coretext_smoke"
POINT_SIZE = 100.0
...
def compile_binary() -> Path:
    ...
    if not BINARY.exists() or BINARY.stat().st_mtime < SWIFT_SOURCE.stat().st_mtime:
        subprocess.run(["swiftc", "-O", str(SWIFT_SOURCE), "-o", str(BINARY)], check=True)
    return BINARY
...
class HarfBuzzShaper:
    ...
    def has_ink(self, glyph_name: str) -> bool:
        ...
        if cached is None:
            pen = self._bounds_pen(self.glyph_set)
...
def main() -> int:
    ...
    parser.add_argument("--sequences", default=str(DEFAULT_SEQUENCES))
    ...
    summary_path = args.font.parent / "coretext_summary.json"
