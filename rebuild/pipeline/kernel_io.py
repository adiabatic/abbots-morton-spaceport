...
The transition stream is the return leg, and it is the real Rust↔Python contract in a way a windows file cannot be: `table.write_windows` records the label view of each row alongside rules that were already folded, and nothing reading it back can re-derive those rules, because `_rules_for_input` reads per-transition provenance and joints while the treaty fold reads the settled cell, seam, and extension the TSV drops. What the stream carries is the whole `FixpointProduct` at exactly the grain the fixpoint left it in: rows in the product's own key order, deep right slots still at class grain, `joint` still the trace's `joint_floor` with the prospect-divergence pass unrun, plus the deep-class map those class tokens resolve through, the provenance the engine fired, and the reachable cells stated at that same class grain. Feed one back through `table.assemble_tables` and the tables are the ones the enumeration that wrote it would have folded.

Its layout follows the artifact idiom next door: gzip with a zeroed stamp, a `# ams-m1-transitions/1\\t<head json>` first line, then one compact JSON array per transition. A cell is spelled once in the head and referenced from the rows by its index there, sorted the way `table._cell_key` sorts the windows file's cells — and every row's settled cell, plus every non-None left-settled one, must be among them, checked while writing so a kernel that invented a cell fails at the boundary rather than deep inside the fold. Order is load-bearing on this side too: the rows keep the key order `assemble_tables` expands and flags in, and a row's provenance keeps the first-seen order the rule fold joins pointers in.
"""
