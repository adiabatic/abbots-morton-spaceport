export const STATE_KEYS = ['class', 'batch', 'unit', 'group', 'config', 'family', 'status', 'machine', 'units', 'docket', 'view'];
...
export function shedWorklist(patch) {
  for (const key of WORKLIST_EXIT_KEYS) {
    if (key in patch) return { units: null, docket: null, view: null, ...patch };
  }
  return patch;
}
