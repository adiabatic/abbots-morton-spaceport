...
# The windows the review surface's worked examples name, and the reason the bundle is filtered on two rules rather than one. Every test that reaches for one of these already spells its codepoints out — the three that used to ask for "some unit of class X" are pinned here instead — so the set is a list of exemplars rather than a sample, and a member selecting nothing means the exemplar dissolved rather than that the filter drifted.
