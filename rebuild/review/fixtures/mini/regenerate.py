...
What it holds: `audit.tsv`, the live divergence audit filtered to windows drawn from the four letters below plus the boundary tokens; `baseline-*.subset.tsv.gz`, each live subset table sliced to those same windows; `M1.otf`, a frozen copy of the after-font the slices were extracted against; and the default settlement and treaty tables, which `rebuild/test_review_tablediff.py` and the table-diff build test want as a directory of real tables beside a real font rather than as anything about today's rules. The three move together, and only together — a slice from one build beside a font from another would have the enricher reporting glyph disagreements that are the bundle's fault rather than the code's.

The one thing the bundle cannot freeze is the spec: `build_m1` loads it from `repo_root`, so the settlement the enricher re-derives is always today's. A rune edit that changes how any of these windows settles therefore leaves the frozen `new` cells describing a rebuild that no longer happens, and the mismatch that follows is the bundle's cue to be regenerated, not a defect in the code under test.
...
def main() -> int:
    ...
    print(f"{len(kept)} audit rows over {len(windows)} windows", flush=True)
    return 0
