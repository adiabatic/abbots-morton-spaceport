...
- Each unit's `content_key`: a byte-identity contract with every verdict on record, since the unit's id derives from it, checked by `rebuild/test_carry_verdicts.py`, with `carry_content_hash` in `rebuild/review/unit_cache.py` as the authority that computes it. `picture_identical` is a carry-presentation key (`CARRY_PRESENTATION_KEYS` there), so flipping it on a unit never moves its stamp.
...
`pin._preserve_authored_outgoing` adapts a pinned schema without outgoing inheritance by declaring every frozen ligature stance's complete local contract as an explicit exception. It adds only that exception form to the pinned schema and takes no rules from the working tree. A pin whose schema supports `outgoing` needs no adaptation. This keeps the frozen font and tables paired with their authored behavior under the current loader.
