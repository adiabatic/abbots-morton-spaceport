...
- `check_manifest`, `check_unit`, and `check_shards` in `rebuild/review/build.py` — the §7 contract checker, run over this directory by `rebuild/test_review_build.py` and `rebuild/test_surface_checks.py`, and the same checker that gates a real build: `check_unit` over the units that build computed, a cache-served unit held instead by the `content_key` stamp its shard and its store record must agree on, and the cross-unit predicates over both.
