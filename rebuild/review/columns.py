...
class MappingPool:
    """One pool for every per-config class map a unit carries (`audit.Unit.config_classes`): ids in first-seen order, the empty mapping at id 0, keyed on `tuple(mapping.items())` in the mapping's own insertion order and never sorted, because that order — the order the audit states a unit's configs in — is in the shipped fragment's bytes while the content key hashes order-blind, so a pool that sorted would move `units/*.json` without moving an id. `pooled` hands back the one instance the pool holds for a value, typed `Mapping[str, str]` so that a write through it is a type error at the line that makes it rather than a value every unit sharing the instance sees; `rebuild/test_unit_cache.py` holds that no build, cold or served, writes through a pooled instance. A mapping whose keys or values are not strings is refused when it is first seen, since an id column could not name them. `elements` is twice the distinct mappings' entries summed — a key id and a value id an entry — and `len` their number, the empty one excluded, so `pile_tally.pool_bytes` prices the pool as it prices a tuple pool."""
