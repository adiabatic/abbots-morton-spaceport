...
def signature_code_paths(repo_root: Path) -> list[Path]:
    ...
    Three inputs sit outside the roster on argument rather than hash. The codepoints-to-text derivation, `build.signature_text`, has two halves: `audit.parse_codepoints` and `audit.format_codepoints`, whose round trip rebuild/test_review_audit.py::test_parse_codepoints pins and whose drift `build.ink_sig` turns into a KeyError on its first lookup, and the `chr` join between a parsed window and the text the comparator shapes. That join is the definition of what text a window's signature is taken over, so an edit to it is a change to what a signature means, and like this module's own `signature_key` schema and the store's framing it is expressed through `SIGNATURE_STORE_FORMAT` rather than the roster. A key-schema move cannot serve a wrong digest on its own, since the key is a content hash and every row re-keys into a miss; a join edit under an unbumped format would, which is why rebuild/test_review_audit.py::test_the_signature_text_is_pinned_beside_the_store_format holds `build.signature_text`'s answers and the format as one literal pair, red on a join edit that changes a text until the format moves with it. The uharfbuzz and fontTools versions ride no stamp, a gap shared with the unit store.
    """
    root = Path(repo_root)
