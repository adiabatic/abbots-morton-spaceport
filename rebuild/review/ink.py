...
VERIFICATION_METHOD = (
    "Shaped with uharfbuzz in both shipped fonts under every config in the unit's set; "
    "outlines decomposed with fontTools DecomposingRecordingPen, translated by the cumulative "
...
class InkComparator:
    ...
    def ink_pieces(self, side: str, text: str, features: dict[str, bool]) -> tuple:
        """The placed outlines of one shaped run, sorted: one piece per glyph that carries ink, translated to its pen position. Inkless glyphs (space, ZWNJ, empty markers) contribute no piece."""
        shaper = self._sides[side][0]
        result = shaper.shape(text, features or None)
        pieces = []
        pen_x = 0
