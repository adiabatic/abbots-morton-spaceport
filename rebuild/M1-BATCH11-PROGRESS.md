...
The after-·See forms are the runes' first live `bind:` contracts, paired with the first live `contract:` (qsSee's by-1, widened to name qsOut_qsTea): each stance carries a `diagonal-leg` redraw — the raked leg that interlocks with ·See's contracted tail — swapped in by a bind contract keyed on `left: qsSee`. The bind bitmap carries its own geometry, so the exit anchor rides it implicitly and no See-scoped exit extends exist; qsFee's ordinary +1 composes on top for ·See·Out·Fee. The word-final arch redraw additionally trims its y0 pixel (`trim: 1`), matching the old `en-trim-1`; the seam row of the interlock is deliberately empty on both sides, which is why the blessing block in `rebuild/m1-contact-allow.yaml` covers anchor, seam-gap, and overlap spellings.

The ligature is a single `hapax` stance, entryless nowhere but exitless everywhere: the old font compiles no exit-bearing variant, so followers always arrive unjoined, and its own `diagonal-leg` bind (untrimmed, unlike the letter's) handles the after-·See form.
