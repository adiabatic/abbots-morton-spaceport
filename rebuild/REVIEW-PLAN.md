...
The executable authority is `check_manifest` / `check_unit` / `check_output_dir` in `rebuild/review/build.py`: the build runs them over its own output and over `rebuild/review/fixtures/` and fails on any violation, so they — not this section — are what a change has to satisfy. What follows orients a reader to the shape of the JSON and to the design decisions its fields carry; it is deliberately not exhaustive, and where it and the checkers disagree the checkers are right. The JSON is the only interface between the engine (`rebuild/review/*.py`) and the frontend (`static/` + `jstests/`); `rebuild/review/fixtures/` lets the frontend build against the contract without running a generation.
