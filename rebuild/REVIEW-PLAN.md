...
- `test_review_audit.py`: TSV/ledger loading on a fixture audit; dedupe counts against the checked-in census pins (`rebuild/review-census-pins.json`, regenerated by `uv run python -m rebuild.review.census --update`); every ledger exemplar resolves to a unit; ordering is deterministic (byte-identical shards across two builds).
