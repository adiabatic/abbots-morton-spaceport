...
class TestTheWidthsAlreadyOnRecord:
    ...
    def test_the_shipped_pair_seats_the_whole_delta_wave_on_both_fleet_boxes(self, total: int, monkeypatch):
        """The criterion the two constants are chosen against, as an assertion rather than a comment that a re-seed can drop: on both fleet machines the solo width covers every configuration past `default`, so the wave runs in one round and the build never waits out a trailing round holding a single delta. A re-seed that costs the 32 GiB box its fourth delta fails here instead of quietly lengthening every table build. The override is cleared first, because a shell that exported a width of its own would satisfy the inequality whatever the constants say."""
        from rebuild.pipeline.conform import SETTLEMENT_CONFIGS
