...
15,528 rows in 2,411 units across 14 nonzero classes (the 15th, `kern-channel-out-of-scope`, is genuinely count-0). Of those, **1,549 units (10,666 rows) are machine-approved as ink-identical** and need no eyes, leaving **862 human units (4,862 rows) in 3 batches**. The three name-grain classes that used to be the bulk-approval slog now show only their visible stragglers — a direct consequence of the ink census: `dangling-anchor-dropped` is down from 1,334 units to 116, `zwnj-word-initial-unification` from 213 to 7, `bare-name-live-join` from 139 to 14, and those 137 stragglers genuinely render differently and need human eyes (the per-unit machine verdict is what protects against blind class-level `g`-approval). Suggested order: start with the small, individually judgeable classes to calibrate.

| Order | Class                               | Status         | Human units | Human rows | Machine-approved |
| ----- | ----------------------------------- | -------------- | ----------- | ---------- | ---------------- |
| 1     | `may-quad-order-deferral`           | drift-accepted | 1           | 7          | 0                |
| 2     | `zwnj-follower-exit-restored`       | drift-accepted | 6           | 30         | 0                |
| 3     | `pre-ligature-cleanup-regularized`  | drift-accepted | 6           | 38         | 0                |
| 4     | `zwnj-word-initial-unification`     | intended       | 7           | 49         | 206              |
| 5     | `bare-name-live-join`               | intended       | 14          | 94         | 125              |
| 6     | `pea-chain-regularized`             | drift-accepted | 15          | 101        | 0                |
| 7     | `same-seam-extension-non-summing`   | intended       | 33          | 225        | 0                |
| 8     | `ss03-chain-join-gains`             | drift-accepted | 40          | 120        | 0                |
| 9     | `entered-it-baseline-join-gain`     | drift-accepted | 46          | 319        | 0                |
| 10    | `marker-staging-ligature-formation` | intended       | 52          | 187        | 0                |
| 11    | `regrouping-floor-drift`            | drift-accepted | 64          | 435        | 0                |
| 12    | `dangling-anchor-dropped`           | intended       | 116         | 812        | 1,218            |
...
Classes 1–11 total 284 units — careful, one-at-a-time judgment. Classes 12–14 (578 units) keep `g` (group-approve) useful within family-pair groups, but note that the dangling/zwnj/bare-name remnants are exactly the units the machine could **not** vouch for: every one of them renders differently in the two fonts.

### Machine-approved units

`rebuild/review/ink.py` is the build-time gate: a unit is `ink_identical` iff shaping its text in both shipped fonts under **every** config in its set produces identical placed outlines (uharfbuzz shaping via the validation `Shaper`; per-glyph `DecomposingRecordingPen` outlines translated by the cumulative `x_advance` plus `x_offset`/`y_offset`; pieces sorted and compared). These units get `batch: null`, are excluded from batches, sidebar counts, and progress denominators, and surface in the app only behind the "Show machine-approved" toggle, badged and verdict-disabled. The manifest records the totals (`machine_approved`: 1,549 units / 10,666 rows, the method one-liner, per-class counts), the export CLI reports them as a `machine_approved` section in the triage YAML (counts, per-class counts, method, compact unit-id ranges — never as drafted pins), and `verdicts.json` stays human-verdicts-only. The numbers are pinned by tests and must reproduce exactly on a rebuild; in table-diff mode the same comparison runs over each entry's witness string, and a witnessless entry stays in the human workload because there is nothing to shape.
