...
| 27  | `policy.contract`               | narrow a connector (qsPea has none)                               | done R24               |
| 28  | `policy.resolve`                | hand-authored ambiguity (none anywhere yet)                       | done R25               |
| 29  | `policy.groups`                 | rune-local predicate classes (qsPea has none)                     | done R26               |
...
| P2  | `when.word`                                                           | run position; space/ZWNJ split, the namer-dot doesn't                 | done R32 |
| P3  | `when.feature`                                                        | active `ssNN` gate                                                    | code     |
| P4  | `leftCondition` (container)                                           | settled-left axes; points up to `when`                                | done R29 |
| P4b | condition leaves: `family`/`class`/`stance`/`joined_at`/`stroke`/`is` | shared selector axes; per-key hovers, or let the container carry them | done R30 |
| P5  | `rightCondition.then`                                                 | one-level lookahead; allowed on prefer, banned on refuse              | done R31 |
| P6  | `*Condition.except`                                                   | static carve-outs                                                     | done R30 |
...
- **R29 — `leftCondition` container.** The settled-left side, kept procedural per D4 with a one-clause pointer up to `when` for the symmetry. Owner took the two-paragraph axes-plus-pointer draft but dropped the worked example (the container glosses each axis, and `from`/R10 already carries a left-side example). Lists all seven axes inline (`family` / `class` / `stance` / `joined_at` / `stroke` / `is` / `except`), with the settled-only reason for `stance` / `joined_at` in its own paragraph. Leaf-key granularity (one hover each vs. let the container carry them) is the next open call — see walk row P4b. → schema (`leftCondition`).
- **R30 — condition leaves + shared scalars (resolves q21).** Walk row P4b's granularity call landed on **a terse hover for every condition leaf** — hovering any of `family`/`class`/`stance`/`joined_at`/`stroke`/`is`/`except` always shows something, rather than leaning on the container. Wiring split: **shared scalars documented once on their `$def`** so the text rides every condition that refs them — `family` → `familyOrList` ("A single family … Matches if any family matches.", owner verbatim), `stroke` → `strokeOrientation` ("A stroke orientation: `horizontal`, `vertical`, or `diagonal`. Matches when the neighbor presents this stroke at the seam."), `is` → `boundaryValue` (the `boundary` catch-all spelled as the three named tokens plus the text's start/end "where there's no glyph at all" — workshopped against the owner's "what's a run's edge?" until it read plain); the entry/exit `stroke` properties keep their own descriptions and override the `$def`. **Context-specific leaves written per-condition** as a sibling of the `$ref` — `class` (same text on all four condition objects), `joined_at` and `stance` (leftCondition only, left-settled). `stance` took the terser point-up-to-`when` form rather than the `joined_at`-parallel spell-out. `except` (all three condition arrays) **resolves q21** with the subtraction model ("Drops neighbors from the match. A neighbor listed here is skipped even when the condition's other keys fit."), the verb workshopped Removes→Drops and the colon→period at the owner's call, closing on a real stands-alone-negation example (`left: {except: [family: qsIt]}`, corrected from the approved `[qsIt]` to the schema-valid `[family: qsIt]` form). The `family`/`class`/`joined_at` leaves were committed earlier by the owner as `Improve schema` (7c81093); `stance`/`stroke`/`is`/`except` this session. → schema (`familyOrList`, `strokeOrientation`, `boundaryValue`, condition `class`/`stance`/`joined_at`/`except`).

- **R31 — `rightCondition.then` (resolves q20).** The one-position lookahead, hung on `then` as a sibling of its `$ref`. q20's use-case-first lean held: owner picked the use-case opener ("Lets one join's decision account for the join just beyond it: it looks past the immediate right neighbor to the letter after that") over the spatial-mechanics one. The restriction paragraph (why `then` rides `prefer` but not `refuse`/`require`) took **five rounds** — a first pass landed the "Plain" taste-layer framing, but the owner reopened it ("I never made a good decision, just gave guidance"), so it was re-iterated from scratch: rejected "afford," rejected the term-forward `window-decidable`/`load error` variant, picked the **"why it's harmless"** angle (`prefer` only breaks ties among surviving stances, so a wider look can't break anything), then flagged that **"seam" was unexplained** — so the final clause drops "seam" for "`refuse` and `require` are vetoes that judge only the join in front of them, never the letter beyond." Two earlier polish steers also held: backticks on every code token, and a clarification that a **backticked** lowercase identifier at a sentence start is fine (so the "Both …"/"A …" restructurings drafted to dodge it were unneeded). Body adds an axes note (only the raw right axes `family`/`class`/`stroke`/`is`/`except`, and `then` can't nest — verified against `rightConditionNoThen`, which has no `then`) and the real ·X·It·It·May example (qsIt.yaml:93). Two commits: the Plain first pass, then the re-iterated restriction paragraph. → schema (`rightCondition.then`).
- **R32 — `when.word` (resolves q23).** Word position, hung on `word` as a sibling of its inline `enum` (description first, then `enum`). q23's surface-it lean held. Owner picked the three-paragraph variant over a terse one-liner: opener names the four values, the middle paragraph derives each from run breaks, and the third paragraph carries the q23 point — the namer dot does **not** break a word, so a letter beside one stays `medial` on that side, with the reminder that `is: namer-dot` still matches it as a neighbor. Verified against settle.py `word_position` / `SPLITTING_KINDS = (edge, space, zwnj)` — the namer-dot is deliberately excluded. Reuses R30's `boundaryValue` vocabulary (`space`/`zwnj`/`namer-dot`, "the start or end of the text"), honoring the subtlety that `boundary` for `is`-matching includes the namer-dot even though word-splitting doesn't. Edited `when.word` only, not the identical `whenWindowDecidable.word` (P9 will mirror it). → schema (`when.word`).

### Source-order pass over real YAML

From here the walk follows the owner's source-order rule (set 2026-06-19): document the next **bare** field that actually appears in a live rune YAML, in document order across any of the files, rather than picking by the abstract Phase-2 row codes. The Phase-2 table above still maps the territory; this pass just reorders which leaf comes next, and biases toward leaves the owner will actually hover while authoring. Root document order is `rune` → `codepoint`/`sequence` → `ductus` → `notes` → `mono` → `stances` → `policy`; the first four were already done, so `notes` was next.

- **R33 — root `notes`.** The first bare root field. Owner picked the terse two-sentence variant over a contrast-plus-example one: it mirrors the existing `ductus` hover's "prose … never read by the build" framing and contrasts on content (join constraints + design rationale, not how-to-draw — the CLAUDE.md ductus/notes split). Only real occurrence is qsTea_qsOy.yaml:6 ("Entryless by declaration…"). The field-inventory `$.notes` row (Region 1) already carried this gloss; this just lands it in the schema. → schema (`notes`).
- **R34 — root `stances`.** Next bare root field after `mono` (which is done and already hands off here: "the proportional, contextual shapes live in `stances`"). Owner picked the fuller variant over a terser one: it says each entry is a stance (a drawing plus its join `surface`), draws the `mono` contrast, and adds that at least one is required and that policy records refer back to a stance by this name. The `stance` `$def` it maps to is still bare (its leaves `way`/`traits`/`bitmap`/`bitmaps` are documented; `y_offset` and the container are next). → schema (`stances`).
- **R35 — `stance.y_offset`.** The mirror of the already-documented `drawing.y_offset` (which `mono` and named `bitmaps` use). Owner's steer (a note, not an option pick): mirror the sibling wording and add that it's the same as the other `y_offset`. Landed the full gloss scoped to the stance plus "— the same as `mono`'s `y_offset`," then dropped the `qsMay … -3` example at the owner's follow-up (D3's simple-scalar exception). Real occurrences: qsDay.yaml:34, qsMay.yaml:34. → schema (`stance.y_offset`).
- **R36 — `entryRow.joined`.** The first bare leaf reached descending into a stance's `surface` (its children `entries`/`exits`/`pairings`/`cells`/`unlocks`/`require` are all documented; so are the entry/exit `x`/`stroke`/`from`/`toward`/`withdrawal`/`ink_y`/`selectable`/`x_off_convention`). `joined` names the drawing to swap in when a predecessor actually joins at this entry — the entry-side mirror of the exit's `withdrawal`. Took four rounds: owner picked the fuller+mirror structure, then flagged two scope errors to fix — **(1)** drop "sibling bitmap" (it implies adjacency; the `bitmaps:` block sits apart from `entries:`), **(2)** pin whether `mono` is reachable. Verified against spec_load.py `_lint_stances` (626-629): `joined`/`withdrawal`/`cells.bitmap` must all resolve to the stance's **own** `bitmaps` map — the loader errors otherwise — so the base `bitmap`, the rune's `mono`, and other stances' bitmaps are all unreachable. Final wording says exactly that, then a `\n\n` break before the withdrawal-mirror paragraph (owner's last edit). Real occurrences: qsMay.yaml:66, qsOy.yaml:39, qsUtter.yaml:82. → schema (`entryRow.joined`).
  - **Consistency follow-up (`withdrawal` resolved in R38; `bitmaps` still open):** the already-documented `exitRow.withdrawal` hover said "name a sibling bitmap," and the `bitmaps` `$def` hover says the names are "wired up elsewhere" — both predate the R36 "not a sibling, scope is the stance's own `bitmaps`, `mono` unreachable" decision. `withdrawal` is fixed (R38); revisit the `bitmaps` `$def` for the same scope precision when the walk reaches it.
- **R37 — `entryRow.joined_x`.** The anchor-x override that travels with `joined` (model.py:84). Four phrasing rounds, all owner notes: rejected "when the `joined` drawing is in play" (jargon), adopted the owner's "the bitmap specified by `joined`" framing, kept `bitmap` un-backticked (it collides with the literal `bitmap` field), and cut an undefined "the base" — the final wording defines what it contrasts against instead: "the row's `x` marks the leftmost ink of the stance's default drawing; when the joined bitmap's leftmost ink lands in a different column, `joined_x` is that column instead." Example uses ·May's grounded loop (`x: 3` → `joined_x: 2` under `joined: pulled-back-grounded`). Real occurrence: qsMay.yaml:117. → schema (`entryRow.joined_x`). With this, all of `entryRow` is documented.
- **R38 — `exitRow.withdrawal` rework (resolves the R36 `withdrawal` follow-up).** Not a bare field — a consistency rework chosen by the owner before moving on, since `withdrawal` is the exit-side mirror of `joined` and predated R36's scope decision. Six rounds, all owner notes, each fixing a real confusion: **(1)** dropped "name a sibling bitmap" for "one of this stance's own `bitmaps`" + the R36 scope sentence ("As with `joined`, only those names resolve: not the base `bitmap`, and not the rune's `mono`"); **(2)** purged the noun "drawing" (owner: it reads like an undefined term — true, the `drawing` `$def` is just `{bitmap, y_offset}` and prose uses it loosely) in favor of plain verbs + `bitmap`; **(3)** rewrote "the successor declines here, or there's no successor at all" (owner: a present follower doesn't vanish) to "either the next letter doesn't join back, or this letter ends the word"; **(4)** cut the ·May-specific "pulled-back shape that retracts the reaching stroke" (collided with ·May's literal `pulled-back` bitmap name in the example) for a general appositive; **(5)** purged "reads right" (owner: undefined) for the concrete "leaves no connector ink reaching at this row," reusing the `safe` clause's own phrasing. Real occurrences: qsPea.yaml (baseline `safe`), qsMay.yaml:79 (`withdrawal: pulled-back`). → schema (`exitRow.withdrawal`).
  - **Follow-up — the noun "drawing" (resolved in R39):** R38 banished it from `withdrawal`; R39 banished it from every hover.
- **R39 — banish the noun "drawing" (cross-cutting sweep).** Owner decision (spun out of the R38 "drawing" follow-up): the word reads like an undefined term, so remove it from all hover text in favor of `bitmap` / plain verbs. Owner picked "banish everywhere," then approved extending past the four hovers I first scoped (I'd under-counted). Net: 11 description edits across `rune` ("base drawing"→"base bitmap"), `mono` ("reference drawing"→"reference `bitmap`", restructured so `y_offset` rides along), `stances`, `drawing.y_offset`, `stance.y_offset`, `bitmaps` ("Named alternate drawings"→"…bitmaps"), `cells` (two spots), `joined`, `joined_x`, and `stub.inks_when`. Verbs ("redraws", "draws", "redraw") and the structural `drawing` `$def` name + its `$ref`s are left untouched — the ban is on the prose noun only. Verified zero `\bdrawing\b` nouns survive in any `description`. → schema (10 hovers).
