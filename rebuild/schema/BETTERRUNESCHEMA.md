...
- **R38 — `exitRow.withdrawal` rework (resolves the R36 `withdrawal` follow-up).** Not a bare field — a consistency rework chosen by the owner before moving on, since `withdrawal` is the exit-side mirror of `joined` and predated R36's scope decision. Six rounds, all owner notes, each fixing a real confusion: **(1)** dropped "name a sibling bitmap" for "one of this stance's own `bitmaps`" + the R36 scope sentence ("As with `joined`, only those names resolve: not the base `bitmap`, and not the rune's `mono`"); **(2)** purged the noun "drawing" (owner: it reads like an undefined term — true, the `drawing` `$def` is just `{bitmap, y_offset}` and prose uses it loosely) in favor of plain verbs + `bitmap`; **(3)** rewrote "the successor declines here, or there's no successor at all" (owner: a present follower doesn't vanish) to "either the next letter doesn't join back, or this letter ends the word"; **(4)** cut the ·May-specific "pulled-back shape that retracts the reaching stroke" (collided with ·May's literal `pulled-back` bitmap name in the example) for a general appositive; **(5)** purged "reads right" (owner: undefined) for the concrete "leaves no connector ink reaching at this row," reusing the `safe` clause's own phrasing. Real occurrences: qsPea.yaml (baseline `safe`), qsMay.yaml:79 (`withdrawal: pulled-back`). → schema (`exitRow.withdrawal`).
  - **New follow-up — the noun "drawing":** R38 banished it from `withdrawal`, but it still opens the R36 `joined` hover ("The drawing to use…") and appears in `mono` ("reference drawing"), `stances` ("a drawing plus its join `surface`"), and `bitmaps` ("Named alternate drawings"). Owner hasn't decided whether to banish it globally; if so, harmonize those four. Logged, not yet actioned.
