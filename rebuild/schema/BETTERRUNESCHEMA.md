...
- **R49 — the `unlock` leaves.** The five keys inside an `unlocks:` row (`feature`/`entry`/`exit`/`pairing`/`when`) took the terse set — one sentence each, two for `when` — since the R16 container hover two lines up keeps the teaching and the worked ·It example. Each grant leaf restates the exactly-one-of rule so it stands alone (D2); `pairing` names the re-admit relationship to `pairings`; `when` points at the usual settled-left/raw-right rules. → schema (`unlock.feature`, `.entry`, `.exit`, `.pairing`, `.when`).

- **R50 — the refuse targeting leaves + a dejargonized record hover (first D6 drafts-file round).** The three targeting keys got one-line hovers ("Limits the refusal to this stance" / "…to joins that enter (exit) at this height"); the owner cut the drafted "Omit it…" tails, moving the omit-behavior up to the record hover where it belongs. Folded in at the owner's yes: the R21 record hover reworked to drop the courtroom register the owner flagged — "veto", "strikes … out of contention", and "kills every joining candidate" are gone; it now leads "Refuses a join: when its `when` matches, the join this record names is off the table, and the build joins the letters some other way or not at all", in the D5 shape, with the ·It·Day example unchanged. "Refusal" over "veto" everywhere — it echoes the key's own name. → schema (`refuseRecord`, `.stance`, `.entry`, `.exit`).
