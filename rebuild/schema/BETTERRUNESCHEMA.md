...
- **Stale-example judgment calls (R40's second half).** Replacement examples needed for `surface.cells`, `exitRow.withdrawal`, `exitRow.toward`, `rightCondition.then`, and `contractRecord` (its `bind` example); plus the minor `when`/`from` quote drift. One question each. _Lean: qsUtter's `baseline-withdrawn` cell for `cells` (it demonstrates `-withdrawn`, which is why the old example was chosen); qsUtter's ·Tea `then` prefer for `then`; state plainly on `withdrawal`/`contract` that the named-bitmap / `bind` forms currently have no live user._
