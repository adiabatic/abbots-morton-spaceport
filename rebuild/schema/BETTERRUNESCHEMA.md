...
- **R31 — `rightCondition.then` (resolves q20).** The one-position lookahead, hung on `then` as a sibling of its `$ref`. q20's use-case-first lean held: owner picked the use-case opener ("Lets one join's decision account for the join just beyond it: it looks past the immediate right neighbor to the letter after that") over the spatial-mechanics one, then the **Plain** restriction framing (`prefer` is the taste layer that can afford to wait; `refuse`/`require` decide from a fixed window at the seam) over the term-forward `window-decidable`/`load error` variant — keeping the q20 "mechanics as a note" steer. Two polish steers on the way: backticks on every code token, and a clarification that a **backticked** lowercase identifier at a sentence start is fine (so the "Both …"/"A …" restructurings I drafted to dodge it were unneeded — original "`refuse` and `require` ban it" stands). Body adds an axes note (only the raw right axes `family`/`class`/`stroke`/`is`/`except`, and `then` can't nest — verified against `rightConditionNoThen`, which has no `then`) and the real ·X·It·It·May example (qsIt.yaml:93). → schema (`rightCondition.then`).
