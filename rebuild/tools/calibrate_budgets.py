...
UNITS: tuple[Unit, ...] = (
    ...
    Unit(
        ...
        pool_units=(),
        step_names=("run_m1",),
        step_caveat="run_m1's peak is the widest single process in its tree, and that is the one build-tables child holding every settlement configuration — default's retained memo beside every seat in flight, default's own memo write and fold at one of them — so the step peak reads the whole table build at whatever width the cycle handed it, never one configuration. DELTA_PEAK_BYTES, the per-delta figure the width is divided out of, and DEFAULT_MEMO_BYTES, the memo taken off the box first, are not measured by any step here: their reading is the direct whole-wave measurement under --cache-census, and the bound they state is that this unit's peak stays under the memo plus one delta per seat of the width.",
