...
UNITS: tuple[Unit, ...] = (
    ...
    Unit(
        ...
        step_names=(),
        step_caveat="",
        note="This lane's width is the cores this process may actually run on, and nothing divides the box by a per-worker cost to reach it, so there is nothing here to calibrate. The observations are collected and reported anyway, so that if the lane ever grows a memory-derived width the figure to seed it with is already on the record rather than a measurement someone still has to go and take.",
    ),
    Unit(
        name="rebuild-validators",
        constant="VALIDATORS_WORKER_BYTES",
        source=VALIDATORS_SOURCE,
        pool_units=("rebuild-validators",),
        step_names=("gate:rebuild-validators",),
        step_caveat="reap_peak_rss_bytes maxes over the child's whole tree rather than summing it, and that tree is one pytest controller plus its workers, so the figure is the widest single worker — an honest reading of this unit, and the only history the journal holds from before pool records existed. The one impurity worth naming: under `make test-rebuild` the pyright child rides into whichever lane spawns first, which is normally contracts, so a validators step peak is normally clean.",
        note="HEAVIEST_WORKER_BYTES in the root conftest.py is held equal to this constant by test_the_root_fallback_prices_a_worker_the_way_the_validators_lane_does in rebuild/test_memory_budget.py, so it is calibrated by this row and is never checked independently — re-seeding one means moving both in the same commit, which that test already enforces.",
    ),
    Unit(
        name="kernel-config",
...
def _width_clause(unit: Unit, constant_bytes: int, *, total_bytes: int, cores: int, root: Path) -> str:
    ...
    if unit.name == "rebuild-validators":
        cap = min(_int_constant(root / VALIDATORS_SOURCE, VALIDATORS_CAP_NAME), cores)
        return memory_budget.describe_fit(constant_bytes, cap=cap, total_bytes=total_bytes)
