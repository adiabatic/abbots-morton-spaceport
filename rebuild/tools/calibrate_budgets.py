...
UNITS: tuple[Unit, ...] = (
    ...
    Unit(
        ...
        pool_units=(),
        step_names=("run_m1",),
        step_caveat="run_m1's peak is the widest single process in its tree, the max over its children, and on both fleet boxes that is the one build-tables child holding every settlement configuration — default's retained memo beside every seat in flight, default's own memo write and fold at one of them — so the step peak reads the whole table build at whatever width the cycle handed it, never one configuration. The other candidate is the string replay's child, which seats every settlement configuration in one wave (REPLAY_PEAK_BYTES apiece, `--replay-threads`) and peaks at about four fifths of the build on the shipped alphabet; should a replay ever outrun the build, this row reads the replay, and the figure to re-seed is then that constant rather than this one. DELTA_PEAK_BYTES, the per-delta figure the width is divided out of, and DEFAULT_MEMO_BYTES, the memo taken off the box first, are not measured by any step here: their reading is the direct whole-wave measurement under --cache-census, and the bound they state is that this unit's peak stays under the memo plus one delta per seat of the width.",
    ...
    Unit(
        ...
        note="A row whose constant is subtracted from the box rather than divided into it: the parent's pile moves with the width only through the one batch reply in flight per worker, so it is surface_job_budget's co-resident term. Phase 2 streams into the shards, so the pile is the workload and the projections rather than every fragment, but it is still corpus-shaped — every migrated letter moves it — so expect to re-seed it per batch. The seed sits above the pool-live plateau a full build reaches from its units phase through its cache write; a served pass, where the parent alone carries the store records after the pool has closed, peaks under that plateau by the better part of a gigabyte on both corpora measured (the qsJay corpus reads 8.44 GB served against 9.39 GB full with the ink-signature records held through the cache phase, and 8.75 GB full with them released at the store write; the qsCheer corpus 7.60 against 8.49), so a full build's row is the one to read when re-seeding, and a served pass's row underneath it is not the constant reading high.",
    ),
    Unit(
        name="surface-worker",
