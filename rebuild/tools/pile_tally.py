"""A debug tally of the large collections (piles) a surface build holds, read at its phase boundaries, so that a peak in the timings journal can be attributed to the pile that caused it. It is off unless the environment sets `AMS_SURFACE_PILE_TALLY=1` (`TALLY_ENV`). The artifact cycle never sets it; the build child inherits it from the caller's environment. With it unset, `from_environment` returns None, the build skips its `if tally:` branches, and the shards and the build's other output are unchanged.
...
A pile registered with `hold` is re-estimated at every later boundary, because the build mutates and drains what it holds. A pile the build has emptied reads as empty at later boundaries and stays in the output. `hold_reading` registers a callable instead, returning `(count, bytes)` or a `Measure`. It serves a pile that keeps its own exact figures (`ink.shape_memo_census`), a pile stored as packed columns (`column_census`, as used by `UnitStore.census` and `build.row_columns_census`), and a pile the build holds only through another object. `build_m1`, `_FreshRunner.hold_piles`, `_write_surface` and `_surface_worker` in `rebuild/review/build.py` register what they hold. A fresh unit's fragment is written to the spool as it is drafted, so no process holds a pile of enrichments. What remains of a fresh unit after its batch is its spool address: a column of the parent's packed unit store (`unit_store`), and a field of the projections a pooled worker holds until it has sent them (`worker.projections`).
...
- `Id()`: an `ID_WIDTH` id into the string table. The string itself is charged to the table, not to the row.
- `Hex(width)`: a hex digest stored as raw bytes, `HEX_WIDTH` of them for the sha256 the build keys by. It is charged whether or not the value is present, because a fixed-width column has no gaps. A present digest whose length disagrees with the declared width raises, so a column declared for one digest cannot silently size another.
- `Derived(bytes, absent)`: whatever the callable returns for the value, for a column whose width depends on the value, such as a window of two to four codepoints stored inline. An absent value costs `absent`, by default the `COUNT_WIDTH` count slot such a column writes for an empty value, and the callable is not called.
- `Many(element)`: a variable-length field, stored as an `OFFSET_WIDTH` offset and a `COUNT_WIDTH` count into a side column of elements, each sized by `element`. A field that is empty costs only the offset and count.
...
class Shape:
    """One node of a packed-row declaration. The module docstring lists the shapes and how each is sized."""
...
def packed_size(shape: Shape, value: object) -> float:
    """Return the bytes a packed row holds for `value` under `shape`: the row's own columns and its share of every side column, excluding the string table. A value the shape cannot read (a name that is not a string under `Id`, a tuple of the wrong length under `Positional`) raises, so a declaration that no longer matches its record fails the tests that build with the tally on instead of sizing the wrong field."""
...
def _fold_members(
    ...
    """Return the column readers under one record's or tuple's members. The members that are bare `Id`s are read together by one multi-argument getter, so a record of a dozen names costs one pass, the same as a record of one."""
...
class Vocabulary(Protocol):
    """A string table as `column_census` reads one: `chars` is the distinct strings' UTF-8 bytes summed, and `len` is their number (`columns.StringTable`)."""
...
class Pool(Protocol):
    """A pool of name tuples as `pool_bytes` reads one: `elements` is the distinct tuples' lengths summed, and `len` is their number (`columns.TuplePool`)."""
...
def pool_bytes(pool: Pool) -> int:
    """Return what a packed side column over the pool takes: each distinct tuple once, at an `ID_WIDTH` id per element plus an `OFFSET_WIDTH` offset. The strings are charged to the string table they are interned in, not here."""
...
def from_environment(
    ...
    """Return a tally when `AMS_SURFACE_PILE_TALLY=1` is in the environment, and None otherwise. The None is the only off switch: a build given None prints and estimates nothing."""
