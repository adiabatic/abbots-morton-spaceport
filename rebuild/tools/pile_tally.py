...
The format, one line per pile per boundary and sorted largest first within a boundary, then one line naming the largest — every line prefixed `[tally] ` (`TALLY`) and whitespace-tokenized, so a `grep '^\\[tally\\]'` over the cycle's per-step log (`tmp/build-logs/<run>/<nn>-surface-build.log`, where the build's stdout lands) reads the whole record back:
