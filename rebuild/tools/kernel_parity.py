...
The ladder lives here — `ladder_order`, `ladder_rungs`, `sub_spec` — and both `bench-the-rebuild/scaling/scaling.py` and `bench-the-rebuild/levers/kernel_all_configs.py` import it, so the sweep and this check cut the same rungs rather than two ladders that could drift apart: ligature closure first and then alphabetical, rungs at `sorted({*range(6, len(base), 2), len(base)})`, and each rung's keep-set filtered so a ligature rides only when every component it names rides with it. Byte-wise a rung adds nothing over the live dump — a nested subset with the registry riding whole can only restate bytes the live check already covers — so the rungs are not here for extra coverage today: they cost milliseconds, they are the sub-issue's stated bar, and they keep this harness cutting the same rungs the scaling sweep measures, which is what lets a ladder reading be read against a parity reading at all.
