...
# What one oracle row-range worker holds at its peak, and so the divisor of the post-build sweeps' width (`sweep_job_budget`). A worker is a spawn process holding its interpreter, a HarfBuzz shaper over M1.otf, the records of its own row range of its configuration's row store — one buffer of the range's record bytes with three packed arrays beside it, an offset and two ages a record, kept out of a scan of the whole member (`oracle_cache.load_store`), so what it holds of the store is its range's rows and not the configuration's — and its configuration's settle memo, loaded whole on the first wave that reaches the crate, which every pass reaches since the renewal slice re-derives one row in `oracle_cache.MAX_RECORD_AGE`, held as the columns its file stores (`conform._MemoStore`: six `array("I")` key columns and a value column, a reached byte a row and an open-addressed probe index of 2^k >= 2N slots over them, 38 bytes a window at 2N slots, the outcome tuples shared with the walk's own dict, which holds only the windows the walk promotes or settles) over 2.6M windows (the `[t] settle_memo` lines count them). The memo is the largest term as measured: in the store-cold width-ten record of that shape the ss10 range, whose configuration loads no memo, reads 0.33 GB where the fourteen ranges holding one read 0.58 to 0.72, and the shape's highest reading on this box is 0.72 GB store-cold and 0.73 GB store-warm at width ten over fifteen ranges. Read a pool record for the box-wide shape and never for one range's cost: `run_m1._spawn_pool` sets no `maxtasksperchild` and a worker reports its process high-water mark (`peak_rss.peak_rss_self_bytes`), so a range that runs second in a reused worker reads at or above the mark the range before it left, whatever its own rows. In the store-warm width-twelve seed record the 128,730-row memo-less `ss10 1/2` reads 0.86 GB, byte for byte the mark of the range its worker ran first, where its sibling `ss10 2/2` at 1,416,030 rows reads 0.43; at width ten the five smallest ranges, which are the five that run second, read 0.86 to 0.88 GB store-warm, above first-wave ranges carrying several times their rows; the store-cold records, where the second runners read at the pack, hide the carry. The seed is four `run_m1 --gates-only` passes on the 32 GiB box (`doc/fleet.md`) taken with the memo held as a dict of key tuples, store-cold (`--fresh-oracle-cache`) and store-warm, at `--jobs 10` over fifteen ranges and — standing in for the 48 GiB box, whose twelve cores choose that plan — at a hand-stated `--jobs 12` over seventeen, with every worker holding its own range's records beside its memo: the highest reading is 0.72 GB store-cold and 0.87 GB store-warm at width twelve, 0.80 GB store-cold and 0.89 GB store-warm at width ten, and no higher over the twelve further store-warm width-ten passes taken after them. In those records a store-warm pass lifts a first-run range over its cold reading by 0.08 to 0.16 GB, a floor on the store term rather than a reading of it: the cold arm re-derives every row and its peak carries walk state that grows with the range — at width ten the 849,618-row ranges read 0.79 to 0.80 GB cold against 0.70 to 0.72 for the other memo-holding ranges — so warm minus cold nets the store gained against the walk state not held. It rounds up past the dict-shaped reading by a quarter and then to the tenth, for the reason every divisor here rounds up — a per-unit cost that errs low is what puts a box into swap — and because the memo grows with the alphabet, so a migration moves it; the constant keeps that seed, a third and more above the columnar shape's highest reading, until the mapped row store lands (issue 263) and one measurement pass over both re-seeds it, and at either figure the division answers the cores. `make job-costs`' oracle-shard row is where it stays honest: `run_m1.run_oracle` files one kind:"pool" record per fan-out, one observation per row range that ran, so the constant is priced against the workers that actually ran, and the cycle's job-costs step is the check that runs it; the rebuild suite holds the constant to the cores and says nothing about a figure that errs low. At this figure neither box in the fleet is memory-bound for this pool: the cores are.
