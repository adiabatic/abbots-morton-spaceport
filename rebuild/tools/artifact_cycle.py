...
# What one oracle row-range worker holds at its peak, and so the divisor of the post-build sweeps' width (`sweep_job_budget`). A worker is a spawn process holding its interpreter, a HarfBuzz shaper over M1.otf, its own row range's records of its configuration's row store, as one buffer with an offset and two ages a record beside it (`oracle_cache.load_store` scans the whole member and keeps the range), and — the largest term — its configuration's settle memo, loaded whole on the first wave that reaches the crate, which every pass reaches since the renewal slice re-derives one row in `oracle_cache.MAX_RECORD_AGE`: the window keys and shared outcome tuples of 1.8M windows (the `[t] settle_memo` lines count them). The seed is the first two cycle passes at width ten over fifteen ranges on the 32 GiB box (`doc/fleet.md`), taken with every worker holding its configuration's whole store beside its memo: store-cold, the widest range read 1.24 GB; store-warm, 1.72 GB, and the shortest ranges among the highest readings, since a worker's memo is its configuration's whole one whatever its share of the rows. It rounds up past the warm reading by a quarter, for the reason every divisor here rounds up — a per-unit cost that errs low is what puts a box into swap — and because the memo grows with the alphabet, so a migration moves it. `make job-costs`' oracle-shard row is where it stays honest: `run_m1.run_oracle` files one kind:"pool" record per fan-out, one observation per row range that ran, so the constant is priced against the workers that actually ran. At this figure neither box in the fleet is memory-bound for this pool: the cores are.
