...
# What the plumbing step's parent holds while the standing fill's pool is live, subtracted from the box before dividing by the worker cost: the verdict chain's human index records and the id of every unit on the surface (`unit_index.load_human_units`) and the fill's rules, memo and decisions. The pile is flat in the width and grows with the alphabet. The standing-fill-parent row of `make job-costs` measures it through the plumbing step's peak: `peak_rss.reap_peak_rss_bytes` takes the maximum process peak in the tree, and the chain parent is much larger than its chunk-bounded workers. The seed rounds above the qsCheer cycle's measured parent peak with the refill pool enabled, over a chain that parsed every index record; the rows a chain holding the human records alone files read far under it, and that row on a memo-drop pass is the re-seed's input. A pass that refills a dropped memo serially holds one unit's `SlideContext` memos in its parent at a time, since `Decider.decide` (rebuild/tools/standing_verdicts.py) empties them behind every unit it serves or computes; that is a different footprint and does not price this pooled shape. Re-measure as the alphabet grows.
STANDING_FILL_PARENT_BYTES = 14_000_000_000
