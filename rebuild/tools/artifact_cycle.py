...
# What the surface build's parent holds while its pool is live, subtracted from the box before dividing by the worker cost: the whole workload, each unit's state and projection, unit-store records and addresses, content keys, one fragment in hand, and one batch reply in flight per worker, which is the one term of it the width moves. Neither ink-signature table is in it: the window-keyed table the ink-duplicate merge reads (`signatures` in rebuild/review/build.py) is released where that merge returns, in the load phase, and the store's records (`signature_entries`) are held through the plan phase and die when the store is written: inline on the main thread as the units phase opens on a serial build, and a few seconds into the units phase on a pooled build, when the thread `build_m1` starts at that boundary (`_SignatureWrite`) finishes its write and drops them, so the records overlap the pool only for the write's length, and the plateau this constant prices holds neither table, while the load and plan phases that do hold them peak four gigabytes under it; the load boundary's `signatures` tally line prices that table strings included, and the digest strings are shared with the records, so the two tables together are what the release takes off, not the sum of their lines. Fragments stream by address into the shards and are released after the checker and sidecar spools consume them. Before the workload loads the same process packs the baseline subset tables (rebuild/review/subset_pack.py; `write_pack` holds every table's keys and row indices beside the pool of distinct rows), a transient well under the pool-live pile that this constant covers without a term of its own. The pile grows with the alphabet. The surface-parent row of `make job-costs` measures it through the surface-build step's peak: `peak_rss.reap_peak_rss_bytes` takes the maximum process peak in the tree, and the parent holds the corpus while each worker holds one batch. The seed rounds above the higher of the two step peaks this implementation reaches at the qsJay corpus on the fleet's 32 GiB box (`doc/fleet.md`), the surface-parent row's readings on two signature-warm, unit-cold width-eight passes with no surface code changed between them — 8.75 GB, whose phase-boundary tallies read 8.61 to 8.66 GB from the units phase through the cache write, a tenth of a gigabyte under its step peak because a boundary reads the pile at rest and the peak reads its high-water mark, and 9.08 GB, whose tallies read 8.71 to 8.72 GB from the units phase through census-facts and 9.08 GB at the cache write, so the cache boundary is where two full builds at one corpus differ — by the third of a gigabyte those two peaks are apart and then up to the tenth, so that the run-to-run spread between full builds at one corpus sits inside the headroom rather than across it. A `--fresh-unit-cache` pass over the same corpus, where the signature pool shapes every digest, tallies 8.79 GB at its cache boundary, which puts its step peak near 8.9 GB, under both; re-seed from the row's step peak, never from a tally line. A served pass over the same corpus, where the parent alone carries the unit-store records after the pool has closed, peaks under the cold pass's plateau by the better part of a gigabyte (8.44 GB against 9.39 GB, measured with the ink-signature records held through the cache phase), so the full pass is the one the seed covers and the one to read when re-seeding. The row considers measurements after the commit that sets the constant, so older implementations' peaks do not price this one. Re-measure as the alphabet grows.
