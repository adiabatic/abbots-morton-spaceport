...
def _closure_digest(root: Path, rel: str) -> str:
    """Rune YAMLs hash by their prose-blind digest (fingerprint.rune_file_digest) so a documentation edit does not re-run the gate; the contracts tests that read the live rune files assert structure, settlement outcomes and round-trip identity, never prose content, which is what keeps the exclusion sound."""
