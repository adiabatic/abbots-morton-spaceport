...
# What the plumbing step's parent holds while the standing fill's pool is live, subtracted from the box before dividing by the worker cost: the verdict chain's whole unit index and the fill's rules, memo and decisions. The pile is flat in the width and grows with the alphabet. The standing-fill-parent row of `make job-costs` measures it through the plumbing step's peak: `peak_rss.reap_peak_rss_bytes` takes the maximum process peak in the tree, and the chain parent is much larger than its chunk-bounded workers. The seed rounds above the qsCheer cycle's measured parent peak with the refill pool enabled. A pass that refills a dropped memo serially also holds the whole domain's `SlideContext` memos in its parent; that is a different footprint and does not price this pooled shape. Re-measure as the alphabet grows.
