...
# What one surface worker holds at its peak, and so the divisor a box divides itself by to reach this build's fan-out width. A worker is a persistent spawn process that draws unit batches off the parent's hand-out queue (`_handout_width` in rebuild/review/build.py: the enricher's settlement batch, or a narrower spread of a small pile), enriches and drafts each one, spools every fragment to disk as it is drafted so that no EnrichedUnit outlives its batch (`_FragmentSpool`), and answers each batch with that batch's projections and spool addresses on the same message, so nothing it holds scales with the corpus or with the width: its own interpreter and shapers (whose shape memo is released at every unit batch boundary, `rebuild.review.ink.release_shape_memos`, so it holds one batch's windows rather than the corpus's — a bound `_MemoizedShaper`'s docstring records as measured rather than assumed, since unreleased the memo alone outgrew every other pile of a serial build), one baseline subset table for every configuration its batches reach — the parent hands the pile out in configuration order (`_configuration_order` in rebuild/review/build.py), so a worker holds one table through the run of the leading configuration and adds only the configurations its later draws touch: on the live corpus, whose units lead with `default` and `ss10` almost entirely and with the four other acceptance configurations in a run about six hand-outs wide between them, four workers of a width-six pool end holding three (the leading table, the one small configuration a middle draw lands in, and `ss10`), the worker whose middle draw runs out of the small configurations into `ss10` holds four, and the worker whose draws span the tiny configurations holds all six — each rebuild/review/enrich.py's SubsetRow projection of the three fields the enricher reads, glyph and seam strings interned, rather than every parsed rowmodel.Row — and one batch's units, projections and addresses, bounded by `PHASE1_HANDOUT_UNITS` the way `release_shape_memos` bounds the memo, held from the hand-out until the reply is pickled and released behind it. The seed is the first full-fresh pooled build on the configuration-sorted hand-out, a width-six pool over the 32-letter corpus on the fleet's 32 GiB box (`doc/fleet.md`) whose four three-table workers read 1.14 to 1.19 GB at their peaks, its four-table worker 1.41 GB and its six-table straddler 1.95 GB, the pile tally (`AMS_SURFACE_PILE_TALLY=1`, rebuild/tools/pile_tally.py) reading `worker.subset_rows` at every worker's batch boundary being what attributes each peak to its table count. The straddler is what the constant prices: every worker is in flight when one crosses the configuration run, and the width is the box divided by the widest of them, not the median. It rounds up past that reading by a quarter, for the reason kernel_exec.DELTA_PEAK_BYTES rounds up past its own measurement, since a per-unit cost that errs low is what puts a box into swap while one that errs high only narrows a pool; and because a batch-drawing worker's peak is a fact about its own draws that no width moves, a wider pool is that reading repeated rather than a new one. A table grows with the alphabet (rebuild/pipeline/baseline_subset.py filters the baseline to `M1_ALPHABET`), so a migration moves this reading. `make job-costs`' surface-worker row is where it stays honest: rebuild/review/build.py files one kind:"pool" record per pooled build, so the constant is priced against the workers that actually ran — and a row that is quiet on a box is not a box this width has sent serial, since both machines in the fleet start a pool. It is a reading to keep current, never a contract.
SURFACE_WORKER_BYTES = 2_500_000_000
# What the surface build's parent holds while its pool is live, subtracted from the box before dividing by the worker cost: the whole workload, each unit's state and projection, the ink-signature store, unit-store records and addresses, content keys, and one fragment in hand. Fragments stream by address into the shards and are released after the checker and sidecar spools consume them. This pile is flat in the width but grows with the alphabet. The surface-parent row of `make job-costs` measures it through the surface-build step's peak: `peak_rss.reap_peak_rss_bytes` takes the maximum process peak in the tree, and the parent holds the corpus while each worker holds one batch. The seed rounds above that row's valid measured high-water mark through the qsCheer migration, including served passes whose manifest+check phase can peak above a cold build's pool-live pile. The row considers measurements after the commit that sets the constant, so older implementations' peaks do not price this one. Re-measure as the alphabet grows.
