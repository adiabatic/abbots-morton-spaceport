...
The skip demands that the surface build be skipping too, which is what makes the stamp knowable before the pass runs. A flag that names a carry output refuses the skip outright, since honoring it would mean writing nothing.

The same provably-unchanged principle guards every other heavy stage, each keyed by a content fingerprint over that stage's full input closure and a green record written only after that exact content passed live: run_m1 skips on rebuild/out/run-m1-green.json (the Stage A fingerprint components plus the contact allow-list, the oracle's subset tables and uv.lock's dependency pins) and re-evaluates its gate from the summary JSONs already on disk; gate:conform skips on conform-green.json, keyed on what the belt samples for rather than on run_m1's closure — the emitted lookup's behavior classes, the font-compilation code and its tools/ closure, the uharfbuzz version, and the sweep horizon, the deep sweep's own posture (`conform_skip_fingerprint`) — so a rune edit that mints no new rule shape leaves that key unmoved, the crate's string replay inside run_m1 having already held the new tables to the engine over every string; the rebuild suite skips on rebuild-contracts-green.json, keyed by rebuild_lane_fingerprint over its closure — the suite's repo closure under rebuild/ and glyph_data/ plus conftest.py, pyproject.toml, uv.lock by its dependency pins and the site fonts outside their head and name tables, and no build artifact at all, which is exactly why the suite can skip whether or not run_m1 rebuilt: a live M1 rebuild writes only under rebuild/out, which that closure does not contain. The record also carries a per-test input closure beside its key, so a pass whose key moved runs only the tests whose closure the diff reaches, a rune edit re-proving the tests that load the spec and nothing else; rebuild.tools.contracts_closure is the authority on what a closure holds and when a test may be kept off, and every doubt there runs the test. That record is also written by rebuild.tools.rebuild_gate, the `make test-rebuild` entry point, so interactive suite greens and cycle greens share it; surface-build skips when the manifest's recorded inputs fingerprint already equals the one a build would stamp now (a rebuild would be byte-identical, mtime-floored generated_at included, so the autosave stays aligned), and when the live surface fails that question but a rehearsal's directory answers it — the last cycle summary's `plan.review_out`, or var/rehearsal-review — that directory is moved into rebuild/out/review instead (`surface-promote`, `promotable_surface` holding the preconditions), stores and all, since every stamp inside a surface is content-only against its manifest and the move keeps the `generated_at` a rebuild would reset. The census step is neither keyed nor skipped: it reads the surface build's census-facts.json sidecar and rewrites one small checked-in file in milliseconds, so it simply runs every pass. The surface skip engages only on cycles where run_m1 itself skipped, and on the gates-only route when the Stage A record on disk is already what that pass will rewrite (`m1_stage_a_current`), since the surface reads nothing else the pass touches — which is the contact-allow bless, the one comparison-side edit outside every Stage A component. Conform's skip is decided after run_m1 has finished instead, over the key the artifacts it left actually carry: a route that leaves the emitted lookup's shapes, the compile code and the shaper under its last green key skips the sweep, whether run_m1 skipped, re-adjudicated, or rebuilt, and the skip is recorded as proved because a matching green is proof about this exact content. Taking that key only once run_m1 is over is also what keeps a live M1 rebuild from invalidating it mid-cycle: there is no key to invalidate until the artifacts have stopped moving. The preflight still answers it ahead of the pass on the one route whose artifacts it can already see — run_m1 skipped, so nothing is about to move — and that is the route --dry-run can predict; on the reuse and rebuild routes the printed plan shows the conform lane as undecided (`run?`), because only a finished run_m1 knows what the artifacts came out as, so a plan that promises the sweep may be answered by a pass that proves it unnecessary. Green records are written only when the key still matches after the work ran, and a red result whose key matches its record deletes the record. --fresh runs everything regardless.
...
# The gate pool's seats, sized to the tasks the chain submits rather than to the box or to the work actually in flight: under the queue policy a parked task holds its worker for the whole wait — conform on make-test, contracts on both — so every gate task has to be seatable at once, with slack on top of that. A seat short of the task count would serialize a wait behind an unrelated task's completion, which is the queueing this pool exists not to do, and a width taken from the cores in hand would put a small box exactly there. `test_the_gate_pool_seats_every_gate_task_at_once` in rebuild/test_artifact_cycle.py is what holds this and the chain's task list in step.
_GATE_POOL_WORKERS = 6
# What one surface worker holds at its peak, and so the divisor a box divides itself by to reach this build's fan-out width. A worker is a persistent spawn process that draws unit batches off the parent's hand-out queue (`_handout_width` in rebuild/review/build.py: the enricher's settlement batch, or a narrower spread of a small pile), enriches and drafts each one, spools every fragment to disk as it is drafted so that no EnrichedUnit outlives its batch (`_FragmentSpool`), and answers each batch with that batch's projections and spool addresses on the same message, so nothing it holds scales with the corpus, with the alphabet or with the width: its own interpreter and shapers (whose shape memo is released at every unit batch boundary, `rebuild.review.ink.release_shape_memos`, so it holds one batch's windows rather than the corpus's — a bound `_MemoizedShaper`'s docstring records as measured rather than assumed, since unreleased the memo alone outgrew every other pile of a serial build), one batch's units, projections and addresses, bounded by `PHASE1_HANDOUT_UNITS` the way `release_shape_memos` bounds the memo, held from the hand-out until the reply is pickled and released behind it, the `SubsetRow`s materialized for one batch's units, and a resident share of the baseline subset pack (rebuild/review/subset_pack.py), which the parent writes once before the pool starts and every worker maps read-only: the page cache holds one copy for the whole box, and what a worker's own figure counts is the pages its lookups touch, not a table and not the alphabet — the parent hands the pile out in configuration order (`_configuration_order` in rebuild/review/build.py), so those pages are mostly one configuration's key range. The seed is every width-eight pool on the mapped pack since the previous seed, on the fleet's 32 GiB box and its 18-core 48 GiB box (`doc/fleet.md`): over passes that draft units, workers read up to 0.52 GB at their peaks on the 32 GiB box, over the qsYe corpus, and the widest worker of each pass reads 0.42 to 0.48 GB on the 48 GiB box, while a served pass's workers read under a tenth of a gigabyte — against 1.16 to 1.95 GB for a width-six pool over the 32-letter corpus holding its subset tables in Python. It rounds up past the widest of the mapped pack's readings by a quarter and then to the hundredth, for the reason kernel_exec.DELTA_PEAK_BYTES rounds up past its own measurement, since a per-unit cost that errs low is what puts a box into swap while one that errs high only narrows a pool; and because a batch-drawing worker's peak is a fact about its own draws that no width moves, a wider pool is that reading repeated rather than a new one. A migration moves this reading only through what a batch holds — a window's rows and shapes — and not through the tables, which is the reason both fleet boxes stay at `SURFACE_JOBS_CAP` as the alphabet fills (`test_the_shipped_surface_divisor_holds_the_32_gib_box_at_the_cap_by_division` in rebuild/test_memory_budget.py holds that claim for the smaller box, `test_both_fleet_boxes_keep_a_pooled_build_under_a_gated_cycle` in rebuild/test_artifact_cycle.py for the larger). On a ten-core box the cap-width pool leaves gate:rebuild-contracts' co-resident suite one worker (`contracts_pool_width` weighs that). `make job-costs`' surface-worker row is where it stays honest: rebuild/review/build.py files one kind:"pool" record per pooled build, so the constant is priced against the workers that actually ran — and a row that is quiet on a box is not a box this width has sent serial, since both machines in the fleet start a pool. It is a reading to keep current, never a contract.
SURFACE_WORKER_BYTES = 660_000_000
# What the surface build's parent holds while its pool is live, subtracted from the box before dividing by the worker cost: the workload table (`audit.UnitTable` in rebuild/review/audit.py) — every unit's class, group, family, echo and cluster as ids into one string table, its config set, kinds, render groups and per-config class map as ids into pools of a few dozen values, its window parsed once into a `u16` side column, its run of the audit rows and its place in the triage index, as fixed-width `array` columns over the unit's ordinal, well under a hundred bytes a unit — the packed unit store (rebuild/review/unit_store.py) over the same ordinal and the same string table, the checker's identity dict, the census's pre-merge snapshot (`census.PremergeSnapshot`, columns over the pre-fold rows at about ten bytes each), one fragment or one materialized `audit.Unit` record in hand, one batch of materialized records while a hand-out is being sent, and one batch reply in flight per worker, which with the hand-out is the term of it the width moves; no list of unit records exists in the parent at any phase (`UnitTable.unit` materializes one for the reader that needs a record, and `units` is the census CLI's and the tests'). The store is where every per-unit product of phase 1 lives from the plan boundary to the cache write — the machine flags and ink deltas, the diff and cluster digests, the seam-home projection with its spans, names and rects, the secondary-home assignments, the fragment's spool or prior address, the shard address the write hands back, the content and input keys, the policy file and the config note — as fixed-width `array` columns and one string table over the vocabulary rather than the corpus (the `[tally] … unit_store` line prices the table beside the columns), materialized one unit at a time for the reduce or the store line that asks for one; a slotted state record, a spool-address object and a written map keyed by id hold the same facts at roughly six- to twentyfold the bytes, pile by pile (the `[tally]` lines of a pass under `AMS_SURFACE_PILE_TALLY=1` read the walked and packed bytes of each pile side by side), which is the reading this shape is chosen off, and the table's own `[tally] … workload.units` line reads the same way against the 645.6 bytes a unit the record list walked. Neither ink-signature table is in it: the window-keyed table the ink-duplicate merge reads (`signatures` in rebuild/review/build.py) is released where that merge returns, in the load phase, and the store's records (`signature_entries`) are held through the plan phase and die when the store is written — inline on the main thread as the units phase opens on a serial build, and a few seconds into the units phase on a pooled build, when the thread `build_m1` starts at that boundary (`_SignatureWrite`) finishes its write and drops them — so the records overlap the pool only for the write's length; the load boundary's `signatures` tally line prices that table strings included, and the digest strings are shared with the records, so the two tables together are what the release takes off, not the sum of their lines. What this figure prices is the whole surface-build step rather than any one phase of it. On a full-fresh pass the load holds the row columns (`audit.RowColumns`, five flat arrays over the audit's rows with their tuple pool, about seventeen bytes a row), the table, the snapshot and both signature tables, and the plateau holds the table, the store's columns, the identity dict and the snapshot in their place, the name tuples gone at the units boundary (`UnitTable.release_names`); the `rss_now_gb=` token beside `rss_gb=` on each `[t] review.build` line is what separates a phase's own resident set from the step's high-water mark (`_phase_timing` in rebuild/review/build.py), and it reads the load boundary as the mark on a full-fresh pass and on a served one alike: `rss_gb=` stands at the step's figure on the load line and never rises after it, the load's `rss_now_gb=` is the highest resident reading of the pass, and the plateau's boundaries read under it on either kind of pass, by a gigabyte and a half on a full-fresh pass and by better than half a gigabyte at the served units boundary, so the row columns and both signature tables standing beside the table are what price the step, not the plateau's store. A served pass's plan phase streams the store (`unit_cache.stream_store`) and folds each record into the columns the moment it is parsed, so what the plan holds beside the table and the store's columns is the map from input key to ordinal (its keys spelled off the store's key column; it dies at the plan's `del named`), one `unit_cache.ServedUnit` in hand, its projection tuples pooled within the record rather than through a table that would keep one entry per unit handed over (`unit_cache._served_unit`), and the addressless residue: the records the store hands over without an address, buffered whole until one walk over the previous surface's shards places them (the `[tally] plan unit_cache.unplaced` line prices them), which on a store this code wrote is no record at all, so a served pass reads at or under a full-fresh one, its plan boundary resident well under its load. A store whose every record is addressless — every part resized underneath it — buffers every record, and that plan reads like a parse of the whole store; the sink narrows the cliff to that case rather than removing it. Fragments stream by address into the shards and are released after the checker and sidecar spools consume them. Before the workload loads the same process packs the baseline subset tables (rebuild/review/subset_pack.py; `write_pack` holds every table's keys and row indices beside the pool of distinct rows), a transient well under the pool-live pile that this constant covers without a term of its own. The pile grows with the alphabet. The surface-parent row of `make job-costs` measures it through the surface-build step's peak: `peak_rss.reap_peak_rss_bytes` takes the maximum process peak in the tree, and the parent holds the corpus while each worker holds one batch. The seed is two untallied width-eight passes at the qsYe corpus on the fleet's 32 GiB box (`doc/fleet.md`), taken with nothing edited between them, the workload held as the table's columns and the served plan folding each record as it is parsed: one full-fresh, driven by `make artifact-cycle`, whose step peak reads 4.01 GB, and one served by hand over the surface it wrote, whose step peak reads 4.00 GB, both made at the load boundary (the load's `rss_now_gb=` reads 3.79 and 3.78, the served plan boundary's 2.90), the two a hundredth of a gigabyte apart where the record-list seed's two full-fresh passes were a twentieth apart and its served pass two gigabytes over them. It rounds up past the higher of the two by a quarter and then to the next whole gigabyte, as STANDING_FILL_PARENT_BYTES does, for the reason every cost here rounds up, since one that errs low is what walks a box into swap while one that errs high only narrows a pool, and because the pile is corpus-shaped, so every migrated letter moves it toward the figure before the row can say so. What the plateau walks past the columns is the checker's identity dict (the `[tally] … checker.identity` line: 214.8 bytes a unit walked against 9.2 packed on the qsYe corpus), the one per-unit residue this figure prices as objects; the one per-unit column held twice over is the window, the store's seam-home codepoint values beside the table's `u16` side column; and the one per-unit transient beside them is the triage sort's (`audit._triage_permutation`, run once at the load and once at the units boundary): one integer word a row, its class, group, window and id terms packed into it, a few dozen bytes each in a list the sort orders in place and drops as the permutation returns, so no `[tally]` line prices it and a units-boundary `rss_now_gb=` reads it only as what the allocator has yet to hand back. A tallied pass reads above the same pass untallied — it rebuilds the plan's input-key map at every boundary and holds the addressless residue past the `del` that frees it — so re-seed from the row's step peak, never from a tally line. The row considers measurements after the commit that sets the constant, so older implementations' peaks do not price this one, and a hand build files pool records alone, so its step peak is read off its `[t] review.build` lines rather than off the row. Re-measure as the alphabet grows.
SURFACE_PARENT_BYTES = 6_000_000_000
# The non-memory bound on the same width, and the half of this build's argument that arithmetic cannot supply: the parent that hands the pile out and merges every reply is one process, and eight is the width the pool is measured to: on the 32 GiB box the units phase's wall at width eight is its width-six wall scaled by the width ratio over the same corpus (`make cycle-timings ARGS='--inner'` reads the `review.build units` phase), so the pool scales to the cap; past it no reading exists, and a wider pool is a measurement to take before it is a re-seed.
SURFACE_JOBS_CAP = 8
# What one worker of the standing fill's refill pool holds at its peak, and so the divisor of that pool's width (`standing_fill_jobs`). A worker is a spawn process holding its interpreter, the rules, a `SlideContext` over the surface's font pair — two shapers — and one chunk of `_STANDING_POOL_CHUNK` units (rebuild/tools/standing_verdicts.py) with that chunk's shape and walk memos and its alignment cache, all three emptied behind every chunk, so the peak is chunk-shaped rather than slice-shaped and does not grow with the pool's share of the domain. The seed is three hand refills in the chain's form (`--open-only --require-reach`) over a dropped memo (`--fresh-memo`), each worker's resident set sampled every 0.1 s on the fleet's 18-core 48 GiB box (`doc/fleet.md`): two eighteen wide, whose thirty-six workers read 95 to 104 MB, and one two wide, whose workers decided about fifty-eight chunks each and read 99 MB, so a worker's peak does not climb with the chunks it decides; it rounds up past the highest of those readings by more than threefold for the reason every divisor here rounds up, and because no journal row prices this worker — `make job-costs` has no standing-fill-worker row, since filing pool records from the fill would put cycle_timings and peak_rss into the memo's code stamp (`MEMO_CODE_MODULES`) and drop the memo on every width edit — so a hand measurement at the chunk width is what to re-take when the chunk or what a decision holds moves. At this figure neither box in the fleet is memory-bound for this pool: the cores are.
STANDING_FILL_WORKER_BYTES = 350_000_000
# What the plumbing step's parent holds while the standing fill's pool is live, subtracted from the box before dividing by the worker cost: every surface id, the human id/echo/notation projection, and the fill's rules, primed keys, decisions and memo. Full human records stream through the chain's consumers; refill misses live in a temporary gzipped spool, with at most one pool wave of records in memory, bounded by the width times `_STANDING_POOL_CHUNK` in rebuild/tools/standing_verdicts.py. The complaint docket retains compact grouping projections. A serial refill holds one unit's `SlideContext` memos at a time because `Decider.decide` clears them behind each unit. The standing-fill-parent row of `make job-costs` reads the whole plumbing step through `peak_rss.reap_peak_rss_bytes`, and the chain sets that peak after the fill, not beside its pool: sampled every 0.1 s over two served passes that merge straight in on the fleet's 18-core 48 GiB box (`doc/fleet.md`), the chain holds at most 2.06 GB through the standing fill and 2.74 GB in the complaint docket. A standalone whole-domain fill prices the in-flight wave at about 18 MB a seat: its parent reads 1.17 GB two wide and 1.45 GB eighteen wide. The seed is the highest step reading any fleet box has filed since the previous seed: 4.36 GB on the 32 GiB box, over a carry across a rune edit (run 2ca8c2192122 at cb205186, 709 verdicts stranded and three echo rounds). The 48 GiB box reads 3.10 to 3.11 GB on carried served passes, 2.74 on served passes that merge straight in (6444755ee9aa, 169f78e7d20d, 376f72a5e4e7), 3.28 on a rules commit whose 1,219 misses are decided serially below `_STANDING_POOL_THRESHOLD` (8a407a3d83f4, 08a24101190c), and 2.19 and 2.33 on memo-drop passes pooled eighteen wide (6ec8760f21c2) and sixteen wide (27aa6ac52892). It rounds beyond a quarter of headroom to the next whole gigabyte. No fleet width moves at this figure: the cores bind the refill pool, and the belt stays at its configuration count. IDs and decisions grow with the alphabet, and no rune edit has yet run through the chain on the 48 GiB box, so that row must track the budget as migration proceeds; bounding the transient records does not make the parent constant-sized.
STANDING_FILL_PARENT_BYTES = 6_000_000_000
# What one oracle row-range worker holds at its peak, and so the divisor of the oracle's width (`sweep_job_budget`). A worker is a spawn process holding its interpreter, a HarfBuzz shaper over M1.otf, the crate's formation surface, the records of its own row range of its configuration's row store — one buffer of the range's record bytes with three packed arrays beside it, an offset and two ages a record, kept out of a scan of the whole member (`oracle_cache.load_store`), so what it holds of the store is its range's rows and not the configuration's — its configuration's settle memo, mapped read-only on the first wave that reaches the crate, which every pass reaches since the renewal slice re-derives one row in `oracle_cache.MAX_RECORD_AGE`: `conform._MemoStore` over the file's own layout, the six id columns, the value column and the 2^k >= 2N-slot probe index as views over the mapping, 69.8 MB for a live memo of 2.6M windows (the `[t] settle_memo` lines count them), so those pages are the page cache's, resident once a box however many workers map the file and in a worker's resident set as its probes touch them — a store-warm walk probes the one row in twenty the store does not serve, a store-cold walk nearly every row — and, on a pass after a family moved, the six id columns whole, 36.2 MB of the mapping, which the retirement fold reads once (`conform._MemoStore.load` over `mask.moved`); the seed's passes run on an unmoved tree, every `[t] settle_memo` line at stale=0, so that fold is outside the seed and inside the headroom — while the worker's own heap holds the file's interned label and outcome tables, a dead byte and a reached byte a row, 0.005 GB at the load; the writer's pile, which only the parent's part absorb holds: no range writes the file, since every range, its configuration cut or not, files the windows it settled fresh as a part (`run_m1._shard_settle_memo`), and the absorb, one task per settlement configuration on this same pool once every range has landed and the witness stage has returned (`run_m1.run_oracle`'s `memo_ready`), carries the standing rows whole beside the parts and the standing index, holding the columns, the writer's folded copies and the index at once (`conform._write_settle_memo`), roughly the file's size plus the columns'; its reading is filed in the pool record beside the ranges' as `<config> absorb`, a process high-water mark like the rest, so it reads at or above the range its worker ran before it, and in every seed record, each of which carries an absorb for every settlement configuration, it reads at a range's mark and never above the record's highest range — and the walk's own state over the range: the chunk of rows in flight (`oracle.ORACLE_ROW_CHUNK`), the waves of windows the crate settles for it, and `windows`, the dict holding the entries the walk promotes out of the mapping or settles fresh. The staged measurement is one process running `oracle.oracle_config_worker` over one shard of `oracle.oracle_shard_plan`, its row cache opened store-warm (`read_dir` the out dir) or store-cold (`read_dir=None`), stamped with `resource.getrusage`'s `ru_maxrss` and the resident set from `ps` at the interpreter, after the imports, spec, `kernel_exec.guard_sweep` and shaper, around `oracle.open_row_cache`, around `_SettledWindowWalk._load_memo` with the mapping's size and the store's heap sizes printed, and at return; the probe and both its logs are kept beside the seed passes' logs under `var/keep/rung263-4/stage-probe/`. Over the 708,015-row `default 1/3` of the width-twelve plan the terms read: the interpreter with its imports, spec, formation surface and shaper 0.07 GB; the range's store records 0.05 GB resident behind a 0.16 GB peak at the load store-warm, and nothing store-cold; the first chunk and its wave, held before the memo maps, 0.14 GB warm and 0.15 cold; the memo 0.005 GB of heap at the load, warm and cold alike, over a 69.8 MB mapping of 36.2 MB of columns and 33.6 MB of index; and the walk from there to the process's peak, the mapping's touched pages inside it, 0.27 GB store-warm, promoting 71,448 windows, and 0.33 GB store-cold, promoting 1,171,462 — a peak of 0.52 GB warm and 0.55 cold, which is what the pool records read for that range. The largest term as measured is the walk over the range's rows, 0.41 to 0.48 GB from the first chunk to the peak, better than fifty times the memo's heap and better than four times the whole mapping even with every page of the mapping netted out of the low end: the memo-less ss10 ranges, whose overlay walk maps nothing, read 0.32 to 0.33 GB store-cold against 0.38 to 0.67 for the ranges holding one — the 0.38 being the width-ten record's 154,476-row `ss03 1/3`, which ran second in the worker that had run the memo-less `ss10` and so reads near its own cost where the records' other narrow ranges read at a wider range's mark, the 77,238-row `ss03+ss05 3/3` at 0.55 there and the width-twelve record's 128,730-row `default 3/3` at 0.52 — and a cold pass's widest ranges read highest because that state grows with the range — at width ten the 849,618-row ranges read 0.64 to 0.67 GB cold against 0.38 to 0.55 for the nine other memo-holding ranges. Read a pool record for the box-wide shape and never for one range's cost: `run_m1._spawn_pool` sets no `maxtasksperchild` and a worker reports its process high-water mark (`peak_rss.peak_rss_self_bytes`), so a range that runs second in a reused worker reads at or above the mark the range before it left, whatever its own rows. In the store-warm width-twelve seed record the 128,730-row memo-less `ss10 1/2` reads 0.52 GB, the mark of the range its worker ran first, where its sibling `ss10 2/2` at 1,416,030 rows reads 0.43; at width ten the five smallest ranges, which are the five that run second, read 0.53 to 0.56 GB store-warm against 0.55 to 0.56 for the first-wave 849,618-row ranges, and the record's maximum, 0.56, is shared by the 849,618-row `ss03 2/3` and the 77,238-row `ss03+ss05 3/3`; the store-cold records, where a second runner reads at the pack, hide the carry, and there `ss10 1/2` reads 0.32 GB, its sibling's mark. The seed is four `run_m1 --gates-only` passes on the 32 GiB box (`doc/fleet.md`), store-cold (`--fresh-oracle-cache`) and store-warm, at `--jobs 10` over fifteen ranges and — standing in for the 48 GiB box, whose twelve cores choose that plan — at a hand-stated `--jobs 12` over seventeen, with every worker holding its own range's records beside its mapped memo and the five absorbs filed beside the ranges: the highest reading is 0.55 GB store-cold and 0.53 GB store-warm at width twelve, 0.67 GB store-cold and 0.56 GB store-warm at width ten (the pool records finished between 2026-09-16T09:17:38Z and 09:20:58Z, their logs under `var/keep/rung263-4/`). It rounds up past the highest of those by a quarter and then to the tenth, for the reason every divisor here rounds up — a per-unit cost that errs low is what puts a box into swap — and because the walk's state grows with the alphabet, so a migration moves it. `make job-costs`' oracle-shard row is where it stays honest: `run_m1.run_oracle` files one kind:"pool" record per fan-out, one observation per row range that ran, so the constant is priced against the workers that actually ran, and the cycle's job-costs step is the check that runs it; the rebuild suite holds the constant to the cores and says nothing about a figure that errs low. At this figure neither box in the fleet is memory-bound for this pool: the cores are.
ORACLE_SHARD_BYTES = 900_000_000
# What one belt worker holds at its peak, and so the divisor of gate:conform's width (`conform_job_budget`). A worker is a spawn process running `conform.conformance_config_worker` for one acceptance configuration, holding its interpreter, a HarfBuzz `Shaper` over M1.otf, the spec's alphabet, splitters, glyph names and anchors, the section 5.7 verdict surface the parent hands down, its configuration's settle memo, mapped read-only (`conform._MemoStore`) and walked with `promote=False` — the mapping's columns and probe index are page-cache pages, resident once a box and counted in a worker's resident set as its probes touch them, which over a belt is nearly all of them, while the heap holds the file's label and outcome tables plus a dead byte and a reached byte a row — and `windows`, the dict holding the windows the walk settles fresh, which is empty on a warm memo, holds every window on a cold one, and is the term that grows. The `ss10` overlay worker maps no memo and reads under a tenth of the constant. The belt's controller reads under a quarter of the constant and rides inside the reserve rather than as a term of its own. The seed is the `conform-belt` pool records of two boxes in `doc/fleet.md`. On the 10-core M1 Pro 32 GiB MacBook Pro, three width-six hand `run_m1 --conform-only` belts finished between 2026-09-20T08:45:15Z and 09:28:24Z, their logs under `var/keep/issue-273/`: warm settlement workers read 0.383 to 0.388 GB and cold ones (memo files moved aside, so every window settles fresh) 0.911 to 0.916 GB, the `ss10` worker 0.061 to 0.063 GB and the controller 0.28 GB. On the 18-core M5 Pro 48 GiB MacBook Pro, a cycle's gate:conform beside a live surface build and four hand belts finished between 2026-09-23T01:18:26Z and 01:43:38Z, their logs under `var/keep/issue-273/m5pro-48gib/`: the cycle's settlement workers, whose walks pruned the windows the witness stage had added to each memo and rewrote the file, read 0.382 to 0.384 GB, warm hand workers at widths six and two 0.231 to 0.237 GB, and cold ones 0.904 to 0.933 GB at width six and 0.904 to 0.913 GB at width two, where a reused worker runs three configurations and carries what the ones before it left, and still reads no higher than a fresh one; the `ss10` worker reads 0.060 to 0.063 GB at width six, and the controller 0.28 GB. It rounds up past the highest of those, 0.933 GB, by a quarter and then to the tenth, for the reason every divisor here rounds up — a per-unit cost that errs low is what puts a box into swap — and because the fresh-window dict grows with the window count, which `rebuild/scaling-ladder.txt` fits against letters and runes, so a migration moves the cold reading. `make job-costs`' conform-belt row is where it stays honest: `run_m1.run_font_conformance` files one kind:"pool" record per pooled belt at `conform.BELT_HORIZON`, one observation per configuration, and the gate:conform step peak is not admitted, since `peak_rss.reap_peak_rss_bytes` maxes over the tree and reads one process. Beside the build lane's larger step (the surface build's parent and workers, or the plumbing step's chain parent and refill pool, `_conform_build_lane`), no box in the fleet is memory-bound for this pool at this figure: the acceptance configurations are.
...
def _lock_line_digest(path: Path) -> str:
    """`fingerprint.lock_digest` with `_sha256_path`'s `absent` for a lock that is not there, which is what the fake repos without one stamp."""
    from rebuild.pipeline import fingerprint
...
def run_m1_skip_fingerprint(root: Path = ROOT) -> str:
    """Content key over everything a full run_m1 reads: the data inputs and pipeline code per file, the contact allow-list the defect gate reads, the full baselines, the oracle's subset tables — which the `baselines` line covers only by proxy — and uv.lock's dependency pins for the toolchain. Matching the recorded green means a rerun would reproduce rebuild/out/m1 byte for byte. Missing it says only that something moved; which lines moved is what decides whether the remedy is a rebuild or a re-adjudication (`gates_only_reuse`)."""
...
def comparison_side_label(label: str) -> bool:
    """Whether one `run_m1_skip_lines` label names an input the comparison reads and the build does not — the roster that lets a cycle re-adjudicate over the tables and font on disk (`run_m1 --gates-only`) instead of paying a kernel fan-out for artifacts that would come back byte-identical. Four kinds qualify, and what they share is that `fingerprint.tables_value` covers none of them, so an enumeration on disk stays exactly as fresh as it was. The ledgers and the kern sidecar (`fingerprint.NON_TABLE_DATA_LABELS`) are read to name, classify and position divergences over rows the fixpoint has already decided. The contact allow-list (`fingerprint.CONTACT_ALLOW_LABEL`) is the defect gate's, and the defect gate reads minted glyphs rather than making any. The oracle's two modules (`fingerprint.COMPARISON_CODE_MODULES`) are the classifier those files feed and the position channel that shapes the rows it calls ink-identical, and rebuild/test_build_code_closure.py is what holds the build to never importing either. The `baselines` line and the `baseline-<config>.subset.tsv.gz` lines are the before side of the comparison, which no table stage and no emitter reads.

    `uv.lock` is deliberately not on the roster, though the tables' stamp misses it too: it pins fontTools and uharfbuzz, so a bump there can move the compiled font's bytes and what HarfBuzz makes of them, and standing on a font a different toolchain built is the one reuse this must never license. Its line covers the dependency pins alone (`fingerprint.lock_digest`), so the project's own version bump never reaches this question at all: the line stays where it was, and nothing moved.
    """
    from rebuild.pipeline import fingerprint
...
def rebuild_lane_closure(root: Path, lane: str) -> tuple[str | None, dict[str, str] | None]:
    """The rebuild suite's input closure as the key and the per-label digest map the key is a digest of — one pass over the files answers both, and keeping the map the key's only source is what makes the per-test selection sound: every input the key can move on is a label the selection can see move. The closure covers the repo files from rebuild_gate_closure_files, which has already dropped the exempt paths, so a bless of a contact signature moves nothing here — plus the site fonts, which are `make all` output its shaping tests measure against, moved by no rune edit, and whose Senior sha every baseline header records and `baseline_subset.prove_font_provenance` holds the oracle's tables to — hashed as one `fonts` label through `fingerprint.fonts_value`, blind to their `head` and `name` tables, so the `make all` a version bump runs moves nothing here while a glyph, anchor or layout change still does; it deliberately contains no build artifact at all, so a verdict-only or artifact-only cycle re-runs nothing here, and a live M1 rebuild — which writes only under rebuild/out — cannot invalidate the key mid-pass. The harness roster rides in for the reason the audit found it: collecting rebuild/ imports test/test_shaping.py in every process, and the tools/ modules come with it. The closure holds the rune files and both human-reviewed ledgers — the divergence ledger and the standing approvals — all three prose-blind, because several tests load the live spec and the live ledgers, and every one of them reads structure rather than prose. The verdict store is absent — the suite exercises it only through fixtures — which is what lets a verdict-only cycle skip the suite entirely. None when git is unavailable, in which case the caller must run the suite unconditionally."""
    from rebuild.pipeline import fingerprint
...
def surface_build_skippable(
    ...
    `ignore` names fingerprint components exempted from the comparison, for a caller asking a narrower question than byte identity — the same hard/warn split status._freshness_check draws. The cycle asks three questions of it in turn rather than one: the strict one first, since a surface that reproduces byte for byte needs nothing done to it at all; when only an ASSET_COMPONENTS member differs, it copies those assets over the served surface and restamps that one component (`assets-refresh`) instead of rebuilding units that cannot have moved; and when the live surface fails both, `promotable_surface` asks the strict question of a rehearsal's directory, which the pass moves into place when it answers yes (`surface-promote`). A component missing from either side still refuses: only a recorded-and-expected pair is ever waved through.
    """
    from rebuild.pipeline import fingerprint
...
def promotable_surface(
    ...
    """The rehearsal directory a live pass can move into rebuild/out/review instead of rebuilding, or None. A rehearsal (`--review-out`) writes a whole surface — shards, sidecars, unit store and signature store — somewhere the live pass never reads, and the live pass that follows would otherwise rebuild those same bytes cold, its own store being stamped for the pre-rehearsal environment. Candidates are asked in order: the `plan.review_out` the last cycle summary recorded (a repo-relative string, so it resolves against `root`), then `var/rehearsal-review` under `root`, the conventional directory: `--review-out` takes any path, but a rehearsal is expected to use that one. Every path asked here derives from `root`, so a scratch repo never reads the live rehearsal.

    A candidate is promotable when it is a directory other than the live one, on the live directory's filesystem (`os.replace` cannot cross one, and a plan must never print a move it cannot make), when `surface_build_skippable` answers yes for it — that function is the authority on what "reproduces these inputs byte for byte" means, the after font and the three stamped sidecars included — and when its `generated_at` is not older than the live surface's. The stamp precondition is the one a byte-identity check cannot supply: `generated_at` is the latest input mtime rather than a build time (rebuild/review/build.py's `_generated_at`), so a rehearsal can carry a stamp older than the surface it would replace, and merge_verdicts refuses a store stamped newer than the surface it is merging onto — a backwards promotion would fail the plumbing step after the tree had already moved. Any unreadable manifest refuses, which costs a rebuild and nothing else.
    """
...
def sweep_job_budget(ncores: int | None = None, total_bytes: int | None = None) -> int:
    """The --jobs budget for run_m1's oracle, whose unit is a row range of one configuration's table and whose pool is therefore as wide as the box allows: the box less its reserve, divided by what one range's worker holds (`ORACLE_SHARD_BYTES`), capped at the cores this process may run on. `make conform-deep` (rebuild/tools/deep_sweep.py) takes it as its default too; gate:conform's belt has a budget of its own, `conform_job_budget`, since its worker holds a different pile and shares the box with the surface build. Nothing is subtracted for a co-resident pool: gate:make-test's pytest pool can still be hot when the oracle starts on a non-rehearsal pass, and at this divisor the pool that shares the box with it fits inside the reserve on either fleet box, where a reservation would narrow the phase every pass for a few seconds of overlap on some. Nor is anything subtracted for run_m1's own table-only branch, whose witness stage and shipped-order walks can still be running when the pool starts, the overlap `doc/parallelism.md` states: what either holds is far inside the reserve. run_m1's memory ceiling lives in the table build, whose width is --kernel-threads and which these jobs never reach. `ncores` and `total_bytes` are keywords for the reason every budget here takes its box as one — an assertion about a machine the suite is not running on has to be a pure function over an invented one."""
    from rebuild.tools import memory_budget
...
def surface_job_budget(
    *,
    ...
    """The --jobs budget for the review-surface build, and the third memory-derived fan-out in this tree: the box less its reserve less what the build holds flat, divided by what one more worker costs, under a cap that is the box's cores and SURFACE_JOBS_CAP together. The two halves are separate constants because this build's flat half is of the same order as its divided half — SURFACE_PARENT_BYTES is the parent, which holds the whole workload as the workload table's columns (`audit.UnitTable`, rebuild/review/audit.py) and the packed unit store's (rebuild/review/unit_store.py), plus the one fragment or materialized unit record in hand (the fragments are read back out of the spools by address and stream through it into the shards rather than piling up in it until manifest+check), and which is there at any width — so it is subtracted before the division exactly as gate:make-test's pool is, rather than smeared through a divisor. SURFACE_WORKER_BYTES is the divisor: a worker's own interpreter and shapers, one batch's units, projections and spool addresses, the rows materialized for that batch, and its resident share of the baseline subset pack it maps read-only (rebuild/review/subset_pack.py; the page cache holds one copy for the box), which is flat in the width because the parent hands the corpus out a batch at a time and takes each batch's results back on the reply, and flat in the alphabet because the tables live in the mapping and not in the worker. The signature pool `_resolve_signature_digests` starts ahead of the units phase takes none of this: its workers are one comparator apiece, flat in the pile they shape and under a tenth of a gigabyte against a surface worker's half of one, so memory binds the unit worker and not them, and that pool runs at `signature_job_budget`'s cores-bound width instead.
    ...
    Erring steep is the deliberate direction for the divisor, because the two ends are not symmetric: a divisor that errs low is what walks a pool into the box's reserve, while one that errs high only hands a roomy box fewer workers than its true cost curve would allow. A box the pooled shape does not fit at all floors at one, which is not a refusal but the serial shape: at width one there is no pool, every fragment exists once instead of twice, and the build is the cheapest it can be on a box that has outgrown it. The parent's pile is priced in SURFACE_PARENT_BYTES rather than in a width so that shrinking it widens this fan-out on every box at once; what the parent holds is structural per unit — one row of the workload table's columns for the unit, and one row of the unit store's for its state, its key and its address, no record of it held past the reader that asked for one — and issue #160's on-disk workload was sized against that residual and closed as not needed.
...
def contracts_pool_width(
    *,
    ...
    """How wide gate:rebuild-contracts' pytest pool runs under a cycle — the number the cycle states on that child's environment as PYTEST_XDIST_AUTO_NUM_WORKERS, and the second fan-out here that memory does not bind: no contracts worker holds a live artifact, so nothing prices one and the width is a count of cores. The suite is submitted beside the surface build, so the cores are the box's less the build's parent and its `surface_job_budget` workers, and less gate:make-test's pool under the overlap policy (`_contracts_pool_terms`), floored at one; a pass whose surface build does not run — skipped, promoted, or assets-refreshed — hands the suite the whole box, since there is nothing to share it with. The cap is what makes co-residency affordable: `doc/parallelism.md` records two full-width pools oversubscribing the box roughly 2:1 tripling the suite's wall, and holding the suite to the cores the build leaves keeps the process sum near the core count instead. The suite's controller is deliberately not in the subtraction — it idles while its workers run, and a seat taken off for it would cost the suite a worker on every pass for a process that holds no core — so the sum overshoots the cores by that one process. The bytes the pool holds come out of the reserve `memory_budget` keeps back rather than off `surface_job_budget`'s co-resident term, the position `_standing_fill_terms` already takes for this same pool: pricing it there would narrow the build every pass to insure against a worker no constant measures (`calibrate_budgets.UNITS` states that too). On a ten-core box beside a cap-width build the arithmetic reaches one under either policy — ten cores less the parent less `SURFACE_JOBS_CAP` workers — which is the serial suite, and the plan line prints that rather than hiding it. The trade is weighed: the two cores the suite gives up there go to the build's units phase, whose wall is the lane's, while the suite's is not on an ordinary pass — under a cycle it runs the closure-selected tests only, and its one-worker `gate:rebuild-contracts` rows in `rebuild/out/cycle-timings.ndjson` end inside their runs' `surface-build` rows, so serial it stays inside the build's shadow; the pass that reruns the whole lane at one worker can outrun the build, and that cost is accepted for the build's two cores until a one-worker row ends after its run's `surface-build` row, the signal to revisit the floor. Overlap takes no higher floor: a twelve-core box beside a cap-width build and gate:make-test's pool reaches one on the arithmetic alone — twelve cores less the parent, `SURFACE_JOBS_CAP` workers and that pool's two — and that one worker fills the box, so a floor of two would put a thirteenth process on it, and would run overlap wider than the queue policy on the ten-core box, to shorten a wall that is not the lane's. A reader who wants the suite wider states PYTEST_XDIST_AUTO_NUM_WORKERS, which wins ahead of all of this and not as a courtesy — the child inherits this process's environment, so a width already stated here is the width that pool is going to take, exactly as `make_test_pool_width` reads it. `ncores` and `total_bytes` are keywords for the reason every budget here takes its box as one — an assertion about a machine the suite is not running on has to be a pure function over an invented one."""
    stated = os.environ.get("PYTEST_XDIST_AUTO_NUM_WORKERS")
    if stated:
        return max(1, int(stated))
...
def _conform_build_lane(
    *,
    ...
    """The build-lane step the belt is priced beside, by its plan step name, and what that step holds: of the surface build (its parent and the workers `surface_job_budget` resolves) and the plumbing step (its chain parent and the refill pool `standing_fill_jobs` resolves), whichever this pass runs that holds more, the surface build on a tie, and `("", 0)` on a pass that runs neither. Each is read off its own budget rather than restated, so the belt's reservation and the step's width can never drift apart. The belt can run beside either one: its lane is submitted as run_m1's gate passes, so it opens beside the surface build, and the plumbing step follows the build in the same lane, so a belt still running then, or one the queue policy starts late behind make-test, runs beside the plumbing step instead."""
...
def _conform_fit_terms(
    *,
    ...
    """The three arguments the belt's width takes — the per-worker divisor, what comes off the box before the division, and the non-memory cap — derived once, so `conform_job_budget` and `conform_job_derivation` are two readings of one derivation rather than two derivations that happen to agree, the way `_surface_fit_terms` serves the surface build. What comes off is the build-lane step `_conform_build_lane` names, and nothing for the build lane on a pass that runs neither the surface build nor the plumbing step; plus gate:make-test's pool under the overlap policy, where that pool is hot beside the belt, and nothing under the queue policy, which parks the belt until make-test has finished. The cap is the acceptance configurations, one worker apiece, and the cores this process may actually run on."""
...
def conform_job_budget(
    *,
    ...
    """The `--jobs` the cycle hands gate:conform: how many of the belt's workers, one spawn process per acceptance configuration (`run_m1.run_font_conformance`), run at once. It is the box less its reserve, less what shares the box with the belt, divided by what one worker holds (`CONFORM_BELT_BYTES`), capped at the acceptance configurations and the cores (`_conform_fit_terms`), and floored at one. The lane is submitted as run_m1's gate passes, so the belt runs beside the build lane: it opens beside the surface build, and a belt still running when the build hands over, or one the queue policy starts late behind make-test, runs beside the plumbing step instead. So of the two, the one this pass runs that holds more comes off the box (`_conform_build_lane`): the surface build's parent and the workers `surface_job_budget` resolves, or the plumbing step's chain parent and the refill pool `standing_fill_jobs` resolves, whose own width leaves the belt out because this subtraction prices the overlap. A pass that runs neither subtracts nothing for the build lane, and `plumbing_runs` defaults to False because only the cycle's plan runs a plumbing step. gate:make-test's pool comes off under the overlap policy only, since the queue policy parks the belt behind make-test. Two things share the box unpriced, their bytes riding in the reserve as every unpriced process's do here: under the overlap policy, gate:rebuild-contracts' pool, which is cores-bound and priced by no constant (`calibrate_budgets.UNITS` states that position), and the build lane's other steps, which no constant prices either.

    The constant prices the belt at its per-edit horizon (`conform.BELT_HORIZON`) only. A cycle run with a deeper `--conform-horizon` holds its windows in process and shares no memo, and the same figure prices it unmeasured, which is why deeper sweeps belong to `make conform-deep`. A hand `run_m1 --conform-only` takes the idle arm (`skip_gates=True, skip_surface=True`, no plumbing step). The floor at one is the serial belt (`conform.run_conformance`), and the cycle states that width on the argv like any other. `ncores` and `total_bytes` are keywords for the reason every budget here takes its box as one — an assertion about a machine the suite is not running on has to be a pure function over an invented one.
    """
    from rebuild.tools import memory_budget
...
def conform_job_derivation(
    *,
    ...
    """`conform_job_budget` said out loud for the plan's Lane conform line and the `--jobs` help: `memory_budget.describe_fit` over the terms it divides, so the clause opens with the width it explains."""
    from rebuild.tools import memory_budget
...
def _standing_fill_terms(
    *, skip_gates: bool, skip_make_test: bool, ncores: int | None
    ...
    """The standing fill pool's three terms, derived once for `standing_fill_jobs` and `standing_fill_derivation` the way `_surface_fit_terms` derives the surface build's: STANDING_FILL_WORKER_BYTES divides, STANDING_FILL_PARENT_BYTES comes off the box first, and the cap is the cores — there is no width past which this pool stops paying that the arithmetic knows of. Under a gated cycle gate:make-test's pool comes off exactly as it does for the surface build, its bytes off the box and two cores off the cap. The plumbing step also opens beside gate:rebuild-contracts, whose pool nothing in the tree prices (`calibrate_budgets.UNITS` states that position), so that term is deliberately left out and a memo-drop pass oversubscribes the box for the pool's few tens of seconds rather than reserving for a worker no constant measures. gate:conform's belt can run beside the step too, and that overlap is priced on the belt's side, where `_conform_build_lane` takes this step's parent and pool off the box before the belt divides it, so this width leaves the belt out."""
    from rebuild.tools import memory_budget
...
def describe_carry_source(resolved: dict, root: Path, *, promoting: bool = False) -> str:
    """The one line that says which master the carry resolved to. A master stamped for an older surface than the served one is named as such and carried all the same: a verdict names its unit by content id, so it lands on the unit of that id on the live surface or on nothing, and never on the wrong window. On a promoting pass the aligned master is stamped for the surface the pass is about to replace — the resolution reads the live manifest before the move — and the line says so, since "the served surface" would be false the moment the promotion lands."""
    try:
...
def classify_rebuild_output(stdout: str, returncode: int, check: str) -> CheckVerdict:
    """Turn the rebuild suite's FAILED/ERROR summary lines into a gate verdict — the one judgment of the suite's output, shared by the cycle's rebuild gate and by the interactive wrapper (rebuild.tools.rebuild_gate). `check` names whose invocation this is ("rebuild-contracts") and does nothing else: it rides into the verdict so the record says which suite ran, while every line of the judgment above it stays blind to the lane. pytest emits ANSI color whenever FORCE_COLOR is set (as it is under the agent harness), wrapping each summary line in escape codes, so strip those first — otherwise no line begins with a literal "FAILED "/"ERROR " and a colored run reports the exit-code placeholder instead of naming its failures. Every failure is unexplained by definition — the suite carries no documented-baseline amnesty — and every green is recordable."""
...
def _do_assets_refresh(
    ...
    """Copy the review app's static files over the served surface and restamp the manifest's assets component, on the pass where that component is the only input that moved. It stands where the surface build would have stood, and everything downstream treats the pass as the skip it is: no unit can have changed, no shard, sidecar or `generated_at` moves, and so the carry is the identity and a listening server keeps its letters — livereload sees the copied files and reloads the tab onto the new shell."""
...
def _do_promote_surface(report: CycleReport, *, emit: console.Digest, plan: Plan) -> bool:
    """Move the rehearsal's surface into place, in this process, on the pass whose plan found one that reproduces these inputs byte for byte. It stands where the surface build would have stood and the build then reports itself skipped over the promoted manifest. It runs long after `_preflight` has stopped the server: the move replaces every shard and the stamp under the app, so it is never a stays-up pass."""
...
def _join_gates(
    ...
    """Fold every gate that ran into the report, and file each one's verdict under this run. Two of the four have no judge of their own — the JS suite and `make test` are pass/fail by exit code and always were — so their verdicts are built here rather than imported, which is what puts all four in the journal in one shape without inventing a judgment either of them does not make. gate:make-test's own wrapper stands down on CYCLE_RUN_ENV precisely so this line is the only one, and `make test`'s rc is honest for the font suite in a way run_m1's is not."""
