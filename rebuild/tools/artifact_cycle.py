...
# What one surface worker holds at its peak, and so the divisor a box divides itself by to reach this build's fan-out width. A worker is a persistent spawn process that enriches a contiguous slice of the corpus and retains every EnrichedUnit across phase 1 so that phase 2 can emit shard JSON from it, so what it holds is its own interpreter and shapers (whose shape memo is released at every unit batch boundary, `rebuild.review.ink.release_shape_memos`, so it holds one batch's windows rather than the slice's — a bound `_MemoizedShaper`'s docstring records as measured rather than assumed, since unreleased the memo alone outgrew every other pile of a serial build), one baseline subset table for every configuration its slice reaches — a slice of three units in a rare configuration pulls in a whole one, and each table is rebuild/review/enrich.py's SubsetRow projection of the three fields the enricher reads, glyph and seam strings interned, rather than every parsed rowmodel.Row — and then the slice's own retained units and fragments, which scale with the slice: half the corpus at width two against an eighth at width eight. That scaling is why the figure is seeded at width two, the narrowest pool the arithmetic ever starts and so the widest a worker ever gets: the first width-two pool's own records (2026-08-28, this corpus) read 14.66 and 14.77 GB per worker, where the 2026-08-27 eight-wide term-by-term measurement (844,512 units: a fixed base of 0.05 GB, four to six tables, 0.87 GB retained, 1.49 GB of fragments, a 0.43 GB pickle buffer in flight) put a typical worker near 7 GB — so a constant seeded narrow is an upper bound at every pooled width, where the eight-wide seed it replaces was outrun by 64% the first time a narrow pool ran. Every reading above was taken while those tables still held full Rows at 0.95 GB apiece, so the table term is narrower now than the seed prices it, and the seed stands as read until one pooled build over the projected tables re-prices it off the surface-worker row. It rounds up past the widest worker observed for the same reason kernel_exec.CONFIG_PEAK_BYTES rounds up past its own measurement, since a per-unit cost that errs low is what puts a box into swap while one that errs high only narrows a pool. `make job-costs`' surface-worker row is where it stays honest: rebuild/review/build.py files one kind:"pool" record per pooled build, so the constant is priced against the workers that actually ran — a row that goes legitimately quiet on a box this width has sent serial, since a serial build starts no pool. It is a reading to keep current, never a contract.
SURFACE_WORKER_BYTES = 16_000_000_000
# What the surface build's parent holds while that pool is live, and so what comes off the box before the division rather than into it: the parent holds the whole workload and every unit's projection and state, and phase 2's output only a chunk at a time — a fragment is pulled out of the worker that made it (or, serial, made from its retained enrichment) as the shard that takes it is being written, and released once the shard, the checker and the sidecar spools have had it, so what used to be every unit's fragment from the moment phase 2 started returning until manifest+check is now `_EMIT_CHUNK`-bounded residue plus the slim per-unit identity the cross-unit predicates keep; a served build reads its prior fragments back by address the same way instead of holding the previous surface's shards whole. It is flat in the width, which is exactly what makes it a co-resident term and not a divisor. The measurement is the `surface-build` step peak the cycle already stamps on every pass, which is honest for this term and only this one: peak_rss.reap_peak_rss_bytes maxes over the tree instead of summing it, and the widest single process under this step is the parent, the workers holding near-even slices of a corpus the parent holds whole. The seed is the first two cold serial builds on the streaming tree, which read 15.43 and 17.10 GB where the fragment-shaped pile had reached 20.72 GB on the same corpus, and it rounds up past the higher of them. The pile is still corpus-shaped — the workload and the projections scale with the units — so it stays a constant to re-seed as the alphabet migrates, off the surface-parent row of `make job-costs`; until this box has cycled through that row's recency window the row still carries the pre-streaming high-water marks and reads loud against this seed, which is the transient calibrate_budgets' window docstring anticipates rather than a reason to seed back up. What the streaming did not buy is width: with the worker still priced at its pre-projection seed, neither box in the fleet comes off its floor of one until a pooled build re-prices SURFACE_WORKER_BYTES.
SURFACE_PARENT_BYTES = 19_000_000_000
