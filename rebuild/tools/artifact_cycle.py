...
# What the surface build's parent holds while its pool is live, subtracted from the box before dividing by the worker cost: the workload's `audit.Unit` list — every unit's codepoints, its id, its config classes, its render groups and its echo — the packed unit store (rebuild/review/unit_store.py) over the unit's ordinal, the checker's identity dict, the census's premerge capture, one fragment in hand, and one batch reply in flight per worker, which is the one term of it the width moves. The store is where every per-unit product of phase 1 lives from the plan boundary to the cache write — the machine flags and ink deltas, the diff and cluster digests, the seam-home projection with its spans, names and rects, the secondary-home assignments, the fragment's spool or prior address, the shard address the write hands back, the content and input keys, the policy file and the config note — as fixed-width `array` columns and one string table over the vocabulary rather than the corpus (the `[tally] … unit_store` line prices the table beside the columns), materialized one unit at a time for the reduce or the store line that asks for one; a slotted state record, a spool-address object and a written map keyed by id hold the same facts at roughly six- to twentyfold the bytes, pile by pile (the `[tally]` lines of a pass under `AMS_SURFACE_PILE_TALLY=1` read the walked and packed bytes of each pile side by side), which is the reading this shape is chosen off. Neither ink-signature table is in it: the window-keyed table the ink-duplicate merge reads (`signatures` in rebuild/review/build.py) is released where that merge returns, in the load phase, and the store's records (`signature_entries`) are held through the plan phase and die when the store is written — inline on the main thread as the units phase opens on a serial build, and a few seconds into the units phase on a pooled build, when the thread `build_m1` starts at that boundary (`_SignatureWrite`) finishes its write and drops them — so the records overlap the pool only for the write's length; the load boundary's `signatures` tally line prices that table strings included, and the digest strings are shared with the records, so the two tables together are what the release takes off, not the sum of their lines. What this figure prices is the whole surface-build step rather than any one phase of it. On a full-fresh pass the `[t] review.build` lines read load, plan and units-through-cache within a tenth of a gigabyte of each other — the load holds the audit rows, the units and both signature tables, and the plateau holds the Unit list, the store's columns, the identity dict and the premerge capture in their place — so no boundary stands out as the mark. A served pass reads above a full-fresh one, not under it: its plan phase parses every store line the workload names into a `unit_cache.ServedUnit` before folding it into the columns and releasing it, so the plan boundary is that pass's mark and the step's, two gigabytes over the full-fresh plateau; a `load_store` that streamed each record through a sink is the lever on it, and until it lands the served pass is the one the seed covers. Fragments stream by address into the shards and are released after the checker and sidecar spools consume them. Before the workload loads the same process packs the baseline subset tables (rebuild/review/subset_pack.py; `write_pack` holds every table's keys and row indices beside the pool of distinct rows), a transient well under the pool-live pile that this constant covers without a term of its own. The pile grows with the alphabet. The surface-parent row of `make job-costs` measures it through the surface-build step's peak: `peak_rss.reap_peak_rss_bytes` takes the maximum process peak in the tree, and the parent holds the corpus while each worker holds one batch. The seed is three untallied width-eight passes at the qsYe corpus on the fleet's 32 GiB box (`doc/fleet.md`) with no surface code changed between them: two full-fresh, one driven by `make artifact-cycle` and one by hand, whose step peaks read 5.36 GB and 5.31 GB, and one served over the surface the hand pass wrote, whose step peak reads 7.42 GB at its plan boundary. It rounds above the served reading by more than half a gigabyte, wider than the tenth the full-fresh spread asks for, because a served pass's plan phase is one parse over the whole store and its run-to-run spread is not measured beside it; for the reason every cost here rounds up, one that errs low is what walks a box into swap, while one that errs high only narrows a pool. A tallied pass reads above the same pass untallied — it rebuilds the plan's input-key map at every boundary and holds a served pass's parsed records past the `del` that frees them — so re-seed from the row's step peak, never from a tally line. The row considers measurements after the commit that sets the constant, so older implementations' peaks do not price this one. The terms left to move are the `Unit` list, the largest pile the plateau walks, the load phase's audit rows, which the load holds and `release_rows` drops as the plan opens, and the served pass's parse: a columnar `Unit` is the lever on the first, a worker reading its batch's rows back from the audit file by offset while the parent keeps `row_count` and the offsets the lever on the second, and the streaming sink the lever on the third. Re-measure as the alphabet grows.
SURFACE_PARENT_BYTES = 8_000_000_000
