...
# What the surface build's parent holds while its pool is live, subtracted from the box before dividing by the worker cost: the whole workload, each unit's state and projection, the ink-signature store's entries (`signature_entries` in rebuild/review/build.py, held for the store write; the window-keyed table the ink-duplicate merge reads, `signatures`, is released where that merge returns, in the load phase, and what that frees is its `(codepoints, config)` tuple keys and its container, since its digest strings are the entries' own and stay until the store write — the load boundary's `signatures` tally line prices the whole table, strings included, so it overstates the release by the strings' share), unit-store records and addresses, content keys, one fragment in hand, and one batch reply in flight per worker, which is the one term of it the width moves. Fragments stream by address into the shards and are released after the checker and sidecar spools consume them. Before the workload loads the same process packs the baseline subset tables (rebuild/review/subset_pack.py; `write_pack` holds every table's keys and row indices beside the pool of distinct rows), a transient well under the pool-live pile that this constant covers without a term of its own. The pile grows with the alphabet. The surface-parent row of `make job-costs` measures it through the surface-build step's peak: `peak_rss.reap_peak_rss_bytes` takes the maximum process peak in the tree, and the parent holds the corpus while each worker holds one batch. The seed rounds above the peak this implementation reaches at the qsJay corpus — 9.39 GB, a signature-warm, unit-cold width-eight pass on the fleet's 32 GiB box (`doc/fleet.md`) at its units-through-cache plateau, with the signature table released at the merge — by about two hundred megabytes. A served pass over the same corpus, where the parent alone carries the store records after the pool has closed, peaks at 8.44 GB in its census-facts and cache phases, under the cold pass's plateau, so the full pass is the one the seed covers and the one to read when re-seeding. The row considers measurements after the commit that sets the constant, so older implementations' peaks do not price this one. Re-measure as the alphabet grows.
SURFACE_PARENT_BYTES = 9_600_000_000
