...
# What one oracle row-range worker holds at its peak, and so the divisor of the post-build sweeps' width (`sweep_job_budget`). A worker is a spawn process holding its interpreter, a HarfBuzz shaper over M1.otf, the crate's formation surface, the records of its own row range of its configuration's row store — one buffer of the range's record bytes with three packed arrays beside it, an offset and two ages a record, kept out of a scan of the whole member (`oracle_cache.load_store`), so what it holds of the store is its range's rows and not the configuration's — its configuration's settle memo, mapped on the first wave that reaches the crate, which every pass reaches since the renewal slice re-derives one row in `oracle_cache.MAX_RECORD_AGE` — `conform._MemoStore` over the file's own layout, the six id columns, the value column and the 2^k >= 2N-slot probe index as views over the mapping, so those pages are the page cache's, resident once a box however many workers map the file and in a worker's resident set as its probes touch them — the load reads the header and the two tables and a column page only when a family moved, for the retirement fold, and a store-cold pass, whose every row reaches the walk, probes nearly every row, so such a worker's reading carries most of the file's extent, shared with every other worker of the pool — while its heap holds the file's interned label and outcome tables, a dead byte and a reached byte a row, about 0.02 GB over the windows the `[t] settle_memo` lines count; the writer's pile, for the unit that writes: an uncut range that settled a window fresh files the whole file, and the parent's part absorb, one task per cut configuration on this same pool once every range has landed, carries the standing rows whole beside the parts and the standing index — either holds the columns, the writer's folded copies and the index at once (`conform._write_settle_memo`), roughly the file's size plus the columns', and the absorb's reading is filed in the pool record beside the ranges' as `<config> absorb`, a process high-water mark like the rest, so it reads at or above the range its worker ran before it — and the walk's own state over the range: the chunk of rows in flight (`oracle.ORACLE_ROW_CHUNK`), the waves of windows the crate settles for it, and `windows`, the dict holding the entries the walk promotes out of the columns or settles fresh. The staged measurement is one process running `oracle.oracle_config_worker` over one shard of `oracle.oracle_shard_plan`, its row cache opened store-warm (`read_dir` the out dir) or store-cold (`read_dir=None`), stamped with `resource.getrusage`'s `ru_maxrss` and the resident set from `ps` at the interpreter, after the imports, spec, `kernel_exec.guard_sweep` and shaper, around `oracle.open_row_cache`, around `_SettledWindowWalk._load_memo` with the store's column, index and reached sizes printed, and at return; the probe and both its logs are kept beside the seed passes' logs under `var/keep/rung263-2/stage-probe/`. The seed's passes and probe hold the memo as heap columns rather than as a mapping — 72.5 MB of columns and 33.6 MB of index inside the load's 0.13 to 0.14 GB below — so the seed bounds a mapping worker's memo term from above by that whole, and the oracle-shard row of `make job-costs` over passes that map the file, absorb readings included, is the next seed's input. Over the 708,015-row `default 1/3` of the width-twelve plan the terms read: the interpreter with its imports, spec, formation surface and shaper 0.06 GB; the range's store records 0.05 GB resident behind a 0.15 GB peak at the load store-warm, and nothing store-cold; the first chunk and its wave, held before the memo loads, 0.14 GB warm and 0.15 cold; the memo load 0.13 to 0.14 GB, of which the columns are 72.5 MB, the index 33.6 MB and the reached bytes 2.6 MB; and the walk from there to the process's peak 0.20 GB store-warm, promoting 72,159 windows, and 0.26 GB store-cold, promoting 1,171,462 — a peak of 0.58 GB warm and 0.61 cold, which is what the pool records read for that range. The largest term as measured is the walk over the range's rows, 0.34 to 0.40 GB from the first chunk to the peak, better than twice the memo: the memo-less ss10 ranges, whose overlay walk loads nothing, read 0.32 to 0.33 GB store-cold against 0.45 to 0.74 for the ranges holding one, the 0.45 being the narrowest of them, the 154,476-row `ss03 1/3`, and a cold pass's widest ranges read highest because that state grows with the range — at width ten the 849,618-row ranges read 0.71 to 0.74 GB cold against 0.45 to 0.64 for the nine other memo-holding ranges. Read a pool record for the box-wide shape and never for one range's cost: `run_m1._spawn_pool` sets no `maxtasksperchild` and a worker reports its process high-water mark (`peak_rss.peak_rss_self_bytes`), so a range that runs second in a reused worker reads at or above the mark the range before it left, whatever its own rows. In the store-warm width-twelve seed record the 128,730-row memo-less `ss10 1/2` reads 0.61 GB, the mark of the range its worker ran first, where its sibling `ss10 2/2` at 1,416,030 rows reads 0.43; at width ten the five smallest ranges, which are the five that run second, read 0.62 to 0.71 GB store-warm, four of them above every first-wave 849,618-row range's 0.62 to 0.64 and the fifth, the 77,238-row `ss03+ss05 3/3`, at 0.62 beside the lowest of those, and the record's own maximum is one of the four; the store-cold records, where a second runner reads at the pack, hide the carry, and there `ss10 1/2` reads 0.33 GB, its sibling's mark. The seed is four `run_m1 --gates-only` passes on the 32 GiB box (`doc/fleet.md`), store-cold (`--fresh-oracle-cache`) and store-warm, at `--jobs 10` over fifteen ranges and — standing in for the 48 GiB box, whose twelve cores choose that plan — at a hand-stated `--jobs 12` over seventeen, with every worker holding its own range's records beside its columnar memo: the highest reading is 0.63 GB store-cold and 0.69 GB store-warm at width twelve, 0.74 GB store-cold and 0.71 GB store-warm at width ten, and one more store-warm width-ten pass over the same columnar shape, the pool record finished at 2026-09-16T05:17:40Z, reads 0.73 GB. It rounds up past the highest of those by a quarter and then to the tenth, for the reason every divisor here rounds up — a per-unit cost that errs low is what puts a box into swap — and because the memo and the walk's state both grow with the alphabet, so a migration moves it. `make job-costs`' oracle-shard row is where it stays honest: `run_m1.run_oracle` files one kind:"pool" record per fan-out, one observation per row range that ran, so the constant is priced against the workers that actually ran, and the cycle's job-costs step is the check that runs it; the rebuild suite holds the constant to the cores and says nothing about a figure that errs low. At this figure neither box in the fleet is memory-bound for this pool: the cores are.
ORACLE_SHARD_BYTES = 1_000_000_000
