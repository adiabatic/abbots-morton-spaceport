...
# What one configuration's walk holds at its peak: the engine's trace memo over every distinct window the texts naming one family reach at horizon 5, which on the live alphabet with ·Jay is thirteen million windows. Measured with one `replay-strings` over `default` alone at horizon 5, `--families=qsAh`, `--threads=1`, under `/usr/bin/time -l`, with the trace key packed onto ordinals and the trace entry at sixteen bytes: 14.67 GB resident, the `peak memory footprint` line level with it, 7,254,381 texts over 13,084,282 windows in 72 s of walk; the same walk five configurations wide, on the alphabet with ·Zoo, paged the box (19 GB resident, the system time several times the user time) and ran each configuration seven times slower, which is why the width divides the box by this figure rather than assuming the configurations fit together. The seed sits a gigabyte above the reading rather than at the tenth: the walk grows with every letter, since the texts naming one family reach more windows as the alphabet does, the cost of a seed that errs low is that paged run, and the gigabyte costs neither fleet box a walk — the 32 GiB Mac walks one configuration at a time and the 48 GiB box two, which `rebuild/test_deep_replay.py` pins. Re-measure it the same way whenever the alphabet grows or the trace memo's key or entry changes shape.
DEEP_REPLAY_PEAK_BYTES = 15_700_000_000
