...
def _entry_drop_holds(match, before, after, intern):
    """Whether one pivot piece is the named entry drop: same height, same own-frame origin, both on the grid, and the after picture is the before picture with the named left-side columns gone and the remaining cells shifted left by that count — no cell dropped outside those columns, no unnamed cell gained."""
    if before[3] != after[3] or before[4] != after[4]:
        return False
    if before[2] % PIXEL_SIZE or after[2] % PIXEL_SIZE or before[3] % PIXEL_SIZE:
        return False
    ...
    if painted is None or kept is None or not kept:
        return False
    columns = match["after"]["entry_drop"]
    shifted = {(column + columns, row) for column, row in kept}
    dropped = painted - shifted
    return bool(dropped) and not (shifted - painted) and all(column < columns for column, _row in dropped)


def _entry_geometry(match, unit, comparator):
    """Whether the window's rendered before→after change is exactly the named left-side entry stretch coming off the named pivot, re-derived from the fonts: shape the window under one of the unit's configs, judge each pivot piece as the same picture compacted left by the declared column count at the same placement, height, and own-frame origin, and require every span strictly between the pivots to render identically once displaced by the cumulative drop — the span before the first pivot by nothing, the span after the first pivot by the full drop, and one more drop for every further pivot. Anything the contract cannot hold — no pivot on the before side, pivot counts that disagree, a shaped run that contradicts the unit's recorded glyphs, an off-grid placement, a non-rectilinear outline, a dropped cell outside the named columns, an unnamed extra cell — reads as no match, so the unit queues."""
    codepoints = unit.get("codepoints") or ""
    if not codepoints:
        return False
    try:
    ...
    for before_index, after_index in zip(before_pivots, after_pivots):
        before, after = before_run[before_index], after_run[after_index]
        if before[2] != after[2] or not _entry_drop_holds(match, before, after, intern):
            return False
    for step, (before_span, after_span) in enumerate(
        zip(_split_around(before_run, before_pivots), _split_around(after_run, after_pivots))
    ):
        if not _span_settled(intern, before_span, after_span, -columns * step):
            return False
    return True


def _matches_entry_drop(match, unit, excluded, context=None):
    """A letter that gives up a named stretch of left-side entry, matched at the rendered-pixel grain: the old-font pivot form gives way to a named new form whose own-frame picture is the old one compacted left by the declared column count, origin and placement standing still, everything after the pivot sliding closer by that count. Same origin and placement pin the lost ink to the letterform rather than to a slide of the pivot itself, and any other ink change anywhere in the window fails this match closed — which is where the composed reading picks up, so a window this shape refuses only because a second separately-blessed change moved a pixel it has no vocabulary for may still be explained by both rules together. One shaped config speaks for all of them, on the same digest-agreement precondition the slide shape holds. except_left reads the whole window, as the slide and ink-gain shapes do."""
    deltas = unit.get("ink_deltas")
    if not isinstance(deltas, dict) or not deltas:
        return False
    if len(set(deltas.values())) != 1 or set(deltas) != set(unit.get("configs") or []):
...
def _validate_entry_drop(rule_id, match) -> None:
    ...
    if match["after"]["entry_drop"] == 0:
        _fail(
            f"rule {rule_id!r}: match.after.entry_drop is 0; an unmoved window is ink-identical and "
            "machine-approved already"
        )
    if match["after"]["entry_drop"] < 0:
        _fail(
            f"rule {rule_id!r}: match.after.entry_drop is {match['after']['entry_drop']}; an entry drop "
...
def _retarget_piece_holds(before, after):
    """Whether one piece of a retargeted join kept its own-frame origin, both on the grid. Height and picture may change — that is the shape's point, a join moving to a new row with both letters free to redraw. Placement under a running displacement is the walk's job, not this contract's."""
    if before is None or after is None:
        return False
    if before[4] != after[4]:
        return False
...
def _retarget_geometry(match, unit, comparator):
    """Whether the window's rendered before→after change is exactly the named join changing height, re-derived from the fonts: shape the window under one of the unit's configs, find every pivot–follower pair whose recorded seam moved from the named height to the named new height and whose after cells the rule names, require both letters to keep their column placement and own-frame origin, and require each span strictly outside those pairs to render identically once displaced by the cumulative shift — the span before the first pair by nothing, the span after the first pair by the full shift, and one more shift for every further pair. Anything the contract cannot hold — no pair, a shaped run that contradicts the unit's recorded glyphs, a pivot or follower that slid or whose origin moved, an off-grid placement — reads as no match, so the unit queues."""
    codepoints = unit.get("codepoints") or ""
    if not codepoints:
        return False
    try:
    ...
    for index in pairs:
        for piece_index in (index, index + 1):
            ...
            if before is None or after is None:
                return False
            if not _retarget_piece_holds(before, after) or before[2] != after[2]:
                return False
    intern = comparator.intern
    shift = match["after"]["shift"]
    pivots = set(pairs)
...
def _matches_join_retarget(match, unit, excluded, context=None):
    """A named join that has changed height, matched at the rendered-pixel grain: the pivot and the follower may both redraw, they keep their column placement and own-frame origin, the named seam becomes the named new height, and everything after the follower sits the declared number of columns over. Origin and column unmoved pin the change to the join's row and the two letters' forms rather than to a slide or a dropped attachment, and any other ink change anywhere in the window fails this match closed — which is where the composed reading picks up, so a window this shape refuses only because a second separately-blessed change moved a pixel it has no vocabulary for may still be explained by both rules together. One shaped config speaks for all of them, on the same digest-agreement precondition the slide shape holds. except_left reads the whole window, as the slide and join-dropped shapes do."""
    deltas = unit.get("ink_deltas")
    if not isinstance(deltas, dict) or not deltas:
        return False
    if len(set(deltas.values())) != 1 or set(deltas) != set(unit.get("configs") or []):
    ...
    verdict = context.memo.get(key)
    if verdict is None:
        verdict = context.memo[key] = _retarget_geometry(match, unit, context.comparator)
    if not verdict:
        return False
    return not any(_joining_family(name) in excluded for name in unit["before"]["glyphs"])
...
def _entry_event(match, rule_id, index, after_names, intern, before_pieces, after_pieces):
    """Whether one entry-drop candidate's own contract holds at the rendered grain, one position at a time: the after side settles into one of the rule's named after forms, and the pivot keeps its height and own-frame origin while its after picture is its before picture compacted left by the declared column count. Placement under the running displacement is the walk's job, not this contract's, mirroring `_gain_event` leaving the span equality to the walk. None when any of that fails, which leaves the piece to be judged as ordinary span ink."""
    before, after = before_pieces.get(index), after_pieces.get(index)
    if before is None or after is None:
        return None
    if not _named_pivot(after_names[index], match["after"]["pivots"]):
...
def _retarget_event(match, rule_id, index, before_pieces, after_pieces):
    ...
    if not _retarget_piece_holds(before_pieces.get(index + 1), after_pieces.get(index + 1)):
        return None
    return Event(rule_id, "retarget", match["after"]["shift"])
...
def _composed_walk(rules, unit, context):
    ...
    if any(index + 1 in events for index, event in events.items() if event.kind == "retarget"):
        return None
...
SHAPES = {
    ...
    "join-retargeted": Shape(
        keyed_by="retarget",
        before=("pivot", "seam_out", "follower"),
        after=("retarget", "pivot_cells", "receiver_cells", "shift"),
...
def rule_reach(rules, units, records, stamp, context=None) -> Run:
    """The whole pass in one place, so the records a run writes and the tally it reports can never disagree about what any rule reached: the composed reading first, because it claims a window before any single rule is asked about it, then each rule's own matcher over what is left. A caller outside the CLI — a validators-lane test holding the checked-in rules against the live surface — gets the same numbers the run printed, without re-deriving a single matcher decision."""
