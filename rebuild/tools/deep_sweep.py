"""The periodic deep form of gate:conform (issue #74): the same exhaustive font-vs-settle sweep the per-edit belt runs, taken to horizon 5 or deeper. The belt shapes every string up to four tokens on every edit and is chartered for what only shaping the real binary can test — HarfBuzz's application semantics and whether the six-slot window abstraction is itself sufficient. This tool is where that second question gets asked at a depth the belt cannot afford, over strings long enough to reach past the window's own reach.
...
The structural check the belt runs — every splitter-separated buffer identical to its segments shaped alone — comes along at this depth, which is where its coverage past the belt's horizon now lives: the standalone horizon-5 boundary gate that used to prove it on every build is gone, and nothing else shapes a length-5 string. The ZWNJ slot's own structure — zero advance, no ink — needs no depth at all: read-back proves it statically off the font bytes once per build.
