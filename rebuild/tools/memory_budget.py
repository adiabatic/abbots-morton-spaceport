...
`AMS_TOTAL_MEMORY_BYTES` is the one environment variable here, and it overrides the probe and never the policy: a container states its own allowance, a large box reproduces a small box's widths, and a `--dry-run` prints the same plan on every machine. Junk in it is ignored rather than raised on, because a typo in a reproduction knob must not take a build down. `AMS_KERNEL_THREADS` and `PYTEST_XDIST_AUTO_NUM_WORKERS` keep winning unconditionally over every width in the tree they reach; this module neither reads nor undercuts them, and a call site that honors one honors it before it asks anything here. The two pools sized at the cores alone (`run_m1._core_bound_threads`: the window packing and the shipped-order walks) take no override, since what they hold is outside anything the kernel's division prices.
