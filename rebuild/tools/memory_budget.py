...
No per-unit cost lives here, and the omission is the point. The repo has exactly three genuinely memory-bound fan-outs, and each argues its width from facts that are not fungible — a live configuration holding its whole working set until it has written its artifacts for the kernel fan-out, a transient peak plus a non-memory tail for the rebuild validators lane, a parent holding the whole corpus beside workers holding slices of it for the review-surface build, where what is divided is the box less that parent — so a central `UNIT_COSTS` mapping would hold the numbers while leaving their arguments behind at the call sites. Costs stay at their call sites as named constants, in the docstring that already has to justify the width, and this module owns only the arithmetic. `CONFIG_PEAK_BYTES` in `rebuild/pipeline/kernel_exec.py` is the worked example and the shape a second call site should copy: the constant, one sentence naming what measured it and where that measurement is still reported, and a single `how_many_fit` resolved right where the width is named, so the argument for a width and the width itself are never in different files. The validators lane is the other fan-out and is on this arithmetic now, in exactly that shape: `VALIDATORS_WORKER_BYTES` in `rebuild/conftest.py` — still the authority on it — is what one of its workers holds, `VALIDATORS_LANE_CAP` beside it is the non-memory bound the arithmetic cannot supply, and `VALIDATORS_WORKERS` is one `how_many_fit` over the pair, so a small box narrows to what it has room for while a large one stops where widening stops paying. The review-surface build is the third, in `rebuild/tools/artifact_cycle.py` and in the same shape with one term more: `SURFACE_WORKER_BYTES` is what one worker holds and is the divisor, `SURFACE_PARENT_BYTES` is the parent that holds the whole corpus at any width and so is subtracted from the box before the division rather than smeared through it, and `SURFACE_JOBS_CAP` is the non-memory bound — the one call site here whose flat half is as large as its divided half, which is why it names two constants rather than one. The root `conftest.py` prices the lane-less runs it answers for at that same per-worker figure, and is the one place in the repo where a per-unit cost is deliberately written twice: importing it would execute a second copy of a conftest, so the comment there names the authority and a check in `rebuild/test_memory_budget.py` holds the two literals equal.
