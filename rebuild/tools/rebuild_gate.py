...
def _run_lane(lane: str, env: dict[str, str], force: bool) -> tuple[int, bool]:
    """Run (or validly skip) one lane, returning its exit code and whether it actually spawned a suite. A nonzero code is a hard failure and stops the run; every other outcome — a skip, a clean green, a baseline-only green, a green whose closure drifted — is a zero the caller carries on from."""
