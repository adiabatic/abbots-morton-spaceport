...
AMS_RUN_PYRIGHT rides the environment into the spawned suite: pyright checks the whole tree from `[tool.pyright] include`, and the suite's root conftest starts it in pytest_configure and joins it in pytest_sessionfinish, so it runs beside the whole xdist pool rather than ahead of it — under its own green record (`rebuild.tools.pyright_gate`), so a run this wrapper narrows to a rune edit's tests spawns no type check at all. A red check exits the suite nonzero with no FAILED/ERROR line, which `classify_rebuild_output` buckets as a hard failure, so this wrapper goes red and clears the contradicted lane green the same as for a failed test.
