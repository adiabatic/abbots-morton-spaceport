"""The on-demand kernel differential (issue #40; the standing per-cycle form retired at the issue #48 cutover): whenever either engine's kernel sources move, prove that the Rust kernel and the Python kernel still fold to the same three artifacts and the same contract digest. Rust builds the cycle's tables now and Python's settle kernel still ships — `gate:conform` re-settles every swept string through it and `emit_gsub` calls `formation_blocked` — so every future settlement-semantics change is written twice, and this gate is the permanent enforcement of that tax: its key is kernel code alone, so it arms exactly when the twin implementations could have drifted and never on a rune edit.

Both sides are built here. Pre-cutover the gate compared the kernel's fold against the artifacts the cycle's Python engine had already written, and refusing a rust-stamped table set was its first check; now that the artifacts on disk are the kernel's own fold, reading them would compare the kernel against itself, so the Python side is enumerated fresh in-process — one fixpoint per configuration, the cost the per-cycle gate once existed to spare, acceptable in a gate that only ever arms on a kernel edit. Each side's product folds through the same `assemble_tables` and the same writers, and both windows heads carry `kernel_fixpoint.INPUTS_STAMP`: the stamp names sources, this comparison is about semantics, and a real fingerprint would put a value that moves on every rune edit inside a byte comparison. `rebuild/tools/kernel_fixpoint.py` remains the deeper standalone harness — stream bytes, three worlds, fan-out widths — where this gate compares the shipping world at artifact grain.
