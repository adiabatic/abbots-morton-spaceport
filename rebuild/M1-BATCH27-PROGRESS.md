...
- **Blocking: the Manual pin at the-manual.html:3883, `·Day | ·Ye ~b~ ·Ooze ~b~ ·Tea | ·It ~b~ ·Zoo`, fails** (the rebuild draws `·Ooze | ·Tea.half ~x~ ·It ~b~ ·Zoo`). ·Ye brings the sentence into scope; the grouping itself predates ·Ye and is the standing ·Tea·Day tight-bond preference every baseline exiter carries (qsJai's `exit none over exit baseline when right ·Tea then anything but ·Pea`, on qsOoze among the rest), which declines `·Ooze ~b~ ·Tea` whenever ·It follows. The old font keeps the left join into ·Tea and lets unentered ·It exit at the baseline before ·Zoo, ·Day, ·I and ·Utter, for every baseline left. The pin gate has no waiver channel, so the batch cannot land until the author picks one: except `·It then ·Zoo` (or the whole ·Zoo/·Day/·I/·Utter set) from the tight-bond records, or amend the pin. A qsTea-side yield (`entry baseline over exit x-height when right ·It then ·Zoo`) was tried and is incomparable with those records at non-nested specificity, so it is not a way out. Until the pin is green the run_m1 gate stops before the oracle, so the oracle score below is unmeasured.
- **Blocking: read-back's GSUB budget.** With ·Ye in the alphabet the settlement lookup's subtable-offset headroom falls a few hundred bytes under the floor `readback.SUBTABLE_OFFSET_HEADROOM_FLOOR` holds the font to (`rebuild/out/m1/readback_summary.json` reports the headroom and the subtable count), so the build stops at read-back before the pin gate and the oracle. The floor is the early warning, well ahead of the uint16 wall; the two times it fired before were answered in the encoding (the Extension promotion, the format-2 repack), never by moving the floor. Most of the alphabet still to migrate is ahead of this one, so it wants a structural answer (a further repack, or splitting the settlement lookup) or an argued new floor; both are the author's call and belong to the speed-up tracker, not this batch.
