//! The settlement vocabulary: the values `rebuild/pipeline/settle.py` hands between its stages, ported so that every string Python carries is one of three things here — an interned [`Sym`] when the spec authored it, a closed enum when the kernel itself closed the set, or an owned `String` when nothing but formatting ever reads it.
//!
//! That split is the module's whole rule. A rune, a stance, a height, a bitmap name and a class name are authored, so they ride as symbols and compare on a `u32`; the six right-token kinds, the four word positions, the elimination and decided stages, and the adjustments grammar `model.py` documents are the kernel's own closed vocabularies, so they ride as enums and cannot be misspelled. Python's `left.kind != cond.is_token` compares one of each — a kernel kind against an authored value — and its twin here compares the condition's symbol against [`Vocab`], the one place the closed vocabulary is looked up in the spec's own string pool.
//!
//! Entry and exit *states* are the subtle case and are deliberately not `Option<Sym>`. Wherever Python compares a height against the literal `"none"` — pairings, `cells:` rows, `joined_at`, `self_entry` — the value being compared is a state, a single symbol that is either a height or the none state, and [`Vocab::height_state`] is the one crossing between it and the `Option<Sym>` form that means "this side is live at this height, or is not live at all". Keeping the two shapes distinct is what stops a `None` from silently comparing unequal to an authored `none`.
//!
