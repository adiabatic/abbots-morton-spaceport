//! The interned model: `rebuild/pipeline/model.py`'s dataclass tree with every vocabulary string collapsed to a `Sym` and every number to an `i64`. Prose — `ductus` values, `notes`, `why`, `description` — stays an owned `String`, because nothing downstream keys on a paragraph; everything else rides as an integer, which is the whole reason the port exists (RUST-PORT-PLAN.md: the packing is the win, not the language).
//!
