...
- [ ] **E2 — `halves-entry-extension-restored` (172 rejects).** The receiver entry-extension records keyed on `class: halves-that-exit-at-x-height` across `qsPea`/`qsMay`/`qsDay`/`qsTea` (and the `qsDay_qsUtter` ligature). Overlaps the round-2 "structurally impossible" analysis (`POLICY-ROUND-2-REPORT.md`) — re-read before touching. Decide: delete vs narrow per receiver.
- [ ] **E3 — `extension-non-summing` rejects (10).** ·May·Day·Utter family (`qsMay` `extend`, target ·Tea+Oy and ·May·Day). Keep the 20 approves (·Tea+Oy→·Day).
