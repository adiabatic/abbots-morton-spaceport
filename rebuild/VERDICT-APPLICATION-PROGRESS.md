...
- **ZWNJ rows are gone from UNMATCHED as of 2026-07-03** — the ratified ZWNJ-equals-word-boundary rule (see WHATNEXT.md) absorbs every non-position ZWNJ row into the `zwnj-boundary-echo` ledger class at top precedence, so per-config UNMATCHED diffs against pre-2026-07-03 audits will show ZWNJ windows vanishing; that is the rule working, not a regression. `run_m1.main()` now also runs the ZWNJ split-buffer gate (hard-fails on any post-ZWNJ vs word-initial rendering difference) before the oracle.
